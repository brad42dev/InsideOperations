...
async fn issue_saml_jwt(state: &AppState, user_id: Uuid) -> Response {
    ...
    let user_row = match sqlx::query(
        "SELECT id, username, full_name, email, is_service_account, is_emergency_account
         FROM users WHERE id = $1 AND enabled = true",
    )
    .bind(user_id)
    .fetch_optional(&state.db)
    .await
    ...
    let is_service_account: bool = user_row.try_get("is_service_account").unwrap_or(false);
    let is_emergency_account: bool = user_row.try_get("is_emergency_account").unwrap_or(false);
    ...
    if !is_service_account && !is_emergency_account {
        ...
        if eula_not_accepted {
            ...
            tracing::info!(user_id = %user_id, "SAML: EULA acceptance required — redirecting to EULA page");
