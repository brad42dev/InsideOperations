...
import type { WorkspaceLayout, LayoutPreset, PaneConfig, GridItem } from '../pages/console/types'
...
export function makeNewWorkspace(name: string, layout: LayoutPreset = '2x2'): WorkspaceLayout {
  return {
    id: uuidv4(),
    name,
    layout,
...
export interface WorkspaceState {
  ...
  updateWorkspace: (id: string, updater: (w: WorkspaceLayout) => WorkspaceLayout) => void
  ...
  swapPanes: (workspaceId: string, sourceId: string, targetId: string, gridItems: GridItem[]) => void
  ...
  temporal(
    (set, get) => ({
      ...
      createWorkspace: (name, layout = '2x2') => {
        ...
        set((s) => ({ workspaces: [...s.workspaces, ws], activeId: ws.id, editMode: true }))
        return ws
      },
      ...
      duplicateWorkspace: (id) => {
        ...
        set((s) => ({ workspaces: [...s.workspaces, copy], activeId: copy.id }))
        return copy
      },
      ...
      changeLayout: (id, layout) =>
        set((s) => ({
          workspaces: s.workspaces.map((w) => {
            ...
            const blankCount = Math.max(0, slotsRemaining - fromOverflow.length)
      ...
      updateGridItems: (id, gridItems) =>
        set((s) => ({
          workspaces: s.workspaces.map((w) => (w.id === id ? { ...w, gridItems } : w)),
        })),
      ...
      updatePane: (workspaceId, pane) =>
        set((s) => ({
          workspaces: s.workspaces.map((w) =>
            w.id === workspaceId
              ? { ...w, panes: w.panes.map((p) => (p.id === pane.id ? pane : p)) }
              : w,
          ),
        })),
          ...
          workspaces: s.workspaces.map((w) => {
            ...
            return {
              ...
              gridItems: gridItems.map((gi) => {
                if (gi.i === sourceId) return { ...gi, x: tgtItem.x, y: tgtItem.y, w: tgtItem.w, h: tgtItem.h }
                if (gi.i === targetId) return { ...gi, x: srcItem.x, y: srcItem.y, w: srcItem.w, h: srcItem.h }
                return gi
              }),
            }
          }),
      ...
      clearPanes: (id) =>
        set((s) => ({
          workspaces: s.workspaces.map((w) =>
            w.id === id
              ? { ...w, panes: w.panes.map(() => ({ id: uuidv4(), type: 'blank' as const })) }
              : w,
          ),
        })),
    }),
