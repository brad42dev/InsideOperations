...
export const useTabStore = create<TabStore>((set, get) => ({
  ...
  saveTabViewport(tabId, viewport) {
    set((state) => ({
      tabs: state.tabs.map((t) => (t.id === tabId ? { ...t, viewport } : t)),
    }));
  },
      ...
      tabs: state.tabs.map((t) =>
        t.id === tabId
          ? {
              ...t,
              graphicId,
              ...(graphicName !== undefined ? { graphicName } : {}),
            }
          : t,
      ),
    }));
  ...
  findTabByGraphicId(graphicId) {
    ...
    return (
      get().tabs.find(
        (t) => t.graphicId === graphicId && t.type === "graphic",
      ) ?? null
    );
  },
  ...
  closeGroupTabsForGraphic(graphicId) {
    ...
    const toClose = tabs.filter(
      (t) => t.type === "group" && t.graphicId === graphicId,
    );
    ...
    if (activeTabId && closeIds.has(activeTabId)) {
      ...
      const parentTab = remaining.find(
        (t) => t.graphicId === graphicId && t.type === "graphic",
      );
      if (parentTab) {
