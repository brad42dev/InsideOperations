...
export function useOfflineRounds() {
  ...
  const refreshCounts = useCallback(async () => {
    try {
      const db = await openDb();
      const items = await new Promise<PendingRoundResponse[]>(
        (resolve, reject) => {
      ...
      const failures = items.some(
        (item) => item.retryCount > 0 && !item.requiresReview,
      );
  ...
  const saveOfflineResponse = useCallback(
    async (
      response: Omit<
        ...
        "id" | "synced" | "idempotencyKey" | "retryCount"
      >,
    ): Promise<void> => {
      const db = await openDb();
      return new Promise<void>((resolve, reject) => {
  ...
  const getPendingResponses = useCallback(async (): Promise<
    PendingRoundResponse[]
  > => {
    const db = await openDb();
    return new Promise((resolve, reject) => {
