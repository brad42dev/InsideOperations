...
async function deleteTilesForGraphic(db: IDBDatabase, graphicId: string): Promise<void> {
  const all = await getAllTiles(db)
  const toDelete = all.filter((r) => r.graphicId === graphicId).map((r) => r.id)
...
export async function evictLRU(): Promise<void> {
  ...
  const graphicStats = new Map<string, { lastAccessed: string; totalBytes: number }>()
  let totalBytes = 0
  ...
  const sorted = Array.from(graphicStats.entries()).sort(
    ([, a], [, b]) => a.lastAccessed.localeCompare(b.lastAccessed),
  )
...
export async function getTile(
  ...
  const record = await new Promise<TileCacheRecord | null>((resolve, reject) => {
    const tx = db.transaction(STORE_NAME, 'readonly')
