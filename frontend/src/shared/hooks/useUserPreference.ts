...
export function useUserPreference<T extends Primitive>(
  ...
  const [value, setValueState] = useState<T>(() => {
    try {
      ...
      if (typeof parsed === typeof defaultValue || parsed === null) return parsed as T
      return defaultValue
    } catch {
      return defaultValue
    }
  ...
  useEffect(() => {
    let cancelled = false
    preferencesApi.get().then((result) => {
      if (cancelled || serverLoadedRef.current) return
      serverLoadedRef.current = true
      if (result.success) {
        const serverValue = (result.data as Record<string, unknown>)[key]
        if (serverValue !== undefined && serverValue !== null && typeof serverValue === typeof defaultValue) {
          const typed = serverValue as T
          setValueState(typed)
          try { localStorage.setItem(LS_KEY, JSON.stringify(typed)) } catch { /* ignore */ }
        }
      }
    }).catch(() => { /* network error — keep localStorage/default */ })
  ...
  const setValue = useCallback(
    (newValue: T): void => {
      ...
      try { localStorage.setItem(LS_KEY, JSON.stringify(newValue)) } catch { /* ignore */ }
      // Debounce server write to avoid one request per keystroke / rapid toggle
      if (debounceTimerRef.current) clearTimeout(debounceTimerRef.current)
      debounceTimerRef.current = setTimeout(() => {
        preferencesApi.patch({ [key]: newValue }).catch(() => { /* best-effort */ })
      }, 500)
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [key, LS_KEY],
