...
const G_KEY_MAP: Record<string, string> = Object.fromEntries(
  ROUTE_REGISTRY.filter((r) => r.g_key).map((r) => [
    r.g_key.split(" ")[1].toLowerCase(),
    r.path,
  ]),
);
...
const _gKeyTimerRef: { current: ReturnType<typeof setTimeout> | null } = {
  current: null,
};
...
const _setGKeyHintVisible: { current: ((v: boolean) => void) | null } = {
  current: null,
};
...
const _navigateRef: { current: ((path: string) => void) | null } = {
  current: null,
};
...
function buildBreadcrumbs(pathname: string): Crumb[] {
  if (pathname === "/" || pathname === "")
    return [{ label: "Inside/Operations", path: "/" }];
...
function AlertBell() {
  ...
  const canAcknowledge =
    user?.permissions.includes("alerts:acknowledge") ?? false;
  ...
  const recentAlerts = (recentAlertsRaw ?? []).slice().sort((a, b) => {
...
function CornerTrigger({ corner, onDwellComplete }: CornerTriggerProps) {
  ...
  useEffect(() => {
    function isInCorner(x: number, y: number): boolean {
      ...
      switch (corner) {
        case "tl":
          return x <= CORNER_ZONE && y <= CORNER_ZONE;
        case "tr":
          return x >= w - CORNER_ZONE && y <= CORNER_ZONE;
        case "bl":
          return x <= CORNER_ZONE && y >= h - CORNER_ZONE;
        case "br":
          return x >= w - CORNER_ZONE && y >= h - CORNER_ZONE;
      }
    }
  ...
  return (
    <div style={containerStyle} data-testid={`kiosk-corner-trigger-${corner}`}>
      {showButton && (
        <button
          style={buttonStyle}
...
export default function AppShell() {
  ...
  const sidebarEdgeDwellRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const preKioskSidebarRef = useRef<"expanded" | "collapsed" | "hidden">(
    "expanded",
  );
  ...
  function enterKiosk() {
    ...
    showToast({
      title: "Kiosk mode active. Press Escape to exit.",
      variant: "info",
      duration: 2000,
    });
  ...
  useEffect(() => {
    ...
    async function bootSyncLock() {
      try {
        ...
        if (
          typeof session.idle_timeout_ms === "number" &&
          session.idle_timeout_ms > 0
        ) {
  ...
  return (
    ...
    >
        ...
        <>
          ...
          {sidebarHidden && (
            <div
              ...
              onMouseEnter={() => {
                if (sidebarEdgeDwellRef.current)
                  clearTimeout(sidebarEdgeDwellRef.current);
                sidebarEdgeDwellRef.current = setTimeout(
                  () => setSidebarPeek(true),
                  200,
                );
              }}
          ...
          <aside
            style={{
              width: sidebarHidden
                ? 0
                : sidebarCollapsed
                  ? "var(--io-sidebar-collapsed, 48px)"
                  : "var(--io-sidebar-width, 240px)",
              position: "relative",
          ...
          >
            ...
            <nav
              ...
              style={{
                flex: 1,
                overflowY: "auto",
                padding: sidebarShowFull ? "8px 6px" : "8px 4px",
              }}
            >
                      ...
                      <NavLink
                        ...
                        style={({ isActive }) => ({
                          ...
                          background: isActive
                            ...
                            : "transparent",
                          transition:
                            "background var(--io-duration-fast), color var(--io-duration-fast)",
                          justifyContent: sidebarShowFull
                            ? "flex-start"
                            : "center",
                          borderLeft:
                            isActive && sidebarShowFull
                        ...
                        >
                          ...
                          {!sidebarShowFull &&
                            (sidebarBadges[item.path] ?? 0) > 0 && (
                              ...
                              >
                                {(sidebarBadges[item.path] ?? 0) > 99
                                  ? "99+"
                                  : sidebarBadges[item.path]}
                              </span>
                            )}
                        </span>
                        {sidebarShowFull && (
      ...
      >
        ...
        {topbarHidden && !isKiosk && (
          <div
            ...
            onMouseEnter={() => {
              if (topbarPeekTimerRef.current)
                clearTimeout(topbarPeekTimerRef.current);
              topbarPeekTimerRef.current = setTimeout(
                () => setTopbarPeek(true),
                200,
              );
            }}
      ...
      {isKiosk &&
        (["tl", "tr", "bl", "br"] as CornerPosition[]).map((corner) => (
          <CornerTrigger
            key={corner}
            corner={corner}
      ...
      {gKeyHintVisible && (
        <div
          className="gkey-hint-overlay"
          role="tooltip"
          aria-label="Go to — keyboard navigation shortcuts"
          style={{
            position: "fixed",
            bottom: "80px",
            left: isKiosk
        ...
        >
          ...
          {Object.entries(G_KEY_MAP).map(([key, path]) => {
            const label =
              ROUTE_REGISTRY.find((r) => r.path === path)?.sidebar_label ??
              path;
            return (
              <div
                key={key}
