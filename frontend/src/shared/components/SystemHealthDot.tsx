...
async function fetchOpcSources(): Promise<OpcSourceStat[]> {
  try {
    ...
    if (!r.ok) return []
    const body = await r.json() as { data?: OpcSourceStat[] } | OpcSourceStat[]
    if (Array.isArray(body)) return body
    if (Array.isArray((body as { data?: OpcSourceStat[] }).data)) return (body as { data: OpcSourceStat[] }).data
    return []
  } catch {
    return []
  }
...
export function SystemHealthDot() {
  ...
  const [wsState, setWsState] = useState<WsConnectionState>(wsManager.getState())
  ...
  if (services) {
    if (services.some((s) => s.status === 'unhealthy'))    serviceAggregate = 'unhealthy'
    else if (services.some((s) => s.status === 'degraded')) serviceAggregate = 'degraded'
    else if (services.every((s) => s.status === 'healthy')) serviceAggregate = 'healthy'
  ...
  const wsDisconnectedTooLong = disconnectedAt !== null && (Date.now() - disconnectedAt) > 30_000
  ...
  const wsLabelColor =
    ...
    wsState === 'connecting'   ? '#fbbf24' :
                                 '#ef4444'

  const opcLabel = totalOpc === 0
    ? 'No sources configured'
    : `${activeOpc}/${totalOpc} sources active`

  const opcLabelColor = allOpcOffline ? '#ef4444' : activeOpc < totalOpc ? '#fbbf24' : '#22c55e'

  const serverLabel = serviceAggregate === 'healthy' ? 'Reachable' : serviceAggregate === 'unknown' ? 'Unknown' : serviceAggregate === 'degraded' ? 'Degraded' : 'Unreachable'
  const serverColor = serviceAggregate === 'healthy' ? '#22c55e' : serviceAggregate === 'degraded' ? '#fbbf24' : serviceAggregate === 'unhealthy' ? '#ef4444' : 'var(--io-text-muted)'

  const dotTitle =
    dotColor === 'green'  ? 'All services healthy' :
    dotColor === 'yellow' ? 'Some services degraded' :
    dotColor === 'red'    ? 'Critical: service offline or WS disconnected' :
      ...
      {open && (
        <div
          ...
          style={{
            ...
            minWidth: '220px',
            background: 'var(--io-surface-elevated, var(--io-surface-secondary))',
        ...
        >
          <div style={{ fontWeight: 700, fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--io-text-muted)', marginBottom: '8px' }}>
            System Status
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
            ...
            <span style={{ color: wsLabelColor, fontWeight: 600 }}>{wsLabel}</span>
          </div>

          {/* OPC Data */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
            <span style={{ color: 'var(--io-text-secondary)' }}>OPC Data</span>
            <span style={{ color: opcLabelColor, fontWeight: 600 }}>{opcLabel}</span>
          </div>

          {/* Server reachability */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <span style={{ color: 'var(--io-text-secondary)' }}>Server</span>
            <span style={{ color: serverColor, fontWeight: 600 }}>{serverLabel}</span>
          </div>

          {/* Last update */}
          <div style={{ color: 'var(--io-text-muted)', fontSize: '11px', marginBottom: canMonitor ? '10px' : '0' }}>
