...
import type { ChartConfig, ChartTypeId, ChartPointSlot, PerSeriesScale } from './chart-config-types'
...
function NumericInput({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <label style={{ fontSize: 11, color: 'var(--io-text-muted)', fontWeight: 500 }}>{label}</label>
      <input
        type="number"
        value={value ?? ''}
...
export default function ChartScalingTab({ chartType, config, points, pointMeta, onChange }: ChartScalingTabProps) {
  ...
  const visibleCards = supportsMultiScale ? MODE_CARDS : MODE_CARDS.filter((c) => c.id !== 'multiscale')
  ...
  const seriesSlots = points.filter((p) => p.role === 'series' || p.role === 'y' || p.role === 'point')
  ...
  const cardStyle = (active: boolean): React.CSSProperties => ({
    ...
    borderRadius: 6,
    cursor: 'pointer',
    background: active ? 'color-mix(in srgb, var(--io-accent) 10%, transparent)' : 'var(--io-surface)',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 640 }}>
        ...
        {visibleCards.map((card) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              >
                {activeMode === card.id && (
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--io-accent)' }} />
                )}
              </div>
              <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--io-text-primary)' }}>{card.label}</span>
            </div>
            <div style={{ fontSize: 12, color: 'var(--io-text-muted)', paddingLeft: 22 }}>{card.desc}</div>
          </div>
        ))}
      </div>
      ...
      {activeMode === 'fixed' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--io-text-primary)' }}>Y Axis Range</div>
          ...
          {isXY && (
            <>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--io-text-primary)' }}>X Axis Range</div>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <NumericInput
                  label="X Min"
                  value={scaling.xMin}
      ...
      {activeMode === 'multiscale' && supportsMultiScale && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {seriesSlots.length === 0 && (
            <div style={{ fontSize: 13, color: 'var(--io-text-muted)', padding: '12px 0' }}>
          ...
          {seriesSlots.length > 0 && (
            <>
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <select
                        value={ss.mode}
                        onChange={(e) => patchSeriesScale(slot.slotId, { mode: e.target.value as PerSeriesScale['mode'] })}
                        style={{
                          ...inputStyle,
                          width: '100%',
                          cursor: 'pointer',
                      ...
                      >
                        {Object.entries(SERIES_MODE_LABELS).map(([val, label]) => (
                          <option key={val} value={val} disabled={val === 'range' && !hasRange}>
                            {label}{val === 'range' && !hasRange ? ' (no range)' : ''}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      {ss.mode === 'custom' && (
                        <input
                          ...
                          onChange={(e) => {
                            const raw = e.target.value
                            patchSeriesScale(slot.slotId, { min: raw === '' ? undefined : Number(raw) })
                          }}
                          style={inputStyle}
                        />
                      )}
                    ...
                    <div>
                      {ss.mode === 'custom' && (
                        <input
                          ...
                          onChange={(e) => {
                            const raw = e.target.value
                            patchSeriesScale(slot.slotId, { max: raw === '' ? undefined : Number(raw) })
                          }}
                          style={inputStyle}
                        />
                      )}
                      {ss.mode === 'range' && (
                        <span style={{ fontSize: 11, color: 'var(--io-text-muted)' }}>
                          {meta?.eu_range_high != null ? meta.eu_range_high : '—'}
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <div style={{ fontSize: 12, color: 'var(--io-text-muted)', marginTop: 4 }}>
                <b>Scale to Range</b> uses the engineering unit range configured in the OPC server for each point.
              </div>
            </>
          )}
        </div>
