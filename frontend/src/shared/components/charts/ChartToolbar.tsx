...
const NO_SECONDS_UNITS: TimeUnit[] = [
  "minutes",
  "hours",
  "days",
  "weeks",
...
export function fromMinutes(totalMinutes: number): {
  ...
  if (m >= 1) return { value: Math.round(m), unit: "minutes" };
...
interface ChartToolbarProps {
  ...
  /** Whether the chart grid lines are visible */
  showGrid?: boolean;
  onToggleGrid?: () => void;
}
...
export default function ChartToolbar({
  ...
  function clampedFromMinutes(mins: number): { value: number; unit: TimeUnit } {
    ...
    if (!allowSeconds && parsed.unit === "seconds")
  ...
  const durLabel =
    tier === "wide" ? "Duration" : tier === "medium" ? "Dur" : "Dur";
  const bucketLabelText =
    tier === "wide" ? "Bucket Size" : tier === "medium" ? "Bucket" : "Bkt";
  const aggLabelText =
    tier === "wide" ? "Aggregate" : tier === "medium" ? "Agg" : "Agg";
          ...
          onAggregateChange && (
            <div style={groupStyle}>
              ...
              <select
                ...
                onChange={(e) =>
                  onAggregateChange(e.target.value as AggregateType)
                }
                style={selectStyle}
              >
                {aggregates.map((a) => (
        ...
        {onToggleGrid && (
          <button
            ...
            style={{
              ...
              padding: "3px 4px",
              cursor: "pointer",
              color: showGrid ? "var(--io-success)" : "var(--io-text-muted)",
              display: "flex",
              alignItems: "center",
              borderRadius: 3,
              lineHeight: 0,
