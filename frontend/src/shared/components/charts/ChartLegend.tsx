...
function LegendStrip({ items, side, highlighted, onHighlight }: StripProps) {
  ...
  const borderProp = { top: 'borderBottom', bottom: 'borderTop', left: 'borderRight', right: 'borderLeft' } as const
    ...
    >
      {items.map((item) => {
        ...
        return (
          <div
            key={item.label}
            onClick={onHighlight ? (e) => { e.stopPropagation(); onHighlight(item.label, e.ctrlKey || e.metaKey) } : undefined}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: 5,
...
function FloatingLegend({ items, highlighted, onHighlight }: FloatingProps) {
  ...
  function handlePointerDown(e: React.PointerEvent) {
    // Let clicks on items bubble without starting a drag
    ...
    function onMove(me: PointerEvent) {
      ...
      const nx = Math.max(0, Math.min(maxX, dragOrigin.current.px + me.clientX - dragOrigin.current.mx))
      const ny = Math.max(0, Math.min(maxY, dragOrigin.current.py + me.clientY - dragOrigin.current.my))
...
export function ChartLegendLayout({ legend, items, highlighted, onHighlight, children }: ChartLegendLayoutProps) {
  ...
  if (mode === 'floating') {
    return (
      <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
        {children}
        <FloatingLegend items={items} highlighted={highlighted} onHighlight={onHighlight} />
      </div>
    )
  }

  const pos = (['top', 'bottom', 'left', 'right'].includes(rawPos) ? rawPos : 'top') as 'top' | 'bottom' | 'left' | 'right'
  const isRow = pos === 'left' || pos === 'right'

  return (
    <div style={{ display: 'flex', flexDirection: isRow ? 'row' : 'column', flex: 1, minHeight: 0 }}>
      {(pos === 'top' || pos === 'left') && (
        <LegendStrip items={items} side={pos} highlighted={highlighted} onHighlight={onHighlight} />
      )}
