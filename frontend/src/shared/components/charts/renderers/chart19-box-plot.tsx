...
function computeBoxStats(values: number[]) {
  ...
  const upper =
    [...sorted].reverse().find((v) => v <= q3 + 1.5 * iqr) ?? sorted[0];
  ...
  const { names, boxData, outlierData, allPointsData, meanData } = useMemo<{
    ...
    seriesSlots.forEach((slot, i) => {
      ...
      if (stats) {
        ...
        if (showMean) {
          meanData.push(
            horizontal ? { xAxis: stats.mean } : { yAxis: stats.mean },
          );
        }
      } else {
      ...
      series: [
        {
          ...
          ...(showMean && meanData.length > 0
            ? {
                markPoint: {
                  data: meanData.map((m, i) => ({
                    name: `Mean ${i}`,
                    coord: (horizontal
                      ? [m.xAxis ?? 0, i]
                      : [i, m.yAxis ?? 0]) as [number, number],
                  })),
                },
              }
            : {}),
