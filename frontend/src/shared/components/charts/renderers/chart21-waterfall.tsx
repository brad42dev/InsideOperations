...
export default function WaterfallChart({ config }: RendererProps) {
  ...
  const increaseColor = typeof config.extras?.increaseColor === 'string'
    ? config.extras.increaseColor
    : typeof config.extras?.positiveColor === 'string'
      ? config.extras.positiveColor
      : '#10B981'
  const decreaseColor = typeof config.extras?.decreaseColor === 'string'
  ...
  const { names, baseData, valueData, totalValue } = useMemo(() => {
    ...
    const valueData: { value: number; itemStyle: { color: string; opacity?: number } }[] = []
    ...
    rawValues.forEach((v, i) => {
      ...
      valueData.push({
        ...
        itemStyle: {
          ...
          ...(highlighted.size > 0 ? { opacity: highlighted.has(itemName) ? 1 : 0.15 } : {}),
        },
      })
      running += v
    })
    ...
    valueData.push({
      ...
      itemStyle: {
        color: totalColor,
        ...(highlighted.size > 0 ? { opacity: highlighted.has('Total') ? 1 : 0.15 } : {}),
      },
    })

    return { names: allNames, baseData, valueData, totalValue: running }
  }, [seriesSlots, values, latestData, increaseColor, decreaseColor, highlighted])
  ...
  const option: EChartsOption = useMemo(() => {
    ...
    const tooltipFormatter = (params: unknown) => {
      const arr = params as { name: string; value: number; seriesIndex: number }[]
