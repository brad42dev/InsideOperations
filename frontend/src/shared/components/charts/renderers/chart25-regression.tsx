...
type RegressionModel = 'linear' | 'polynomial' | 'exponential' | 'logarithmic' | 'power'

function computeR2(x: number[], y: number[], predict: (xi: number) => number): number {
...
function linearRegression(
  x: number[],
  y: number[],
): { slope: number; intercept: number; r2: number; predict: (xi: number) => number } {
  const n = x.length
...
function exponentialRegression(
  ...
  const validPairs = x.map((xi, i) => ({ xi, yi: y[i] })).filter((p) => p.yi > 0)
...
function logarithmicRegression(
  ...
  const validPairs = x.map((xi, i) => ({ xi, yi: y[i] })).filter((p) => p.xi > 0)
  if (validPairs.length < 2) {
    return { r2: 0, predict: () => 0, label: 'y = 0' }
  }
  ...
  const { slope, intercept, r2 } = linearRegression(lx, ly)
  const predict = (xi: number) => (xi > 0 ? intercept + slope * Math.log(xi) : intercept)
  return {
    r2,
    predict,
...
function powerRegression(
  ...
  const validPairs = x.map((xi, i) => ({ xi, yi: y[i] })).filter((p) => p.xi > 0 && p.yi > 0)
  if (validPairs.length < 2) {
    return { r2: 0, predict: () => 0, label: 'y = 0' }
  }
  const lx = validPairs.map((p) => Math.log(p.xi))
...
export default function Chart25Regression({ config }: RendererProps) {
  ...
  const degree = typeof config.extras?.degree === 'number' ? config.extras.degree : 2
  ...
  const { timestamps, series, equations } = useMemo(() => {
    ...
    if (sortedTs.length === 0) return { timestamps: [], series: [], equations: [] }
    ...
    seriesDataMap.forEach((ptMap, pid) => {
      ...
      eqs.push({ label: `${slotLabelStr}: ${label}  R²=${r2.toFixed(3)}`, r2, color })
    })
        ...
        {showEquation && equations.length > 0 && (
          ...
          >
            {equations.map((eq) => (
              <div
                key={eq.label}
                style={{ fontSize: 10, color: eq.color, fontFamily: 'monospace' }}
              >
                {eq.label}
              </div>
            ))}
