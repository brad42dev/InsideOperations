...
export default function Chart24Shewhart({ config }: RendererProps) {
  ...
  const { timestamps, series, xRange } = useMemo(() => {
    ...
    const vals = rows
      .map((r) => (r.value ?? r.avg) as number | null)
    ...
    const valIdx: number[] = [];
    rows.forEach((r, i) => {
      const v = r.value ?? r.avg;
      if (v !== null && Number.isFinite(v as number)) valIdx.push(i);
    });

    const dataArr: (number | null)[] = rows.map(
      (r) => (r.value ?? r.avg ?? null) as number | null,
    );
