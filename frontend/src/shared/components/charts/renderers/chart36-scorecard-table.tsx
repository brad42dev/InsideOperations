...
function periodLabel(startMs: number, bucketMs: number): string {
  ...
  if (bucketMs <= 8 * 60 * 60 * 1000) {
    const shift = d.getHours() < 8 ? 'Night' : d.getHours() < 16 ? 'Day' : 'Eve'
...
function cellColors(
  ...
  if ((hiAlarm !== null && value >= hiAlarm) || (loAlarm !== null && value <= loAlarm)) {
    return { bg: 'rgba(239,68,68,0.25)', fg: '#EF4444' }
  }
  if ((hiWarn !== null && value >= hiWarn) || (loWarn !== null && value <= loWarn)) {
...
export default function ScorecardTableChart({ config }: RendererProps) {
  ...
  const aggFn = ((config.extras?.aggregateFunction as string) ?? 'mean') as AggFn
  const refreshSeconds = typeof config.extras?.refreshSeconds === 'number'
  ...
  const thresholds: Record<string, ColThreshold> = useMemo(() => {
    const raw = config.extras?.thresholds as Record<string, Partial<ColThreshold>> | undefined
  ...
  const { rows, periodStarts } = useMemo(() => {
    ...
    const periodStarts = sortedBuckets.map((bi) => windowStartMs + bi * bucketMs)
  ...
  if (tableMode === 'event_stats') {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, color: 'var(--io-text-muted)', fontSize: 13 }}>
        <span>Event Statistics Table</span>
        <span style={{ fontSize: 11 }}>Requires event-condition query endpoint (Phase 3)</span>
      </div>
    )
  }
  ...
  return (
    <div style={{ position: 'relative', flex: 1, minHeight: 0, overflow: 'auto' }}>
      ...
      {!isLoading && rows.length === 0 && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--io-text-muted)', fontSize: 13, padding: 24 }}>
      ...
      {rows.length > 0 && (
        <table style={{
          ...
          <thead>
            <tr style={{ background: 'var(--io-surface-raised)', position: 'sticky', top: 0, zIndex: 2 }}>
              <th style={{
                textAlign: 'left',
                padding: '6px 8px',
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr key={ri} style={{ borderBottom: '1px solid var(--io-border)' }}>
                <td style={{
                  padding: '5px 8px',
                ...
                {row.map((cellVal, ci) => {
                  ...
                  const { bg, fg } = cellColors(cellVal, th.loWarn, th.hiWarn, th.loAlarm, th.hiAlarm)
                  return (
                    <td key={ci} style={{
                      padding: '5px 8px',
                      textAlign: 'right',
