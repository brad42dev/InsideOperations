...
export default function Chart10AnalogBar({ config }: RendererProps) {
  ...
  const rangeLo = typeof config.extras?.rangeLo === 'number' ? config.extras.rangeLo : 0
  const rangeHi = typeof config.extras?.rangeHi === 'number' ? config.extras.rangeHi : 100
  const desiredLo = typeof config.extras?.desiredLo === 'number' ? config.extras.desiredLo : null
  const desiredHi = typeof config.extras?.desiredHi === 'number' ? config.extras.desiredHi : null
  const isVertical = config.extras?.orientation === 'vertical'

  const liveValue = pointSlot ? (values.get(pointSlot.pointId)?.value ?? null) : null
  const setpointValue = setpointSlot ? (values.get(setpointSlot.pointId)?.value ?? null) : null
  ...
  if (liveValue !== null) {
    ...
    else if (desiredHi !== null && liveValue > desiredHi) pointerColor = '#F59E0B'
  }
  ...
  // Value label
  let valueLabelX = 0, valueLabelY = 0
  if (isVertical) {
    valueLabelX = svgW / 2
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Pointer triangle */}
        {pointerPath && (
          <path d={pointerPath} fill={pointerColor} />
        )}

        {/* Range labels */}
        {isVertical ? (
          <>
            <text x={svgW / 2} y={barY - 6} textAnchor="middle" fontSize={10} fill="var(--io-text-muted)">{rangeHi}</text>
            <text x={svgW / 2} y={barY + barLen + 14} textAnchor="middle" fontSize={10} fill="var(--io-text-muted)">{rangeLo}</text>
          </>
        ) : (
          <>
            <text x={barPad} y={barY + barThick + 14} textAnchor="middle" fontSize={10} fill="var(--io-text-muted)">{rangeLo}</text>
            <text x={barPad + barLen} y={barY + barThick + 14} textAnchor="middle" fontSize={10} fill="var(--io-text-muted)">{rangeHi}</text>
          </>
        )}
        ...
        >
          {liveValue !== null ? liveValue.toLocaleString(undefined, { maximumFractionDigits: 2 }) : '—'}
        </text>
      </svg>
    </div>
  )
