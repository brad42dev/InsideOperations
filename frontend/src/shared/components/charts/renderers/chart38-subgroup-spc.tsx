...
const SPC_CONSTANTS: Record<
  number,
  { A2: number; A3: number; D3: number; D4: number; B3: number; B4: number }
> = {
  2: { A2: 1.88, A3: 2.659, D3: 0, D4: 3.267, B3: 0, B4: 3.267 },
  ...
  6: { A2: 0.483, A3: 1.287, D3: 0, D4: 2.004, B3: 0.03, B4: 1.97 },
...
function buildSubgroupsTimeBased(
  ...
  for (let i = 0; i + n <= valid.length; i += n) {
    ...
    const r = Math.max(...vals) - Math.min(...vals);
    const s = Math.sqrt(
      vals.reduce((acc, v) => acc + (v - xbar) ** 2, 0) / (n - 1),
    );
    // Center time is midpoint of the subgroup
    const centerMs =
      (new Date(chunk[0].timestamp).getTime() +
        new Date(chunk[n - 1].timestamp).getTime()) /
      2;
...
function weViolation(
  ...
  if (i >= 8) {
    ...
    if (values.slice(i - 8, i + 1).every((x) => (x > mean ? 1 : -1) === side))
      return true;
  }
  if (i >= 5) {
...
export default function SubgroupSPCChart({ config }: RendererProps) {
  ...
  const subgroupSize = clampN(
    typeof config.extras?.subgroupSize === "number"
      ? config.extras.subgroupSize
      : 5,
  );
  ...
  const { xbarStats, dispStats, labels } = useMemo(() => {
    if (subgroups.length < 2)
      return { xbarStats: null, dispStats: null, labels: [] };
    ...
    const labels = subgroups.map((g) =>
      new Date(g.centerMs).toLocaleString([], {
        month: "short",
        day: "numeric",
        hour: "2-digit",
        minute: "2-digit",
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          const lines = ps
            .filter(
              (p) => p.value !== null && p.seriesName !== "X-bar Violations",
            )
            .map(
              (p) =>
                `${p.seriesName}: ${typeof p.value === "number" ? p.value.toFixed(3) : "—"}`,
            );
      ...
      series: [
        ...
        {
          ...constLine(xbarStats.mean, "#10B981"),
          name: "X̄ Mean",
          xAxisIndex: 0,
          yAxisIndex: 0,
        },
        {
          ...constLine(xbarStats.ucl, "#EF4444"),
          name: "UCL",
          xAxisIndex: 0,
          yAxisIndex: 0,
        },
        {
          ...constLine(xbarStats.lcl, "#EF4444"),
          name: "LCL",
          xAxisIndex: 0,
          yAxisIndex: 0,
        },
        ...
        {
          ...constLine(dispStats.mean, "#10B981"),
          name: `${subchartType === "S" ? "S" : "R"} Mean`,
          xAxisIndex: 1,
          yAxisIndex: 1,
        },
        {
          ...constLine(dispStats.ucl, "#EF4444"),
          name: `${subchartType === "S" ? "S" : "R"} UCL`,
          xAxisIndex: 1,
          yAxisIndex: 1,
        },
        ...(dispStats.lcl > 0
          ? [
              {
                ...constLine(dispStats.lcl, "#EF4444"),
                name: `${subchartType === "S" ? "S" : "R"} LCL`,
                xAxisIndex: 1,
                yAxisIndex: 1,
              },
            ]
  ...
  return (
    <div style={{ position: "relative", flex: 1, minHeight: 0, width: "100%" }}>
      ...
      {!insufficientData && subgroups.length >= 2 && <EChart option={option} />}
    </div>
  );
}
