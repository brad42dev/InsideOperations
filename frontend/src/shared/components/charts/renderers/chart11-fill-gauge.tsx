...
export default function Chart11FillGauge({ config }: RendererProps) {
  ...
  const warnHi = typeof config.extras?.warnHi === 'number' ? config.extras.warnHi : null
  const warnLo = typeof config.extras?.warnLo === 'number' ? config.extras.warnLo : null
  const critHi = typeof config.extras?.critHi === 'number' ? config.extras.critHi : null
  const critLo = typeof config.extras?.critLo === 'number' ? config.extras.critLo : null

  const span = rangeHi - rangeLo || 1
  const fillFrac = liveValue !== null ? clamp((liveValue - rangeLo) / span, 0, 1) : 0
