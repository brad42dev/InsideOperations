...
function alignSeries(
  ...
  for (const xRow of xRows) {
    ...
    if (!yRow) {
      ...
      let best: { timestamp: string; value: number | null } | undefined = undefined
...
function computeDensity(
  ...
  const grid = Array.from({ length: binCount }, () => new Array(binCount).fill(0) as number[])
  ...
  const xBins = Array.from({ length: binCount }, (_, i) => parseFloat((xMin + (i + 0.5) * xStep).toFixed(4)))
  const yBins = Array.from({ length: binCount }, (_, i) => parseFloat((yMin + (i + 0.5) * yStep).toFixed(4)))
...
export default function XYScatterChart({ config }: RendererProps) {
  ...
  const legendItems: LegendItem[] = [
    ...(xSlot ? [{ label: slotLabel(xSlot), color: xSlot.color ?? '#4A9EFF' }] : []),
    ...(ySlot ? [{ label: slotLabel(ySlot), color: ySlot.color ?? '#F59E0B' }] : []),
  ]
  const durationMinutes = config.durationMinutes ?? 60
  const timeColoring = config.extras?.colorByTime === true || config.extras?.timeColoring === true
  const densityMode = (config.extras?.densityMode as string) ?? 'auto'
  const densityBins = typeof config.extras?.densityBins === 'number' ? config.extras.densityBins : 50
  ...
  const { data: xData, isLoading: xLoading } = useQuery({
    ...
    queryFn: () => pointsApi.history(xSlot!.pointId, { start, end, limit: 5000 }),
  ...
  const { data: yData, isLoading: yLoading } = useQuery({
    ...
    queryFn: () => pointsApi.history(ySlot!.pointId, { start, end, limit: 5000 }),
    enabled: queryEnabled,
    select: (res) => res.success ? res.data.rows : [],
  })
  ...
  const useDensity = densityMode === 'on' || (densityMode === 'auto' && aligned.length >= 1000)
    ...
    if (useDensity && densityResult && densityResult.data.length > 0) {
      ...
      return {
        ...
        xAxis: {
          ...
          axisLabel: { color: textMuted, fontSize: 9, rotate: 30, interval: Math.ceil(densityBins / 8) - 1 },
        ...
        yAxis: {
          ...
          axisLabel: { color: textMuted, fontSize: 9, interval: Math.ceil(densityBins / 6) - 1 },
    ...
    const scatterData = aligned.map((p) => {
      ...
      return {
        ...
        itemStyle: timeColoring
          ...
          : { color: xSlot?.color ?? '#4A9EFF', opacity: typeof config.extras?.opacity === 'number' ? config.extras.opacity : 0.7 },
      }
    })
      ...
      series: [
        {
          type: 'scatter',
          data: scatterData,
          symbolSize: typeof config.extras?.symbolSize === 'number' ? config.extras.symbolSize : 5,
      ...
      ...(timeColoring
        ? {
            visualMap: {
              ...
              left: 'center',
              textStyle: { color: textPrimary, fontSize: 10 },
              formatter: (v: unknown) => new Date(v as number).toLocaleTimeString(),
            },
          }
        : {}),
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [aligned, config, xSlot, ySlot, timeColoring, useDensity, densityResult, densityBins])
  ...
  return (
    <ChartLegendLayout legend={config.legend} items={legendItems} highlighted={highlighted} onHighlight={toggle}>
      <div style={{ position: 'relative', flex: 1, minHeight: 0, width: '100%' }}>
        ...
        {aligned.length > 0 && <EChart option={displayOption} onEvents={onEvents} height={300} />}
      </div>
    </ChartLegendLayout>
  )
}
