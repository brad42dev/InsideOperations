...
function weViolationAttr(vals: number[], ucls: number[], lcls: number[], i: number): boolean {
...
export default function AttributeControlChart({ config }: RendererProps) {
  ...
  const fixedN = typeof config.extras?.fixedSampleSize === 'number' ? config.extras.fixedSampleSize : 100
  ...
  const { attrPoints, limits, labels } = useMemo(() => {
    if (!historyBatch || !defectSlot) return { attrPoints: [], limits: null, labels: [] }

    const defectBatch = historyBatch.find((b) => b.point_id === defectSlot.pointId)
    const sampleBatch = sampleSlot ? historyBatch.find((b) => b.point_id === sampleSlot.pointId) : undefined
  ...
  const option: EChartsOption = useMemo(() => {
    ...
    const violations = showWERules
      ? statVals.map((_, i) => weViolationAttr(statVals, limits.ucl, limits.lcl, i))
      : statVals.map(() => false)
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: unknown) => {
          const ps = params as { axisValueLabel: string; seriesName: string; value: number | null; dataIndex: number }[]
          if (!ps.length) return ''
