...
export default function ParallelCoordChart({ config }: RendererProps) {
  ...
  const legendItems: LegendItem[] = axisSlots.map((slot, i) => ({ label: slotLabel(slot), color: autoColor(i) }))
  ...
  const lineOpacity =
    typeof config.extras?.lineOpacity === 'number' ? config.extras.lineOpacity : 0.4
  ...
  const buckets = useMemo(() => {
    ...
    return bucketData(historyBatch, pointIds, windowStartMs, windowEndMs, bucketMs)
  ...
  const axisRanges = useMemo(() => {
    return axisSlots.map((_, i) => {
      const vals = buckets.map((b) => b.values[i]).filter((v): v is number => v !== null)
  ...
  }, [buckets, axisSlots, colorBy, lineOpacity, axisRanges, outlierBucketIndices])
