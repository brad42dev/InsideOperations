...
export default function RadarChart({ config }: RendererProps) {
  ...
  const seriesSlots = config.points.filter((p) => p.role === 'series').slice(0, MAX_AXES)
  ...
  const option: EChartsOption = useMemo(() => {
    ...
    const hasMatchingHighlight = anyHighlighted && slotLabels.some((l) => highlighted.has(l))
          ...
          data: [
            {
              ...
              symbol: 'circle',
              symbolSize: 4,
              lineStyle: { color: autoColor(0), width: 2, opacity: seriesOpacity },
