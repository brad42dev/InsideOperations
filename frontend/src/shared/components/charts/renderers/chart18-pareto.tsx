...
export default function ParetoChart({ config }: RendererProps) {
  ...
  const thresholdPercent =
    typeof config.extras?.thresholdPercent === "number"
      ...
      const items = seriesSlots.map((slot, i) => {
        ...
        const val =
          ws !== undefined
            ? ws.value
            : (latestData?.find(
                (r: { point_id: string; value: number | null }) =>
                  r.point_id === slot.pointId,
              )?.value ?? 0);
        return {
          name: slotLabel(slot),
  ...
  const option: EChartsOption = useMemo(() => {
    ...
    const isaPriorityMarkers =
      mode === "priority_distribution"
        ? sortedNames
            .map((name) => {
              const target = ISA_TARGETS[name];
              return target
                ? {
                    xAxis: horizontal ? target : undefined,
                    yAxis: !horizontal ? target : undefined,
                  }
                : null;
            })
            .filter(Boolean)
