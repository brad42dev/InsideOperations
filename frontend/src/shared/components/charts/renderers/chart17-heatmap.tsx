...
export default function HeatmapChart({ config }: RendererProps) {
  ...
  const calendarYear = typeof config.extras?.calendarYear === 'number'
  ...
  const queryEnd = calendarMode && !isHistorical
    ? new Date(`${calendarYear}-12-31T23:59:59Z`).toISOString()
    : end

  const { data: rows, isLoading } = useQuery({
    queryKey: ['history', valueSlot?.pointId, start, queryEnd, 'heatmap', calendarMode, calendarYear],
    queryFn: () => pointsApi.history(valueSlot!.pointId, { start, end: queryEnd, limit: 50000 }),
    enabled: !!valueSlot,
  ...
  const calendarData = useMemo(() => {
    ...
    return Array.from(dayMap.entries()).map(([date, { sum, count }]) => [
      date,
      parseFloat((sum / count).toFixed(4)),
    ] as [string, number])
  }, [rows, calendarMode])
    ...
    if (calendarMode) {
      ...
      return {
        ...
        calendar: {
          ...
          yearLabel: { show: true, textStyle: { color: textMuted, fontSize: 12 } },
          dayLabel: { firstDay: 0, nameMap: ['S', 'M', 'T', 'W', 'T', 'F', 'S'], textStyle: { color: textMuted, fontSize: 10 } },
  ...
  return (
    <div style={{ position: 'relative', flex: 1, minHeight: 0, width: '100%' }}>
      ...
      {displayData.length > 0 && <EChart option={option} height={calendarMode ? 200 : 300} />}
    </div>
  )
}
