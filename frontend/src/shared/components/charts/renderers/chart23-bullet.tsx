...
export default function BulletChart({ config }: RendererProps) {
  ...
  const ranges: RangeZone[] = Array.isArray(rawRanges) ? (rawRanges as RangeZone[]) : DEFAULT_RANGES
  const target = typeof config.extras?.target === 'number' ? config.extras.target : null
