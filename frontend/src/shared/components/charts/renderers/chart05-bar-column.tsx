...
export default function BarColumnChart({ config }: RendererProps) {
  ...
  const comboLineLabel = (config.extras?.comboLineLabel as string) ?? 'Target'
    ...
    const barSeries: EChartsOption['series'] = stacked
      ? seriesSlots.map((slot, i) => ({
          ...
          itemStyle: {
            ...
            ...(highlighted.size > 0 ? { opacity: highlighted.has(slotLabel(slot)) ? 1 : 0.15 } : {}),
          },
          label: { show: showValues, position: 'inside' as const, fontSize: 10, color: '#fff' },
        }))
      : [
          {
            type: 'bar' as const,
            ...
            label: showValues
              ? { show: true, position: 'top' as const, fontSize: 10, color: textMuted }
              : { show: false },
          },
        ]

    // Error bar series using ECharts custom series
    const errorBarSeries: EChartsOption['series'] = errorBars !== 'none' && errorData.length > 0
      ? [{
          ...
          renderItem: (_params: unknown, api: any) => {
            ...
            const barCoord = api.coord([idx, (low + high) / 2])
            const lowCoord = api.coord([idx, low])
            const highCoord = api.coord([idx, high])
            if (horizontal) {
              return {
                type: 'line', shape: { x1: lowCoord[0], y1: barCoord[1], x2: highCoord[0], y2: barCoord[1] },
                style: { stroke: textMuted, lineWidth: 2 },
              }
            }
            return {
              type: 'line', shape: { x1: barCoord[0], y1: lowCoord[1], x2: barCoord[0], y2: highCoord[1] },
              style: { stroke: textMuted, lineWidth: 2 },
            }
          },
  ...
  }, [values, latestData, config, seriesSlots, dataValues, horizontal, stacked, stackMode, errorBars, comboLine, comboLineLabel, showValues, highlighted])
