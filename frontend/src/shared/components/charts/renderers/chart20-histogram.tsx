...
function ViolinPlot({
  ...
  const stats = useMemo(() => {
    ...
    const upper =
      [...sorted].reverse().find((v) => v <= q3 + 1.5 * iqr) ??
  ...
  const violinData = kde.map((p) => {
    const normDensity =
      maxDensity > 0 ? (p.density / maxDensity) * violinWidth : 0;
...
export default function HistogramChart({ config }: RendererProps) {
  ...
  const legendItems: LegendItem[] = pointSlot
    ? [{ label: slotLabel(pointSlot), color: pointSlot.color ?? autoColor(0) }]
    : [];
  ...
  const normalOverlay =
    config.extras?.normalOverlay === true || config.extras?.showNormal === true;
  const extraBinCount =
    typeof config.extras?.bins === "number"
      ? config.extras.bins
      : typeof config.extras?.binCount === "number"
        ? config.extras.binCount
        : null;
  ...
  const { data: rows, isLoading } = useQuery({
    ...
    queryFn: () =>
      pointsApi.history(pointSlot!.pointId, { start, end, limit: 10000 }),
    enabled: !!pointSlot,
  ...
  const { labels, counts, normalCurve, cpk } = useMemo(() => {
    if (vals.length < 2)
      return { labels: [], counts: [], normalCurve: [], cpk: null };
    ...
    const normalCurveData: number[] =
      normalOverlay && stddev > 0
        ? bins.map((b) => {
    ...
    if (showCapability && stddev > 0) {
      const usl =
        typeof config.extras?.usl === "number" ? config.extras.usl : null;
      const lsl =
        typeof config.extras?.lsl === "number" ? config.extras.lsl : null;
        ...
        {labels.length > 0 && (
          <div style={{ position: "relative", height: "100%" }}>
            ...
            {cpk && (
              ...
              >
                Cp: {cpk.cp.toFixed(3)} | Cpk: {cpk.cpk.toFixed(3)} | σ:{" "}
                {cpk.sigma.toFixed(4)}
              </div>
            )}
          </div>
