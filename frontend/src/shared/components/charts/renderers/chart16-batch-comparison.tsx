...
export default function Chart16BatchComparison({ config }: RendererProps) {
  ...
  const primarySlot =
    config.points.find((p) => p.role === "series") ?? config.points[0] ?? null;
  ...
  const { timestamps, series, xRange } = useMemo(() => {
    ...
    batchSeries.forEach(({ points }) =>
      points.forEach((_, k) => allOffsets.add(k)),
    );
    ...
    >
      ...
      >
        ...
        >
          ...
          {isFetching && (
            <span style={{ marginLeft: 8, color: "var(--io-text-muted)" }}>
              Loading…
            </span>
          )}
        </div>
