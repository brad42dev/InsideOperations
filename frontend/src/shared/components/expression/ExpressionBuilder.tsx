...
function getTileColor(type: TileType): string {
  ...
  if (type === "point_ref" || type === "field_ref" || type === "constant")
    return "#2563eb";
...
function getNestingStyle(depth: number, theme: Theme): NestingStyle {
  ...
  const bgAlpha = isHighContrast ? 0.15 : isDark ? 0.1 : 0.08;
...
function exprReducer(
  ...
  switch (action.type) {
    ...
    case "SET_OUTPUT_PRECISION":
      return { ...state, outputPrecision: action.value };
    case "SET_SAVE_FUTURE":
      return { ...state, saveForFuture: action.value };
    case "SET_SHARE":
      return { ...state, shareExpression: action.value };
    ...
    case "MOVE_TILE": {
      ...
      const toIndex =
        action.toIndex > srcArr.length ? dstArr.length : action.toIndex;
    ...
    case "UNDO": {
      ...
      return {
        ...
        tiles: prev,
        past: rest,
        future: [cloneTiles(state.tiles), ...state.future].slice(
          0,
          MAX_HISTORY,
        ),
      };
...
function WorkspaceTile({
  ...
  const bg =
    isContainer || isControlFlow ? "transparent" : getTileColor(tile.type);

  const containerBorder =
    isContainer || isControlFlow
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    if (e.key === "Delete" || e.key === "Backspace") {
      e.preventDefault();
      const idsToDelete = allSelectedIds.includes(tile.id)
        ? allSelectedIds
        : [tile.id];
...
function getTileLabel(tile: ExpressionTile): string {
  switch (tile.type) {
    ...
    case "agg_count":
      return "count()";
    case "point_ref":
      return tile.pointLabel ?? tile.pointId ?? "current_point";
    case "field_ref":
      return tile.fieldName ? tile.fieldName : "field?";
    case "constant":
      return String(tile.value ?? 0);
    default:
      return tile.type;
  }
}
...
function getTileAriaLabel(tile: ExpressionTile, depth: number): string {
  ...
  if (isContainer || isControlFlow) {
    switch (tile.type) {
      case "group":
        return `Group, level ${depth + 1}`;
      case "square":
        return `Square container, level ${depth + 1}`;
      case "cube":
        return `Cube container, level ${depth + 1}`;
      case "round":
        return `Round container, increment ${tile.precision ?? 1}, level ${depth + 1}`;
      case "negate":
        return `Negate container, level ${depth + 1}`;
      case "abs":
        return `Absolute value container, level ${depth + 1}`;
      case "if_then_else":
        return `If-then-else container, level ${depth + 1}`;
      default:
        return `Container, level ${depth + 1}`;
    }
  }
  switch (tile.type) {
    case "add":
    ...
    case "elapsed_since":
      return "Elapsed since function";
    case "duration_above":
      return "Duration above function";
    case "duration_below":
      return "Duration below function";
    case "agg_avg":
      return "Average aggregate function";
    case "agg_sum":
      return "Sum aggregate function";
    case "agg_min":
      return "Minimum aggregate function";
    case "agg_max":
      return "Maximum aggregate function";
    case "agg_count":
      return "Count aggregate function";
    case "point_ref":
      return `Point Reference: ${tile.pointLabel ?? tile.pointId ?? "current point"}`;
    case "field_ref":
      return `Field Reference: ${tile.fieldName ?? "field"}`;
    case "constant":
          ...
          <select
            ...
            onChange={(e) =>
              dispatch({
                type: "UPDATE_TILE",
                id: tile.id,
                patch: { precision: parseFloat(e.target.value) },
              })
            }
            onClick={(e) => e.stopPropagation()}
            style={{
  ...
  function walk(
    ...
    for (const tile of current) {
      if (tile.id === targetParentId) {
        ...
        return [...path, { parentId: tile.id, label: getTileLabel(tile) }];
      }
...
function DropZoneRow({
  ...
  const droppableId =
    parentId !== null ? `container-zone-${parentId}` : "root-zone";
      ...
      <div
        ...
        style={{
          ...
          animation: isDragging
            ? "none"
            : "io-cursor-blink 1.06s step-end infinite",
          flexShrink: 0,
          alignSelf: "center",
          borderRadius: "1px",
        }}
  ...
  return (
    ...
    >
      <div
        ...
        title={
          depthBlocked ? "Maximum nesting depth (5 levels) reached." : undefined
        }
      >
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: "var(--io-radius)",
        border: isContainer
          ? `${depth0Style.borderStyle} ${depth0Style.color}`
          : "none",
        background: isContainer
          ? `rgba(${hexToRgb(depth0Style.color)}, ${depth0Style.bgAlpha})`
          : bg,
        color: "#fff",
        fontSize: "12px",
        fontWeight: 500,
...
function validateExpression(state: ExprBuilderState): ValidationResult {
  const errors: string[] = [];
  ...
  function checkTiles(tiles: ExpressionTile[]) {
    for (const tile of tiles) {
      if (
        tile.type === "constant" &&
        (tile.value === undefined || isNaN(tile.value))
      ) {
        errors.push("All constant tiles must have a numeric value.");
      }
      if (
        tile.type === "point_ref" &&
        tile.pointId === undefined &&
        tile.pointLabel === undefined
      ) {
        // null pointId is valid (means current_point), only check if explicitly undefined
      }
      const containers = ["group", "square", "cube", "round", "negate", "abs"];
      if (
        containers.includes(tile.type) &&
        (!tile.children || tile.children.length === 0)
      ) {
        errors.push(
          `"${getTileLabel(tile)}" container must have at least one child tile.`,
        );
      }
...
export function ExpressionBuilder({
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (isPaletteSource) {
      ...
      const overIsContainer =
        overTile != null && CONTAINER_TILE_TYPES.includes(overTile.type);
      if (overIsContainer && overTile != null) {
        const children =
          overTile.children ?? overTile.condition ?? overTile.thenBranch ?? [];
        dispatch({
          type: "INSERT_TILE",
          tile,
        ...
        const insertIndex =
          toLoc?.index ?? (parentId === null ? state.tiles.length : 0);
        dispatch({
          type: "INSERT_TILE",
          tile,
          parentId,
    ...
    if (
      activeIds.includes(String(active.id)) &&
      activeIds.includes(String(over.id))
    ) {
  ...
  function runBenchmark() {
    ...
    function scanTiles(tiles: ExpressionTile[]) {
      for (let i = 0; i < tiles.length; i++) {
        ...
        if (tile.type === "power" && i > 0 && tiles[i - 1]?.type === "power") {
          warnings.push(
            "Nested exponentiation detected — this may produce unexpectedly large values.",
          );
        }
        ...
        if (
          tile.type === "divide" &&
          i + 1 < tiles.length &&
          tiles[i + 1]?.type === "constant" &&
          tiles[i + 1].value === 0
        ) {
          warnings.push(
            "Potential divide-by-zero: division followed by a constant 0.",
          );
        }
    ...
    if (countTiles(state.tiles) > 100) {
      warnings.push(
        "Expression has more than 100 tiles — evaluation may be slow.",
      );
    }
    ...
    const worker = new Worker(
      new URL(
        "../../../workers/expressionBenchmark.worker.ts",
        import.meta.url,
      ),
      { type: "module" },
    );
    ...
    worker.onmessage = (
      e: MessageEvent<{ avgMs: number; result: number | null }>,
    ) => {
  ...
  function handleOkClick() {
    ...
    if (state.saveForFuture && !state.name.trim()) {
      setNameError(
        'Expression name is required when "Save for Future Use" is checked.',
      );
      return;
    }
    setNameError(null);
  ...
  const handleConfirmOk = useCallback(async () => {
    ...
    const benchmarkResult = await new Promise<
      { avgMs: number; result: number | null } | "timeout" | "error"
    >((resolve) => {
      const worker = new Worker(
        new URL(
          "../../../workers/expressionBenchmark.worker.ts",
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "flex-end" }}>
          ...
          <div style={{ flex: 2 }}>
            ...
            <input
              ...
              onChange={(e) =>
                dispatch({ type: "SET_DESC", value: e.target.value })
              }
            />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
          ...
          >
            <input
              ...
              onChange={(e) =>
                dispatch({ type: "SET_SAVE_FUTURE", value: e.target.checked })
              }
          ...
          {isAdmin && (
            ...
            >
              <input
                ...
                onChange={(e) =>
                  dispatch({ type: "SET_SHARE", value: e.target.checked })
                }
                style={{ accentColor: "var(--io-accent)" }}
              />
            ...
            {(["float", "integer"] as const).map((opt) => (
              ...
              >
                <input
                  ...
                  onChange={() =>
                    dispatch({ type: "SET_OUTPUT_TYPE", value: opt })
                  }
                  style={{ accentColor: "var(--io-accent)" }}
                />
          ...
          {activeDragId &&
            (() => {
              ...
              if (activeDragId.startsWith("palette-")) {
                const tileType = activeDragId.slice(
                  "palette-".length,
                ) as TileType;
      ...
      <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
        <button
          style={{
            ...
            padding: "5px 12px",
            fontSize: "12px",
            opacity: state.past.length > 0 ? 1 : 0.4,
          }}
        ...
        <button
          style={{
            ...
            padding: "5px 12px",
            fontSize: "12px",
            opacity: state.future.length > 0 ? 1 : 0.4,
          }}
        ...
        >
            ...
            {pointRefs.map((ref) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTestValues((v) => ({ ...v, [ref.id]: e.target.value }))
                  }
          ...
          >
            <button
              ...
              title={
                !validation.valid
                  ? "Fix validation errors before running benchmark"
                  : "Run 10,000-iteration benchmark in background thread"
              }
            >
          ...
          {benchmarkResult === "timeout" && (
            ...
            >
              Benchmark timed out (exceeded 5 seconds). Expression may be too
              expensive to evaluate in real-time.
            </div>
          )}
              ...
              <span style={{ color: "var(--io-text-secondary)" }}>
                ...
                <span style={{ color: "var(--io-text-primary)" }}>
                  {benchmarkResult === null
                    ? "Error (division by zero or incomplete expression)"
      ...
      >
        ...
        {showPreview && (
          ...
          >
            {previewStr
              ? `= ${previewStr}`
              : "No tiles yet — add tiles from the palette above."}
          </div>
        )}
      </div>
          ...
          {validation.errors.map((err, i) => (
            <div
              ...
              style={{
                fontSize: "13px",
                color: "var(--io-danger)",
                marginBottom: i < validation.errors.length - 1 ? "4px" : 0,
              }}
            >
              • {err}
            </div>
                  ...
                  onClick={() => {
                    ...
                    void (async () => {
                      ...
                      if (result.success) {
                        showToast({
                          title: "Saved",
                          description: "Expression saved for later use.",
                          variant: "success",
                          duration: 3000,
                        });
                      } else {
      ...
      <Dialog.Root open={showSlowWarning} onOpenChange={setShowSlowWarning}>
        <Dialog.Portal>
          ...
          >
            ...
            >
              This expression averages{" "}
              <strong>{slowWarningAvgMs.toFixed(3)} ms</strong> per evaluation,
              which exceeds the 1 ms threshold for real-time use. It may impact
              system performance. Consider simplifying the expression before
              applying.
            </Dialog.Description>
            <div
              style={{
      ...
      <Dialog.Root open={showCutConfirm} onOpenChange={setShowCutConfirm}>
        <Dialog.Portal>
          ...
          >
            ...
            >
              Cut {state.selectedIds.length} tile
            ...
            >
              The selected tile{state.selectedIds.length !== 1 ? "s" : ""} will
              be copied to the clipboard and removed from the workspace.
            </Dialog.Description>
            <div
              style={{
                display: "flex",
