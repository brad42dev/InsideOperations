  ...
  switch (tile.type) {
    ...
    case "field_ref" as any: {
      return {
        type: "field_ref",
        field_name:
          (tile as ExpressionTile & { fieldName?: string }).pointLabel ?? "",
    ...
    case "if_then_else": {
      ...
      const thenNode = tile.thenBranch
        ? parseInfixTiles(tile.thenBranch)
        : null;
      const elseNode = tile.elseBranch
        ? parseInfixTiles(tile.elseBranch)
        : null;
 ...
 */
function childrenToNode(
  children: ExpressionTile[] | undefined,
): ExprNode | null {
