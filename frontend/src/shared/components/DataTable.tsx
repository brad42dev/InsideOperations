...
const textFilterFn: FilterFn<unknown> = (
  row,
  columnId,
  filterValue: string,
) => {
...
numericRangeFilterFn.autoRemove = (v: [number | "", number | ""]) =>
  v[0] === "" && v[1] === "";

const enumFilterFn: FilterFn<unknown> = (
  row,
  columnId,
  filterValue: string,
) => {
...
export default function DataTable<T extends object>({
  ...
  const tanstackColumns: TanstackColumnDef<T>[] = columns.map((col) => {
    ...
    if (col.filterType === "text") filterFn = textFilterFn as FilterFn<T>;
      ...
      filterFn = numericRangeFilterFn as FilterFn<T>;
  ...
  const startIndex = Math.max(
    0,
    Math.floor(scrollTop / resolvedRowHeight) - VIRTUAL_BUFFER,
  );
  ...
  function getPinnedStyle(
    ...
    if (isPinned === "left") {
      ...
      const offset = leftCols
        .slice(0, idx)
        .reduce((sum, c) => sum + (colSizeMap[c.id] ?? 150), 0);
      return {
        position: "sticky",
        left: offset,
        zIndex: 2,
  ...
  return (
    ...
    >
      ...
      {showExport && (
        ...
        >
          <span
            style={{ flex: 1, fontSize: "12px", color: "var(--io-text-muted)" }}
          >
            ...
            {hg.headers.map((header) => {
              ...
              return (
                ...
                >
                  ...
                  {isSortable && (
                    <span
                      style={{ opacity: sortDir ? 1 : 0.3, fontSize: "10px" }}
                    >
                  ...
                  {col?.filterType === "text" && (
                    <input
                      ...
                      style={{
                        ...
                        borderRadius: 3,
                        background:
                          "var(--io-surface-primary, var(--io-surface))",
                        color: "var(--io-text-primary)",
                        boxSizing: "border-box",
                      }}
                    />
                  ...
                  {col?.filterType === "numeric" && (
                    <div style={{ display: "flex", gap: 2 }}>
                      <input
                        ...
                        onChange={(e) => {
                          const cur = (column.getFilterValue() as [
                            number | "",
                            number | "",
                          ]) ?? ["", ""];
              ...
              {visibleRows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => {
                    ...
                    const condStyle: React.CSSProperties = col?.conditionStyle
                      ? col.conditionStyle(value, row.original)
                      : {};
