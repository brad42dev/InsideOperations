    ...
    if ("children" in node && Array.isArray(node.children)) {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        if (child.id === id) {
          return {
            node: child,
            parent: node as SceneNode & { children: SceneNode[] },
            index: i,
          };
        }
        const found = search(
...
function updateNode(
  doc: GraphicDocument,
  id: NodeId,
  updater: (node: SceneNode) => unknown,
): GraphicDocument {
  ...
  undo(doc: GraphicDocument): GraphicDocument {
    ...
    return updateNode(d, this.parentId, (parent) => {
      ...
      return {
        ...parent,
        children: (
          parent as SceneNode & { children: SceneNode[] }
...
export class DeleteNodesCommand implements SceneCommand {
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    ...
    for (const id of this.nodeIds) {
      ...
      const parentId =
        result.parent.id === d.id ? null : (result.parent.id ?? null);
      this.snapshots.push({
...
export class GroupNodesCommand implements SceneCommand {
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    ...
    for (const { index } of [...this.removedNodes].sort(
      (a, b) => b.index - a.index,
    )) {
  ...
  undo(doc: GraphicDocument): GraphicDocument {
    ...
    for (const { node, index } of [...this.removedNodes].sort(
      (a, b) => a.index - b.index,
    )) {
...
export class UngroupCommand implements SceneCommand {
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    ...
    this.savedGroup = clone(d.children[idx]) as SceneNode & {
      children: SceneNode[];
    };
    this.groupIndex = idx;

    const children =
      (this.savedGroup as SceneNode & { children: SceneNode[] }).children ?? [];
  ...
  undo(doc: GraphicDocument): GraphicDocument {
    ...
    const childIds = new Set(
      (this.savedGroup as SceneNode & { children: SceneNode[] }).children?.map(
        (c) => c.id,
      ) ?? [],
    );
...
export class RotateNodesCommand implements SceneCommand {
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    ...
    for (const id of this.nodeIds) {
      ...
      if (t)
        d = updateNode(d, id, (node) => ({ ...node, transform: clone(t) }));
    }
    return d;
  }
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    ...
    switch (this.alignment) {
      case "left":
        ...
        break;
      case "center-h":
        anchor = (Math.min(...xs) + Math.max(...xs)) / 2;
        break;
      case "right":
        anchor = Math.max(...xs);
        break;
      case "top":
        ...
        break;
      case "center-v":
        anchor = (Math.min(...ys) + Math.max(...ys)) / 2;
        break;
      case "bottom":
        anchor = Math.max(...ys);
        break;
    ...
    for (const { id, x, y } of positions) {
      const isH =
        this.alignment === "left" ||
        this.alignment === "center-h" ||
        this.alignment === "right";
      result = updateNode(result, id, (node) => ({
        ...node,
        transform: {
          ...node.transform,
          position: { x: isH ? anchor : x, y: isH ? y : anchor },
        },
      }));
    }
    return result;
    ...
    if (this.axis === "horizontal") {
      ...
      for (let i = 0; i < items.length; i++) {
        result = updateNode(result, items[i].id, (node) => ({
          ...
          transform: {
            ...node.transform,
            position: { x: min + i * step, y: items[i].y },
          },
        }));
      }
      return result;
      ...
      for (let i = 0; i < items.length; i++) {
        result = updateNode(result, items[i].id, (node) => ({
          ...
          transform: {
            ...node.transform,
            position: { x: items[i].x, y: min + i * step },
          },
        }));
      }
      return result;
...
export class AddDisplayElementCommand implements SceneCommand {
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    return updateNode(clone(doc), this.symbolId, (node) => {
      const children = [
        ...((node as { children?: DisplayElement[] }).children ?? []),
        clone(this.element),
      ];
...
export class ChangeLayerPropertyCommand implements SceneCommand {
  ...
  constructor(
    private layerId: string,
    private patch: Partial<
      Pick<LayerDefinition, "name" | "visible" | "locked" | "order">
    >,
    private prevPatch: Partial<
      Pick<LayerDefinition, "name" | "visible" | "locked" | "order">
    >,
...
export class ChangeShapeConfigurationCommand implements SceneCommand {
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    return updateNode(clone(doc), this.nodeId, (node) => ({
      ...node,
      shapeRef: {
        ...(node as { shapeRef: Record<string, unknown> }).shapeRef,
        configuration: this.newConfig,
      },
    }));
  }
...
export class AddComposablePartCommand implements SceneCommand {
  ...
  execute(doc: GraphicDocument): GraphicDocument {
    return updateNode(clone(doc), this.symbolId, (node) => {
      const parts = [
        ...((node as { composableParts?: ComposablePart[] }).composableParts ??
          []),
        clone(this.part),
      ];
...
export class DetachDisplayElementCommand implements SceneCommand {
  ...
  undo(doc: GraphicDocument): GraphicDocument {
    ...
    if (parentResult) {
      ...
      parent.children = [
        ...(parent.children ?? []),
        clone(this.elementSnapshot) as unknown as SceneNode,
      ];
    }
    return d;
  }
