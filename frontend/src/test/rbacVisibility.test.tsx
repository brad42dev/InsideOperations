...
import PermissionGuard, {
  ForbiddenPage,
  LoadingSpinner,
} from "../shared/components/PermissionGuard";
...
describe("PermissionGuard — permission checks", () => {
  it("renders children when user has the required permission", () => {
    setAuthState({
      user: makeUser(["console:read", "process:read"]),
  ...
  it("admin with system:configure can access SMS providers page guard", () => {
    setAuthState({
      user: makeUser(["system:configure"]),
      isAuthenticated: true,
    });
    render(
      <MemoryRouter>
  ...
  it("user without system:configure sees Access Denied on SMS providers page guard", () => {
    setAuthState({
      user: makeUser(["settings:read", "auth:manage_mfa"]),
      isAuthenticated: true,
    });
    render(
      <MemoryRouter>
...
describe("inline permission checks (UI element gating)", () => {
  it("canDo returns true when permission is present", () => {
    expect(canDo(["console:read", "console:write"], "console:write")).toBe(
      true,
    );
  });
  ...
  it("publish button visible only to users with console:workspace_publish", () => {
    ...
    expect(canDo(publisher.permissions, "console:workspace_publish")).toBe(
      true,
    );
  ...
  it("admin with system:configure can access EULA settings page guard", () => {
    setAuthState({
      user: makeUser(["system:configure", "system:manage_users"]),
      isAuthenticated: true,
    });
    render(
      <MemoryRouter>
  ...
  it("user without system:configure sees Access Denied on EULA settings page guard", () => {
    setAuthState({
      user: makeUser(["settings:read", "system:manage_users"]),
      isAuthenticated: true,
    });
    render(
      <MemoryRouter>
