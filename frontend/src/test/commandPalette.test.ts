...
describe('toAlarmRow — priority mapping', () => {
  it('maps critical severity to critical priority', () => {
    expect(toAlarmRow({ ...BASE_ALARM, severity: 'critical' }).priority).toBe('critical')
  })

  it('maps high severity to high priority', () => {
    expect(toAlarmRow({ ...BASE_ALARM, severity: 'high' }).priority).toBe('high')
  })

  it('maps medium severity to medium priority', () => {
    expect(toAlarmRow({ ...BASE_ALARM, severity: 'medium' }).priority).toBe('medium')
  })
  ...
  it('maps info severity to low priority', () => {
    expect(toAlarmRow({ ...BASE_ALARM, severity: 'info' }).priority).toBe('low')
  })
})

describe('toAlarmRow — state mapping', () => {
  it('maps active + no ack to active state', () => {
    expect(toAlarmRow({ ...BASE_ALARM, state: 'active', acknowledged_at: null }).state).toBe('active')
  })

  it('maps non-active + no ack to unacknowledged', () => {
    expect(toAlarmRow({ ...BASE_ALARM, state: 'resolved', acknowledged_at: null }).state).toBe('unacknowledged')
  })

  it('maps acknowledged_at present to acknowledged state', () => {
    expect(toAlarmRow({ ...BASE_ALARM, acknowledged_at: '2026-03-18T10:05:00.000Z' }).state).toBe('acknowledged')
  })

  it('acknowledged takes priority over active state', () => {
    expect(toAlarmRow({ ...BASE_ALARM, state: 'active', acknowledged_at: '2026-03-18T10:05:00.000Z' }).state).toBe('acknowledged')
  })
})
...
describe('toAlarmRow — tag and message fallbacks', () => {
  ...
  it('uses message field when provided', () => {
    expect(toAlarmRow({ ...BASE_ALARM, message: 'Override message' }).message).toBe('Override message')
  })

  it('falls back to title when message is absent', () => {
    expect(toAlarmRow({ ...BASE_ALARM, message: undefined }).message).toBe('High pressure')
  })
