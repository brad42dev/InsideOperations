...
const constant = (value: number): ExpressionTile => ({
  id: "c",
  type: "constant",
  value,
});
...
const pointRef = (pointId: string): ExpressionTile => ({
  id: "p",
  type: "point_ref",
  pointId,
});
const currentRef = (): ExpressionTile => ({ id: "cr", type: "point_ref" });
const grp = (children: ExpressionTile[]): ExpressionTile => ({
  id: "g",
  type: "group",
  children,
});
const square = (children: ExpressionTile[]): ExpressionTile => ({
  id: "sq",
  type: "square",
  children,
});
const negate = (children: ExpressionTile[]): ExpressionTile => ({
  id: "n",
  type: "negate",
  children,
});
...
describe("evaluateExpression — constants", () => {
  ...
  it("returns null for a constant with NaN value", () => {
    expect(
      evaluateExpression([{ id: "x", type: "constant", value: NaN }], {}),
    ).toBeNull();
  });
});
...
describe("evaluateExpression — basic arithmetic", () => {
  ...
  it("subtracts constants", () => {
    expect(
      evaluateExpression([constant(10), subtract(), constant(4)], {}),
    ).toBe(6);
  });

  it("multiplies constants", () => {
    expect(evaluateExpression([constant(3), multiply(), constant(5)], {})).toBe(
      15,
    );
  });
  ...
  it("divides constants", () => {
    expect(
      evaluateExpression([constant(10), divide(), constant(4)], {}),
    ).toBeCloseTo(2.5);
  });

  it("returns null for division by zero", () => {
    expect(
      evaluateExpression([constant(5), divide(), constant(0)], {}),
    ).toBeNull();
  });
});
...
describe("evaluateExpression — point references", () => {
  ...
  it("adds a point reference to a constant", () => {
    expect(
      evaluateExpression([pointRef("pt-001"), add(), constant(4.5)], vals),
    ).toBe(30);
  });
  ...
  it("uses current_point value when pointId is undefined", () => {
    expect(
      evaluateExpression([currentRef(), multiply(), constant(2)], vals, 7),
    ).toBe(14);
  });
...
describe("evaluateExpression — groups", () => {
  it("evaluates a group", () => {
    expect(
      evaluateExpression([grp([constant(3), add(), constant(2)])], {}),
    ).toBe(5);
  });
...
describe("evaluateExpression — negate", () => {
  ...
  it("negates a point value", () => {
    expect(evaluateExpression([negate([pointRef("pt")])], { pt: 3.5 })).toBe(
      -3.5,
    );
  });
});
