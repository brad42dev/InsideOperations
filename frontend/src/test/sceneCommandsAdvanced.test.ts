...
describe("AlignNodesCommand", () => {
  ...
  it("undo restores previous positions", () => {
    ...
    expect(
      restored.children.find((n) => n.id === "a")!.transform.position.x,
    ).toBe(10);
    expect(
      restored.children.find((n) => n.id === "b")!.transform.position.x,
    ).toBe(80);
  });
});
...
describe("DistributeNodesCommand", () => {
  ...
  it("does nothing with 2 nodes", () => {
    ...
    expect(
      result.children.find((n) => n.id === "a")!.transform.position.x,
    ).toBe(0);
    expect(
      result.children.find((n) => n.id === "b")!.transform.position.x,
    ).toBe(100);
  });
...
describe("FlipNodesCommand", () => {
  it("flips horizontal: none → horizontal", () => {
    const doc = makeDoc([makeGroup("n1", 0, 0)]);
    const prevTf = baseTransform(0, 0);
    const cmd = new FlipNodesCommand(
      ["n1"],
      "horizontal",
      new Map([["n1", prevTf]]),
    );
