...
describe('isPermission — valid permission strings', () => {
  ...
  it('accepts all 7 console permissions', () => {
    ...
    for (const p of consolePerms) {
      expect(isPermission(p), `expected isPermission('${p}') to be true`).toBe(true)
    }
  })

  it('accepts all 6 process permissions', () => {
    const perms = ['process:read', 'process:write', 'process:publish', 'process:delete', 'process:export', 'process:admin']
...
describe('isPermission — count verification', () => {
  it('all 118 permissions resolve to true', () => {
    const allPerms: string[] = [
      // Console (7)
      'console:read', 'console:write', 'console:workspace_write',
      'console:workspace_publish', 'console:workspace_delete', 'console:export', 'console:admin',
      // Process (6)
      'process:read', 'process:write', 'process:publish', 'process:delete', 'process:export', 'process:admin',
      // Designer (7)
      'designer:read', 'designer:write', 'designer:delete', 'designer:publish',
      'designer:import', 'designer:export', 'designer:admin',
      // Dashboards (6)
      ...
      // Events (5)
      'events:read', 'events:manage', 'events:acknowledge', 'events:shelve', 'events:admin',
      // Log (7)
      'log:read', 'log:write', 'log:delete', 'log:template_manage',
      'log:schedule_manage', 'log:export', 'log:admin',
      // Rounds (7)
      'rounds:read', 'rounds:execute', 'rounds:transfer', 'rounds:template_manage',
      'rounds:schedule_manage', 'rounds:export', 'rounds:admin',
      // Settings (4)
    ...
    expect(failed, `These permissions should be valid: ${failed.join(', ')}`).toHaveLength(0)
  })
})
