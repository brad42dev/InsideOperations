...
export default function ProcessDetachedView() {
  ...
  const zoomFit = useCallback(() => {
    ...
    const { width, height } = graphic.scene_data.canvas
    setViewport((vp) => {
      const fitZoom = Math.min(vp.screenWidth / width, vp.screenHeight / height)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--io-bg)', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <button onClick={zoomFit} title="Zoom to fit (Ctrl+0)" style={toolbarBtnStyle}>Fit</button>
        <button
        ...
        {/* Live clock */}
        <span style={{ fontSize: 11, color: 'var(--io-text-muted)', flexShrink: 0, fontFamily: 'monospace' }}>
          {clock}
        </span>
      </div>
      ...
      >
        ...
        {isLoading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--io-text-muted)', fontSize: 13 }}>
            <div className="io-skeleton" style={{ width: '80%', height: '80%', borderRadius: 8 }} />
          </div>
        )}
        ...
        {!isLoading && !graphic?.scene_data && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, color: 'var(--io-text-muted)' }}>
            ...
            <p style={{ margin: 0, fontSize: 14, fontWeight: 500, color: 'var(--io-text-primary)' }}>View not found</p>
