...
function GraphicThumb({ id, name, selected, onClick }: {
  id: string; name: string; selected: boolean; onClick: () => void
}) {
  ...
  return (
    ...
    >
      <div style={{ width: 36, height: 24, flexShrink: 0, background: 'var(--io-bg)', borderRadius: 2, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {errored ? (
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--io-text-muted)" strokeWidth="1.5">
      ...
      <span style={{ fontSize: 12, color: 'var(--io-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
        {name}
      </span>
    </button>
  )
  ...
  return (
    <div style={sidebarStyle}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--io-text-primary)', flex: 1 }}>
          Process
        </span>
        <button
          onClick={onToggle}
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <AccordionSection title="Bookmarks" defaultOpen>
          <div style={{ padding: '4px 0' }}>
            ...
            {bookmarks.length === 0 && (
              <div style={{ padding: '4px 12px 8px', fontSize: 11, color: 'var(--io-text-muted)' }}>
            ...
            {bookmarks.map((bm) => (
              ...
              >
                <span style={{ color: 'var(--io-accent)', fontSize: 12, flexShrink: 0 }}>★</span>
                <span style={{ fontSize: 12, color: 'var(--io-text-primary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {bm.name}
                </span>
                <span style={{ fontSize: 10, color: 'var(--io-text-muted)', flexShrink: 0 }}>
              ...
              {recentViews.map((v) => (
                <button
                  ...
                  style={{
                    ...
                    textAlign: 'left',
                    padding: '5px 12px',
                    background: v.id === selectedId ? 'var(--io-surface-secondary)' : 'transparent',
                    border: 'none',
                    cursor: 'pointer',
                    fontSize: 12,
                    color: 'var(--io-text-primary)',
