    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            onChange({ data_category_id: e.target.value || null })
          }
        >
          <option value="">(none)</option>
          {categories.map((cat) => (
...
function EditSourceDialog({
  ...
  const mutation = useMutation({
    mutationFn: (req: UpdatePointSourceRequest) =>
      pointSourcesApi.update(source!.id, req),
    onSuccess: (result) => {
      if (!result.success) {
        setError(result.error.message);
        return;
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 12, color: "var(--io-text-secondary)" }}>
            {formatDate(cert.first_seen_at)}
          </div>
        </div>
        <div>
          <div
      ...
      {cert.auto_trusted && (
        ...
        >
          Auto-trusted — this certificate was accepted automatically because{" "}
          <strong>OPC Auto-Trust</strong> is enabled on this server. Disable it
          in Settings to require manual approval for new connections.
        </div>
      )}
...
function ServerCertTab({ sourceId }: { sourceId: string }) {
  ...
  const isBusy =
    trustMut.isPending || rejectMut.isPending || deleteMut.isPending;
    ...
    return (
      ...
      >
        ...
        <div style={{ fontWeight: 500, marginBottom: 6 }}>
          No server certificate on record
        </div>
        <div
          style={{
            fontSize: 12,
        ...
        >
          The OPC UA server certificate will appear here the first time this
          source connects. Once seen, you can approve or reject it.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div
        style={{ fontSize: 12, color: "var(--io-text-muted)", lineHeight: 1.6 }}
      >
        The certificate(s) below were presented by the OPC UA server during
        connection. Trust a certificate to allow connections; reject it to block
        them.
      </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {jobs.map((job) => (
              ...
              >
                ...
                >
                  {new Date(job.from_time).toLocaleString()} →{" "}
                  {new Date(job.to_time).toLocaleString()}
                </div>
                {job.status === "complete" && (
        ...
        >
          ...
          >
            ...
            >
              <button
                style={
                  activeTab === "details" ? TAB_TRIGGER_ACTIVE : TAB_TRIGGER
                }
              ...
              <button
                style={
                  activeTab === "server-cert" ? TAB_TRIGGER_ACTIVE : TAB_TRIGGER
                }
...
function MinPublishIntervalControl() {
  ...
  const settingQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const setting = (
        result.data as import("../../api/settings").Setting[]
      ).find((s) => s.key === MIN_INTERVAL_KEY);
  ...
  return (
    ...
    >
      ...
      >
        No OPC source can be configured below this floor. Protects DCS/historian
        systems from aggressive polling. Default: {MIN_INTERVAL_DEFAULT}ms.
        Minimum allowed: {MIN_INTERVAL_FLOOR}ms.
      </div>
...
export default function OpcSourcesPage() {
  ...
  const sourcesQuery = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const hasActive = data.some(
        (s) => s.status === "connecting" || s.status === "error",
      );
  ...
  return (
    <div>
      ...
      >
        ...
        {!sourcesQuery.isLoading && sources.length === 0 && (
          ...
          >
            ...
            <p style={{ margin: 0 }}>
              Add a source to start receiving real-time data.
            </p>
          </div>
        )}
        {sources.length > 0 && (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              ...
              >
                {["Name", "Endpoint", "Status", "Last Connected", ""].map(
                  (col) => (
                    <th
                      key={col}
                      style={{
                      ...
                      {src.status !== "active" && src.enabled && (
                        <button
                          style={{
                            ...
                            color: "var(--io-accent)",
                            fontSize: "12px",
                            cursor: reconnectMutation.isPending
                              ? "not-allowed"
                              : "pointer",
      ...
      <SourceDetailPanel
        ...
        onReconnect={() =>
          detailSource && reconnectMutation.mutate(detailSource.id)
        }
