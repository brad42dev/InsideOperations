...
const CRITICALITY_COLORS: Record<string, { bg: string; color: string }> = {
  ...
  informational: { bg: 'rgba(113,113,122,0.15)', color: 'var(--io-text-muted)' },
}
...
function CriticalityBadge({ value }: { value: string | null }) {
  ...
  const style = CRITICALITY_COLORS[value] ?? { bg: 'var(--io-surface-secondary)', color: 'var(--io-text-muted)' }
  return (
    <span
      style={{
...
function AggBadges({ mask }: { mask: number }) {
  ...
  if (active.length === 0) return <span style={{ color: 'var(--io-text-muted)', fontSize: '12px' }}>none</span>
  return (
  ...
  return (
    <tbody>
      {Array.from({ length: rows }).map((_, ri) => (
        <tr key={ri} style={{ borderBottom: '1px solid var(--io-border-subtle)' }}>
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        >
          ...
          {isDeactivate ? (
            <p style={{ margin: '0 0 24px', fontSize: '14px', color: 'var(--io-text-secondary)', lineHeight: 1.55 }}>
              This will hide{' '}
              <strong style={{ color: 'var(--io-text-primary)' }}>{point.tag_name}</strong> from
              operational views. It can be reactivated later.
            </p>
          ) : (
            <p style={{ margin: '0 0 24px', fontSize: '14px', color: 'var(--io-text-secondary)', lineHeight: 1.55 }}>
              Reactivate{' '}
              <strong style={{ color: 'var(--io-text-primary)' }}>{point.tag_name}</strong> and
              restore it to operational views?
            </p>
          )}
...
function HistoryTab({ pointId }: { pointId: string }) {
  ...
  if (query.isLoading) {
    return (
      <div style={{ padding: '20px', textAlign: 'center', color: 'var(--io-text-muted)', fontSize: '13px' }}>
        Loading history...
      </div>
    )
  }
  ...
  if (query.isError) {
    return (
      <ErrorBanner
        message={query.error instanceof Error ? query.error.message : 'Failed to load history'}
      />
    )
  }
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
            {Object.entries(v.changes).map(([field, change]) => (
              <div key={field} style={{ fontSize: '12px', color: 'var(--io-text-secondary)' }}>
                <span style={{ fontWeight: 500, color: 'var(--io-text-primary)' }}>{field}</span>
                {': '}
                <span style={{ color: 'var(--io-danger)', textDecoration: 'line-through', marginRight: '4px' }}>
...
type ConfigTab = 'general' | 'aggregation' | 'location' | 'conversion' | 'history'
...
function PointConfigDialog({
    ...
    if (point && open) {
      ...
      setExpressionInfo(
        point.custom_expression_id && point.custom_expression_name
          ? { id: point.custom_expression_id, name: point.custom_expression_name }
          : null,
      )
      setFormError(null)
      setTab('general')
  ...
  const updateMutation = useMutation({
    mutationFn: (req: UpdatePointConfigRequest) => pointConfigApi.update(point!.id, req),
    onSuccess: (result) => {
      if (!result.success) {
  ...
  return (
    <>
        ...
        <Dialog.Portal>
          ...
          >
            ...
            >
              <div>
                <Dialog.Title
                  style={{ margin: '0 0 2px', fontSize: '16px', fontWeight: 600, color: 'var(--io-text-primary)' }}
                >
                  Configure Point
                </Dialog.Title>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                >
                  <input
                    ...
                    style={{ accentColor: 'var(--io-accent)', width: '15px', height: '15px' }}
                  />
                  <div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        criticality: e.target.value === '' ? null : (e.target.value as UpdatePointConfigRequest['criticality']),
                      }))
                    }
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        write_frequency_seconds: e.target.value === '' ? null : Number(e.target.value),
                      }))
                    }
            ...
            {tab === 'aggregation' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                >
                  <strong style={{ color: 'var(--io-text-secondary)' }}>Note:</strong> min, max, and
                  count aggregations are always available for all points. The toggles below control
                  which additional operations are semantically meaningful for this point.
                </div>
                ...
                {[
                  ...
                  },
                  {
                    ...
                    description: 'Value represents a running total or accumulator',
                  },
                  ...
                  <label
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      padding: '10px 12px',
                      background: hasAgg(aggMask, bit) ? 'rgba(var(--io-accent-rgb, 234,179,8),0.08)' : 'var(--io-surface-secondary)',
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          aggregation_types: setAgg(aggMask, bit, e.target.checked),
                        }))
                      }
                      style={{ accentColor: 'var(--io-accent)', width: '14px', height: '14px', marginTop: '2px', flexShrink: 0 }}
                    />
                    <div>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          gps_latitude: e.target.value === '' ? null : Number(e.target.value),
                        }))
                      }
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          gps_longitude: e.target.value === '' ? null : Number(e.target.value),
                        }))
                      }
            ...
            {tab === 'conversion' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                >
                  A custom conversion expression transforms the raw OPC value before it is stored.
                  Leave unset to use the raw value.
                </div>
                ...
                {expressionInfo ? (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                      <button style={btnSmall} onClick={() => setExpressionBuilderOpen(true)}>
                        Edit
                      </button>
                      <button
                        ...
                        onClick={() => {
                          ...
                          setForm((f) => ({ ...f, custom_expression_id: null }))
                        }}
                      >
                        Clear
                      </button>
      ...
      {expressionBuilderOpen && (
        <Dialog.Root open={expressionBuilderOpen} onOpenChange={setExpressionBuilderOpen}>
          <Dialog.Portal>
            <Dialog.Overlay
              style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 210 }}
            />
            <Dialog.Content
              style={{
                position: 'fixed',
...
function BulkAggregationBar({
  ...
  return (
    ...
    >
      <span style={{ fontSize: '13px', color: 'var(--io-text-secondary)', fontWeight: 500 }}>
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
        {[
          ...
          <label
            key={bit}
            style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', fontSize: '12px', color: 'var(--io-text-secondary)' }}
          >
            <input
              type="checkbox"
      ...
      <button
        style={{ ...btnPrimary, padding: '6px 14px', fontSize: '12px', marginLeft: 'auto' }}
        disabled={isPending}
...
export default function PointManagement() {
  ...
  // Lifecycle dialog
  const [lifecyclePoint, setLifecyclePoint] = useState<PointConfig | null>(null)
  ...
  return (
    <div>
      ...
      >
        <div>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: 'var(--io-text-muted)' }}>
            Configure per-point aggregation types, application settings, and lifecycle
          </p>
        </div>
        <ExportButton
          module="settings"
      ...
      >
        ...
        {/* Active filter */}
        <div style={{ display: 'flex', border: '1px solid var(--io-border)', borderRadius: 'var(--io-radius)', overflow: 'hidden', flexShrink: 0 }}>
          {(['active', 'inactive', 'all'] as const).map((v) => (
            <button
              key={v}
              style={{
                padding: '7px 12px',
                background: activeFilter === v ? 'var(--io-surface-secondary)' : 'transparent',
                border: 'none',
                borderRight: v !== 'all' ? '1px solid var(--io-border)' : 'none',
                color: activeFilter === v ? 'var(--io-text-primary)' : 'var(--io-text-muted)',
                fontSize: '12px',
        ...
        {/* Source filter */}
        <select
          style={{ ...inputStyle, maxWidth: '200px', flex: '0 1 160px', cursor: 'pointer' }}
          value={sourceFilter}
        ...
        {/* Row count */}
        {pagination && (
          <span style={{ fontSize: '12px', color: 'var(--io-text-muted)', marginLeft: 'auto', flexShrink: 0 }}>
                ...
                {points.map((point, i) => {
                  ...
                  return (
                    <tr
                      key={point.id}
                      style={{
                        borderBottom: i < points.length - 1 ? '1px solid var(--io-border-subtle)' : undefined,
                        background: selected
                    ...
                    >
                      {/* Checkbox */}
                      <td style={{ ...tdStyle, width: '40px', paddingRight: '4px' }}>
                        <input
                          type="checkbox"
                          checked={selected}
                      ...
                      <td style={tdStyle}>
                        ...
                        {point.display_name && (
                          <div
                            style={{ fontSize: '11px', color: 'var(--io-text-muted)', marginTop: '1px' }}
                          >
                            {point.display_name}
                          </div>
                        )}
                      ...
                      <td style={tdStyle}>
                        <div style={{ display: 'flex', gap: '5px' }}>
                          <button style={btnSmall} onClick={() => openConfig(point)}>
                            Configure
                          </button>
                          {point.active ? (
                            <button
      ...
      {pagination && pagination.pages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center' }}>
          <button
            style={{ ...btnSmall, opacity: page <= 1 ? 0.4 : 1, cursor: page <= 1 ? 'not-allowed' : 'pointer' }}
            disabled={page <= 1}
      ...
      <LifecycleDialog
        ...
        onConfirm={() => {
          if (lifecyclePoint) {
            lifecycleMutation.mutate({ id: lifecyclePoint.id, active: lifecyclePoint.active })
          }
        }}
