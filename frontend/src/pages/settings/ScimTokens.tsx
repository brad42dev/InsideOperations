  ...
  return [
    {
      ...
      steps: [
        ...
        },
        {
          ...
          warning: "The token is only shown once. If you didn't copy it, revoke it and create a new one.",
        },
        {
      ...
      steps: [
        {
          ...
          warning: "Standard Google Workspace editions do not support outbound SCIM push. See the note below.",
        },
        {
    ...
    },
    {
      ...
      steps: [
        ...
        },
        {
          ...
          warning: "The token is only shown once when created. Copy it immediately.",
        },
        {
...
function ScimSetupGuide({ scimEndpointUrl }: ScimSetupGuideProps) {
  ...
  const guides = useMemo(() => buildScimGuides(scimEndpointUrl), [scimEndpointUrl]);
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "12px", color: "var(--io-text-muted)", marginTop: "1px" }}>
              Step-by-step instructions for configuring SCIM in your identity provider
            </div>
          </div>
        </div>
        <span style={{ color: "var(--io-text-muted)", fontSize: "13px", fontWeight: 600 }}>
          {open ? "Hide ▲" : "Show ▼"}
        </span>
      </button>

      {open && (
        <div style={{ padding: "16px 20px 20px", background: "var(--io-surface)" }}>
          {/* IdP selector */}
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px", flexWrap: "wrap" }}>
            <label style={{ fontSize: "12px", color: "var(--io-text-secondary)", fontWeight: 500, whiteSpace: "nowrap" }}>
              I'm using:
            </label>
            ...
            {guide.steps.map((step, i) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: "13px", fontWeight: 600, color: "var(--io-text-primary)", marginBottom: "3px" }}>
                    {step.title}
                  </div>
...
function CreateDialog({ open, onOpenChange, onCreated }: CreateDialogProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: CreateScimTokenPayload) => scimApi.createToken(payload),
    onSuccess: (result) => {
      if (result.success) {
        reset();
  ...
  return (
    ...
    >
      <Dialog.Portal>
        <Dialog.Overlay
          style={{ position: "fixed", inset: 0, background: "var(--io-overlay, rgba(0,0,0,0.5))", zIndex: 100 }}
        />
        <Dialog.Content
  ...
  return (
    <Dialog.Root open={!!result} onOpenChange={(v) => { if (!v) onClose(); }}>
      <Dialog.Portal>
        ...
        >
          <Dialog.Title style={{ margin: "0 0 6px", fontSize: "16px", fontWeight: 600, color: "var(--io-text-primary)" }}>
            SCIM Token Created
          </Dialog.Title>
          <Dialog.Description style={{ display: "none" }}>
            Your new SCIM bearer token. Copy it now — it will not be shown again.
          </Dialog.Description>
          <div
            style={{
              display: "flex",
              ...
              borderRadius: "var(--io-radius)",
              background: "color-mix(in srgb, var(--io-warning) 12%, transparent)",
              border: "1px solid color-mix(in srgb, var(--io-warning) 40%, transparent)",
              marginBottom: "16px",
            }}
          >
            <span style={{ fontSize: "16px" }}>⚠</span>
            <p style={{ margin: 0, fontSize: "13px", color: "var(--io-text-secondary)", fontWeight: 500 }}>
...
export default function ScimTokensSection() {
  ...
  const [newTokenResult, setNewTokenResult] = useState<CreateScimTokenResponse | null>(null);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["scim-tokens"] }); },
  });
    ...
    <div>
      ...
      >
        <div>
          ...
          <p style={{ margin: "0 0 14px", fontSize: "13px", color: "var(--io-text-secondary)" }}>
            SCIM enables your identity provider to automatically create, update, and disable user accounts
            in I/O when changes happen in your directory. Tokens below authenticate IdP sync requests.
          </p>
          ...
          >
            ...
            <p style={{ margin: "6px 0 0", fontSize: "11px", color: "var(--io-text-muted)" }}>
              Provide this URL to your identity provider when configuring SCIM provisioning.
            </p>
          </div>
        </div>

        <button onClick={() => setShowCreate(true)} style={{ ...btnPrimary, whiteSpace: "nowrap", marginTop: "28px" }}>
          + New Token
        </button>
      </div>
      ...
      {/* Token table */}
      {isLoading ? (
        <div style={{ fontSize: "13px", color: "var(--io-text-muted)", padding: "12px 0" }}>
          Loading…
        </div>
      ) : tokens.length === 0 ? (
        <div
        ...
        >
          ...
          <p style={{ fontSize: "13px", color: "var(--io-text-secondary)", margin: "0 0 6px", fontWeight: 500 }}>
            No SCIM tokens yet
          </p>
          <p style={{ fontSize: "12px", color: "var(--io-text-muted)", margin: 0 }}>
            Create a token to enable IdP provisioning. Each IdP connection should use its own token.
          </p>
        </div>
      ) : (
        <div style={{ border: "1px solid var(--io-border)", borderRadius: "var(--io-radius)", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
            ...
            <tbody>
              {tokens.map((token, idx) => (
                <tr key={token.id} style={{ borderTop: idx > 0 ? "1px solid var(--io-border)" : undefined }}>
                  <td style={{ padding: "12px 16px", color: "var(--io-text-primary)" }}>
                  ...
                  <td style={{ padding: "12px 16px", color: "var(--io-text-secondary)" }}>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <span
                      style={{
                        ...
                        fontSize: "11px",
                        fontWeight: 600,
                        background: token.enabled ? "var(--io-success-subtle)" : "var(--io-surface-secondary)",
                        color: token.enabled ? "var(--io-success)" : "var(--io-text-muted)",
                      }}
                    >
      ...
      <CreateDialog open={showCreate} onOpenChange={setShowCreate} onCreated={handleCreated} />

      <RevealDialog result={newTokenResult} onClose={() => setNewTokenResult(null)} />
