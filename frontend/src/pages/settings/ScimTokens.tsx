...
import { scimApi, CreateScimTokenPayload, CreateScimTokenResponse } from '../../api/scim'
...
export default function ScimTokensSection() {
  ...
  const handleDelete = (id: string, name: string) => {
    if (window.confirm(`Revoke SCIM token "${name}"? IdPs using this token will lose access.`)) {
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            Bearer tokens for identity providers (Azure AD, Okta) to push user changes via SCIM 2.0.
            SCIM endpoint: <code style={{ fontSize: '12px' }}>/scim/v2</code>
          </p>
        </div>
        <button
            ...
            <tbody>
              {tokens.map((token, idx) => (
                <tr
                  key={token.id}
                  style={{
                    borderTop: idx > 0 ? '1px solid var(--io-border)' : undefined,
                  }}
                >
      ...
      {newTokenResult && (
        <RevealModal result={newTokenResult} onClose={() => setNewTokenResult(null)} />
      )}
    </div>
  )
}
