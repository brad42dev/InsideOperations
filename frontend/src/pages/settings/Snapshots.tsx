...
function Pill({ children, color }: { children: React.ReactNode; color: 'accent' | 'muted' | 'success' }) {
  const colors = {
...
function downloadSnapshotJson(snap: SnapshotDetail) {
  ...
  a.href = url
  const label = snap.label ? snap.label.replace(/\s+/g, '-') : snap.id.slice(0, 8)
...
function CreateSnapshotForm({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      snapshotsApi.create({ target_type: targetType, label: label.trim() || undefined }),
    onSuccess: (res) => {
      if (res.success) {
    ...
    onError: () => {
      showToast({ title: 'Failed to create snapshot', description: 'Network error', variant: 'error' })
    },
  })

  return (
    <div style={{ ...CARD, borderColor: 'var(--io-accent)', marginBottom: 'var(--io-space-5)' }}>
      <h3 style={{ margin: '0 0 var(--io-space-4)', fontSize: '13px', fontWeight: 600, color: 'var(--io-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        New Snapshot
      </h3>
      <div style={{ display: 'flex', gap: 'var(--io-space-3)', flexWrap: 'wrap', alignItems: 'flex-end' }}>
        <div>
          <label style={{ display: 'block', fontSize: '12px', color: 'var(--io-text-muted)', marginBottom: 4 }}>
            Target Type
          </label>
          <select
            style={SELECT}
        ...
        <div>
          <label style={{ display: 'block', fontSize: '12px', color: 'var(--io-text-muted)', marginBottom: 4 }}>
            Label <span style={{ color: 'var(--io-text-muted)' }}>(optional)</span>
          </label>
          <input
            style={INPUT}
      ...
      <p style={{ margin: 'var(--io-space-3) 0 0', fontSize: '12px', color: 'var(--io-text-muted)' }}>
        A snapshot captures the current state of all records for the selected target type. Use the downloaded JSON as an audit trail or import baseline.
      </p>
    </div>
  )
}
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast({ title: 'Download failed', description: res.error.message, variant: 'error' })
      }
    } catch {
      showToast({ title: 'Download failed', description: 'Network error', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ padding: 'var(--io-space-6)', maxWidth: 900 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--io-space-5)' }}>
        <div>
          <h2 style={{ margin: '0 0 4px', fontSize: '20px', fontWeight: 700, color: 'var(--io-text-primary)' }}>
            Change Snapshots
          </h2>
          <p style={{ margin: 0, fontSize: '13px', color: 'var(--io-text-muted)' }}>
            Point-in-time captures of configuration data. Download as JSON for auditing or use as a restore baseline via Bulk Update.
          </p>
        </div>
        {!showCreate && (
      ...
      <div style={{ ...CARD, padding: 0, overflow: 'hidden' }}>
        {isLoading && (
          <p style={{ padding: 'var(--io-space-5)', margin: 0, color: 'var(--io-text-muted)', fontSize: '13px' }}>
            Loading snapshots…
          </p>
        )}
        ...
        {!isLoading && !isError && snapshots.length === 0 && (
          <div style={{ padding: 'var(--io-space-6)', textAlign: 'center' }}>
            <p style={{ margin: '0 0 var(--io-space-2)', fontSize: '14px', fontWeight: 500, color: 'var(--io-text-secondary)' }}>
              No snapshots yet
            </p>
            <p style={{ margin: 0, fontSize: '13px', color: 'var(--io-text-muted)' }}>
              Create a snapshot to capture the current state of a configuration dataset.
            </p>
          </div>
        )}
            ...
            <tbody>
              {snapshots.map((snap) => (
                <tr key={snap.id}>
                  ...
                  <td style={{ ...TD, fontSize: '12px', fontFamily: 'monospace', color: 'var(--io-text-secondary)' }}>
                    {fmtDate(snap.created_at)}
                  </td>
                  <td style={{ ...TD, fontSize: '12px', color: 'var(--io-text-muted)' }}>
                    {snap.created_by ?? '—'}
                  </td>
      ...
      {pagination && pagination.pages > 1 && (
        <div style={{ display: 'flex', gap: 'var(--io-space-2)', alignItems: 'center', marginTop: 'var(--io-space-3)' }}>
          <button
            style={BTN_SECONDARY}
            disabled={page <= 1}
