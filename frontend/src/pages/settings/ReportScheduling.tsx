...
function LockedNotice() {
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontWeight: 600, color: 'var(--io-text-secondary)', marginBottom: '2px' }}>
          Read-only access
        </div>
        <div>You need the <strong>reports:admin</strong> permission to create or modify report schedules.</div>
      </div>
    </div>
  )
}
...
function ScheduleModal({
  ...
  const canSave = form.template_id && form.name.trim() && form.cron_expression.trim()
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            />
            <label htmlFor="sched-enabled" style={{ fontSize: '13px', color: 'var(--io-text-secondary)', cursor: 'pointer' }}>
              Enable immediately
            </label>
          </div>
...
export default function ReportScheduling() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      setAddOpen(false)
      queryClient.invalidateQueries({ queryKey: ['settings-report-schedules'] })
      showToast({ title: 'Schedule created', variant: 'success' })
    },
    onError: (err) => {
      showToast({ title: 'Failed to create schedule', description: err instanceof Error ? err.message : 'Unknown error', variant: 'error' })
    },
  })
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (!result.success) {
        showToast({ title: 'Failed to update', description: result.error.message, variant: 'error' })
        return
      }
      queryClient.invalidateQueries({ queryKey: ['settings-report-schedules'] })
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (!result.success) {
        showToast({ title: 'Failed to delete', description: result.error.message, variant: 'error' })
        return
      }
      queryClient.invalidateQueries({ queryKey: ['settings-report-schedules'] })
