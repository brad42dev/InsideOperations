...
function StatusDot({ status }: { status: string | null }) {
  ...
  else if (status === 'error' || status === 'failed') color = 'var(--io-danger)'
...
function AddConnectorDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      qc.invalidateQueries({ queryKey: ['supplemental-connectors', pointSourceId] })
      onOpenChange(false)
      setForm(EMPTY_FORM)
      setError(null)
    },
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const data: CreateSupplementalConnectorData = {
      ...
      config: isRestType(form.connection_type) ? { base_url: form.base_url } : {},
      auth_config: buildAuthConfig(),
    }
    mutation.mutate(data)
  }

  const patch = (p: Partial<AddConnectorForm>) => setForm((f) => ({ ...f, ...p }))
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ fontSize: '11px', color: 'var(--io-text-muted)', marginTop: '2px' }}>
            {connector.last_test_message
              ...
              : connector.last_tested_at
                ...
                : 'Not yet tested'}
            {!connector.enabled && (
              <span style={{ marginLeft: '6px', color: 'var(--io-text-muted)' }}>(disabled)</span>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
          <button
            style={btnSmall}
            onClick={() => { setTestError(null); testMutation.mutate() }}
          ...
          <button
            ...
            onClick={() => {
              if (confirm(`Delete supplemental connector "${connector.name}"?`)) {
      ...
      {/* Test error */}
      {testError && (
        <div style={{ fontSize: '12px', color: 'var(--io-danger)', paddingLeft: '20px' }}>
          {testError}
        </div>
      )}
    </div>
...
export default function SupplementalConnectorsTab({ pointSourceId, pointSourceName }: Props) {
  ...
  return (
    <div>
      ...
      >
        <div>
          <p style={{ margin: '0 0 4px', fontSize: '13px', color: 'var(--io-text-secondary)' }}>
            Supplemental point data connectors fill historical or metadata gaps from{' '}
            <strong style={{ color: 'var(--io-text-primary)' }}>{pointSourceName}</strong> using
            additional data sources. These are not general-purpose import connectors.
          </p>
        </div>
      ...
      {connectorsQuery.isLoading && (
        <div
          style={{ padding: '32px', textAlign: 'center', color: 'var(--io-text-muted)', fontSize: '13px' }}
        >
          Loading…
        </div>
      )}
      ...
      {!connectorsQuery.isLoading && !connectorsQuery.isError && connectors.length === 0 && (
        ...
        >
          ...
          <p style={{ margin: 0 }}>
            Add a connector to pull additional point data that is not available directly from the OPC
            UA source.
          </p>
        </div>
      )}
      ...
      {connectors.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {connectors.map((c) => (
            <ConnectorCard key={c.id} connector={c} pointSourceId={pointSourceId} />
          ))}
        </div>
      )}
