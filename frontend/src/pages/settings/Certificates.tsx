          ...
          >
            ...
            <dl style={{ margin: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[
                ...
                ['Days Remaining', cert.is_expired ? 'Expired' : `${cert.days_remaining} days`],
                ...(cert.sans.length > 0 ? [['SANs', cert.sans.join(', ')]] : []),
              ].map(([label, value]) => (
                <div key={label} style={{ display: 'flex', gap: '12px' }}>
                  <dt style={{ fontSize: '12px', fontWeight: 600, color: 'var(--io-text-muted)', width: '120px', flexShrink: 0, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    {label}
                  </dt>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
              <button onClick={() => setDetailOpen(false)} style={cancelBtnStyle}>
                Close
              </button>
            </div>
          </div>
...
function UploadModal({ onClose, onUploaded }: UploadModalProps) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      formData.append('cert', new Blob([certPem], { type: 'text/plain' }), 'cert.pem')
      formData.append('key', new Blob([keyPem], { type: 'text/plain' }), 'key.pem')
  ...
  const handleSubmit = () => {
    ...
    if (!certPem.trim()) { setError('Certificate PEM is required'); return }
    if (!keyPem.trim()) { setError('Private Key PEM is required'); return }
      ...
      >
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <textarea
            ...
            value={certPem}
            onChange={(e) => { setCertPem(e.target.value); setError('') }}
            rows={6}
            style={textareaStyle}
          />
        </div>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <textarea
            ...
            value={keyPem}
            onChange={(e) => { setKeyPem(e.target.value); setError('') }}
            rows={6}
            style={textareaStyle}
          />
        </div>
...
export default function CertificatesPage() {
  ...
  const [contextMenu, setContextMenu] = useState<{ cert: CertInfo; pos: ContextMenuPos } | null>(null)
  ...
  const handleDelete = (name: string) => {
    if (window.confirm(`Delete certificate "${name}"? This cannot be undone.`)) {
  ...
  return (
    <div>
      ...
      >
        <div>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: 'var(--io-text-muted)' }}>
            Manage TLS certificates for HTTPS termination and service authentication.
          </p>
        </div>
      ...
      {/* Body */}
      {isLoading ? (
        <div style={{ color: 'var(--io-text-muted)', fontSize: '13px', marginTop: '24px' }}>
          Loading…
        </div>
      ) : isError ? (
        <div
        ...
        >
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              ...
              >
                {['Name', 'Subject', 'Issuer', 'Expiry', 'Days Remaining', 'Status', ''].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
            ...
            <tbody>
              {certs.map((cert) => (
                <tr
                  key={cert.name}
                  style={{ borderBottom: '1px solid var(--io-border-subtle, var(--io-border))' }}
        ...
        <div style={{ marginTop: '16px' }}>
          {certs
            ...
            .map((cert) => (
              ...
              >
                <strong style={{ color: 'var(--io-text-primary)' }}>{cert.name}</strong>
                {' — SANs: '}
                {cert.sans.join(', ')}
              </div>
            ))}
...
function OpcServerCertsSection() {
  ...
  const mutOpts = { onSuccess: () => queryClient.invalidateQueries({ queryKey: ['opc-server-certs'] }) }
  ...
  return (
    <div style={{ marginTop: 32 }}>
      ...
      >
        <div>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: 'var(--io-text-primary)' }}>
            OPC UA Server Certificates
          </h3>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--io-text-muted)' }}>
            Certificates presented by OPC UA servers during connection. Trust to allow;
            reject to block. Go to <strong>OPC Sources</strong> to manage per-source.
          </p>
        </div>
      </div>

      {isLoading && (
        <div style={{ fontSize: 13, color: 'var(--io-text-muted)' }}>Loading…</div>
      )}
      ...
      {!isLoading && (!certs || certs.length === 0) && (
        ...
        >
          No OPC server certificates on record. They will appear here once an OPC source connects.
        </div>
      )}

      {certs && certs.length > 0 && (
        <div style={{ border: '1px solid var(--io-border)', borderRadius: 8, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--io-surface-secondary)' }}>
                {['Source', 'Subject', 'Expires', 'Status', 'Actions'].map((h) => (
                  <th
                    key={h}
                    style={{
                      padding: '10px 14px',
              ...
              {certs.map((cert: OpcServerCert) => {
                ...
                return (
                  <tr
                    ...
                    style={{ borderBottom: '1px solid var(--io-border-subtle, var(--io-border))' }}
                  >
                    <td style={{ padding: '10px 14px', color: 'var(--io-text-primary)', fontWeight: 500 }}>
                      {cert.source_name ?? '—'}
                    </td>
                    <td style={{ padding: '10px 14px', fontFamily: 'monospace', fontSize: 11, color: 'var(--io-text-secondary)', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {cert.subject ?? cert.fingerprint_display.slice(0, 24) + '…'}
                    </td>
                    <td style={{ padding: '10px 14px', color: cert.expired ? '#EF4444' : 'var(--io-text-secondary)', whiteSpace: 'nowrap' }}>
                      {fmtDate(cert.not_after)}{cert.expired ? ' ⚠' : ''}
                    </td>
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ display: 'inline-block', padding: '2px 10px', borderRadius: 99, background: s.bg, color: s.color, fontSize: 11, fontWeight: 600 }}>
                        {s.label}
                      </span>
                    </td>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        {cert.status !== 'trusted' && (
                          <button
                            ...
                            style={{ padding: '4px 10px', fontSize: 12, borderRadius: 5, border: 'none', background: 'var(--io-accent)', color: '#fff', cursor: 'pointer', opacity: busy ? 0.6 : 1 }}
                          >
                            Trust
                          </button>
                        )}
                        {cert.status !== 'rejected' && (
                          <button
                            ...
                            style={{ padding: '4px 10px', fontSize: 12, borderRadius: 5, border: 'none', background: '#EF4444', color: '#fff', cursor: 'pointer', opacity: busy ? 0.6 : 1 }}
                          >
                            Reject
                          </button>
                        )}
                        <button
                          ...
                          style={{ padding: '4px 10px', fontSize: 12, borderRadius: 5, border: '1px solid var(--io-border)', background: 'transparent', color: 'var(--io-text-secondary)', cursor: 'pointer', opacity: busy ? 0.6 : 1 }}
                        >
                          Remove
                        </button>
                      </div>
