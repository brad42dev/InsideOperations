...
export function RestorePreviewModal({
  ...
  onClose: () => void
  onRestored: (result: { rows_restored: number; safety_snapshot_id: string | null }) => void
}) {
  ...
  useEffect(() => {
    snapshotsApi.restorePreview(snapshotId).then((res) => {
      if (res.success) {
        setPreview(res.data)
        setStep(2)
      } else {
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      if (res.success) {
        onRestored({ rows_restored: res.data.rows_restored, safety_snapshot_id: res.data.safety_snapshot_id })
      } else {
        showToast({ title: 'Restore failed', description: res.error.message, variant: 'error' })
        setIsRestoring(false)
      }
    } catch {
      showToast({ title: 'Restore failed', description: 'Network error', variant: 'error' })
      setIsRestoring(false)
    }
  }
      ...
      <div style={MODAL_BOX}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            {snapshotLabel && (
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: 'var(--io-text-muted)' }}>
                {snapshotLabel}
              </p>
            )}
          </div>
        ...
        {/* Step 1: Loading */}
        {step === 1 && (
          <div style={{ padding: 'var(--io-space-6)', textAlign: 'center', color: 'var(--io-text-muted)' }}>
            {loadError ? (
              <p style={{ color: 'var(--io-danger)' }}>Failed to load restore preview: {loadError}</p>
            ) : (
        ...
        {step === 2 && preview && (
          <>
            {/* Summary bar */}
            <div style={{ display: 'flex', gap: 'var(--io-space-4)', flexWrap: 'wrap', fontSize: '13px' }}>
              <span>
                <strong style={{ color: 'var(--io-text-primary)' }}>{preview.total_rows}</strong>
                <span style={{ color: 'var(--io-text-muted)', marginLeft: 4 }}>rows in snapshot</span>
              </span>
              {preview.conflicted_count > 0 && (
                <span style={{ color: 'var(--io-warning)', fontWeight: 600 }}>
                  &#9888; {preview.conflicted_count} conflicted (modified after snapshot)
                </span>
              )}
              <span style={{ color: 'var(--io-text-muted)', fontFamily: 'monospace', fontSize: '11px' }}>
                Snapshot taken: {new Date(preview.snapshot_created_at).toLocaleString()}
              </span>
            </div>

            {/* Mode selector */}
            <div style={{ display: 'flex', gap: 'var(--io-space-4)', alignItems: 'center', flexWrap: 'wrap' }}>
              <label style={{ display: 'flex', gap: 6, alignItems: 'center', cursor: 'pointer', fontSize: '13px' }}>
                <input
                  type="radio"
                  name="restore-mode"
                  value="all"
              ...
              {mode === 'selective' && (
                <div style={{ display: 'flex', gap: 'var(--io-space-2)', marginLeft: 'auto' }}>
                  <button style={{ ...BTN_SECONDARY, fontSize: '11px', padding: '3px 8px' }} onClick={selectAll}>
                    Select All
                  </button>
                  <button style={{ ...BTN_SECONDARY, fontSize: '11px', padding: '3px 8px' }} onClick={clearAll}>
                    Clear
                  </button>
                </div>
              )}
            ...
            {/* Diff table */}
            <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', border: '1px solid var(--io-border)', borderRadius: 6 }}>
            ...
            <label style={{ display: 'flex', gap: 8, alignItems: 'center', cursor: 'pointer', fontSize: '13px', color: 'var(--io-text-primary)' }}>
              <input
                type="checkbox"
        ...
        {step === 3 && preview && (
          <>
            <div style={{ padding: 'var(--io-space-4)', background: 'var(--io-warning-subtle)', borderRadius: 6, border: '1px solid var(--io-warning)', fontSize: '13px', color: 'var(--io-text-primary)', lineHeight: 1.6 }}>
              <strong>You are about to restore {rowCount} row{rowCount !== 1 ? 's' : ''}</strong> from snapshot
              {snapshotLabel ? ` "${snapshotLabel}"` : ` ${snapshotId.slice(0, 8)}…`}
              {createSafetySnapshot && (
                <span> A safety snapshot of the current state will be created first.</span>
              )}
              {preview.conflicted_count > 0 && mode === 'all' && (
                <div style={{ marginTop: 6, color: 'var(--io-warning)', fontWeight: 600 }}>
                  &#9888; {preview.conflicted_count} conflicted row{preview.conflicted_count !== 1 ? 's' : ''} will be overwritten.
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 'var(--io-space-3)', justifyContent: 'flex-end' }}>
              ...
              >
                {isRestoring ? 'Restoring…' : `Restore ${rowCount} Row${rowCount !== 1 ? 's' : ''}`}
              </button>
            </div>
          </>
        )}
...
function RestorePreviewDiffTable({
  ...
  if (rows.length === 0) {
    return (
      <p style={{ padding: 'var(--io-space-4)', color: 'var(--io-text-muted)', fontSize: '12px' }}>
  ...
  const allFields = Array.from(
    new Set(rows.flatMap((r) => Object.keys(r.snapshot_values).filter((k) => k !== 'id')))
        ...
        {rows.map((row) => {
          ...
          return (
            ...
            >
              ...
              <td style={{ ...TD, fontFamily: 'monospace', fontSize: '10px', color: 'var(--io-text-muted)' }}>
                {row.id.slice(0, 8)}…
              </td>
              <td style={TD}>
                {row.conflicted ? (
                  <span style={{ color: 'var(--io-warning)', fontWeight: 600, fontSize: '11px' }}>
                    &#9888; Conflict
                  </span>
                ) : hasChanges ? (
              ...
              {allFields.map((field) => {
                ...
                const snapStr = snapVal === null || snapVal === undefined ? '—' : String(snapVal).replace(/^"|"$/g, '')
                const currStr = currVal === null || currVal === undefined ? '—' : String(currVal).replace(/^"|"$/g, '')
                return (
                  <td key={field} style={{ ...TD, background: isChanged ? 'var(--io-danger-subtle)' : undefined }}>
                    {isChanged ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <span style={{ color: 'var(--io-danger)', textDecoration: 'line-through', fontSize: '10px' }}>
                          {currStr}
                        </span>
                        <span style={{ color: 'var(--io-success)', fontSize: '10px' }}>
                          {snapStr}
                        </span>
                      </div>
                    ) : (
                      <span style={{ color: 'var(--io-text-muted)', fontSize: '10px' }}>{snapStr}</span>
                    )}
                  </td>
                )
              })}
