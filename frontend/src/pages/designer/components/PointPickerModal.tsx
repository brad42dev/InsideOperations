...
function SearchIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
      <circle cx="6" cy="6" r="4.5" stroke="var(--io-text-muted)" strokeWidth="1.3" />
      <line x1="9.5" y1="9.5" x2="12.5" y2="12.5" stroke="var(--io-text-muted)" strokeWidth="1.3" strokeLinecap="round" />
    </svg>
  )
}
...
function TreeBranch({ nodes, selectedId, onSelect, depth = 0 }: {
  nodes: TreeNode[]
        ...
        if (node.point) {
          return (
            ...
            >
              ...
              {node.point.unit && (
                <span style={{ color: 'var(--io-text-muted)', fontSize: '11px', marginLeft: 'auto' }}>
                  {node.point.unit}
                </span>
              )}
            </div>
        ...
        return (
          <React.Fragment key={node.label}>
            ...
            >
              ...
              {node.label}
              {node.children && (
                <span style={{ color: 'var(--io-text-muted)', fontSize: '10px' }}>
                  ({node.children.length})
                </span>
              )}
            </div>
            {isOpen && node.children && (
              <TreeBranch nodes={node.children} selectedId={selectedId} onSelect={onSelect} depth={depth + 1} />
            )}
          </React.Fragment>
        )
      })}
...
export default function PointPickerModal({ open, onClose, onSelect }: PointPickerModalProps) {
  const [search, setSearch] = useState('')
  const [debouncedQ, setDebouncedQ] = useState('')
  const [selected, setSelected] = useState<PointMeta | null>(null)
  const [tab, setTab] = useState<ViewTab>('search')
  const [favorites, setFavorites] = useState<PointMeta[]>(() => loadJsonArray(FAVORITES_KEY))
  ...
  const handleConfirm = useCallback(() => {
    if (!selected) return
    // Update recent
    const newRecent = [selected, ...recent.filter((r) => r.id !== selected.id)].slice(0, 10)
  ...
  const renderList = (points: PointMeta[]) =>
    points.length === 0 ? (
      <div style={{ padding: '16px', color: 'var(--io-text-muted)', fontSize: '12px', textAlign: 'center' }}>
        No points found
      </div>
    ) : (
      points.map((p) => (
        <div
          ...
          onDoubleClick={() => { handleSelect(p); handleConfirm() }}
        >
          <button
          ...
          <span style={{ color: 'var(--io-text-muted)', fontSize: '11px', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {p.display_name || ''}
          </span>
          {p.unit && (
            <span style={{ color: 'var(--io-text-muted)', fontSize: '11px' }}>{p.unit}</span>
          )}
        </div>
      ))
    )
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={dialogStyle} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: '200px' }}>
          ...
          {tab === 'browse' && (
            <TreeBranch nodes={tree} selectedId={selected?.id ?? null} onSelect={handleSelect} />
          )}
        </div>
        ...
        {selected && (
          <div style={{
            ...
            <span><strong>{selected.tagname}</strong></span>
