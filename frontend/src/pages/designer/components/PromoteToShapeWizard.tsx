...
function analyzeGroup(groupNode: Group): GroupAnalysis {
  ...
  const allForBbox: SceneNode[] = [...geometry, ...displayElements, ...textBlocks, ...pipes]
  ...
  if (!isFinite(minX)) { minX = 0; minY = 0; maxX = 48; maxY = 48 }
...
function compositeSvg(analysis: GroupAnalysis, shapeSvgCache: (id: string) => string | null): string {
    ...
    switch (node.type) {
      ...
      case 'symbol_instance': {
        ...
        if (shapeSvg) {
          ...
          parts.push(`<g transform="${transform}"><rect width="24" height="24" fill="none" stroke="#999999" stroke-width="0.5"/></g>`)
        }
        break
      }
      case 'embedded_svg': {
      ...
      case 'image': {
        ...
        parts.push(`<g transform="${transform}"><rect width="${img.displayWidth}" height="${img.displayHeight}" fill="#cccccc" fill-opacity="0.3" stroke="#999999" stroke-width="0.5" stroke-dasharray="4 2"/></g>`)
        break
      }
      case 'annotation': {
        const ann = node as Annotation
        parts.push(`<g transform="${transform}"><rect width="${ann.width}" height="${ann.height}" fill="none" stroke="#999999" stroke-width="0.5" stroke-dasharray="4 2"/></g>`)
        break
      }
      default:
        break
...
function buildGroupConnectionPoints(analysis: GroupAnalysis): CPDraft[] {
  ...
  for (const pipe of analysis.pipes) {
    const startConnected = !!(pipe.startConnection && groupNodeIds.has(pipe.startConnection.instanceId))
    const endConnected = !!(pipe.endConnection && groupNodeIds.has(pipe.endConnection.instanceId))
...
export function PromoteToShapeWizard({
  ...
  const groupNode = (isGroupSource && selectedNodes.length === 1 && selectedNodes[0].type === 'group')
    ? (selectedNodes[0] as Group)
    : null

  // Analyze the group once on mount
  const [groupAnalysis] = useState<GroupAnalysis | null>(() => groupNode ? analyzeGroup(groupNode) : null)
  ...
  const [groupValueAnchors, setGroupValueAnchors] = useState<GroupValueAnchor[]>(() =>
  ...
  const [state, setState] = useState<WizardState>(() => ({
    ...
    connectionPoints: groupAnalysis ? buildGroupConnectionPoints(groupAnalysis) : [],
  ...
  function renderGroupStep0_SourceAnalysis() {
    ...
    if (!analysis) {
      return <p style={{ color: 'var(--io-alarm-high)', fontSize: 12, margin: 0 }}>Error: no group node provided.</p>
    }
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <p style={{ margin: 0, fontSize: 12, color: 'var(--io-text-secondary)', lineHeight: 1.5 }}>
          The group's children have been categorized. Review the summary below before proceeding.
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <SummaryCard icon="checkmark" count={analysis.geometryNodes.length}
            text="geometry element" detail="will become shape SVG" />
          <SummaryCard icon="checkmark" count={analysis.displayElements.length}
            text="data slot" detail="will become value anchors" />
          <SummaryCard icon="checkmark" count={analysis.textBlocks.length}
            text="text zone" detail="will become text zones" />
          <SummaryCard icon="checkmark" count={analysis.pipes.length}
            text="pipe" detail="dangling endpoints become connection points" />
          {widgetCount > 0 && (
            <SummaryCard icon="warning" count={widgetCount}
              text="widget" detail="will be EXCLUDED — widgets cannot be included in shapes" />
          )}
        </div>
        ...
        {hasWidgets && (
          <div style={{
            ...
            color: 'var(--io-text-primary)', lineHeight: 1.5,
          }}>
            <strong>Warning:</strong> {widgetCount} widget{widgetCount !== 1 ? 's' : ''} will be excluded from
            the shape. Only vector elements, display elements, and text blocks can be promoted.
            Proceed without the widget{widgetCount !== 1 ? 's' : ''}, or cancel and remove
            {widgetCount !== 1 ? ' them' : ' it'} from the group first.
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <button onClick={() => setWidgetWarningDismissed(true)} style={primaryBtnStyle}>
  ...
  function renderGroupStep1_SvgPreview() {
    ...
    const displaySvg = groupSvg || `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${groupViewBox}"><rect width="${groupAnalysis.bboxW}" height="${groupAnalysis.bboxH}" fill="none" stroke="#999" stroke-width="0.5"/></svg>`

    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <p style={{ margin: 0, fontSize: 12, color: 'var(--io-text-secondary)', lineHeight: 1.5 }}>
          The following SVG has been generated from the group's vector elements.
          Display elements, text blocks, and pipes are not included here — they become
          value anchors and connection points in the next steps.
        </p>
        <div style={{
          border: '1px solid var(--io-border)', borderRadius: 4, padding: 16,
          textAlign: 'center', background: 'var(--io-surface-elevated)',
        }}>
          <img
            src={svgToDataUri(displaySvg)}
            width={144} height={144}
            alt="Generated shape SVG preview"
            style={{ border: '1px dashed var(--io-accent)', display: 'inline-block' }}
          />
        </div>
        <p style={{ margin: 0, fontSize: 11, color: 'var(--io-text-muted)' }}>
          Bounding box: {groupAnalysis.bboxW.toFixed(1)} x {groupAnalysis.bboxH.toFixed(1)} units
          | {groupAnalysis.geometryNodes.length} geometry element{groupAnalysis.geometryNodes.length !== 1 ? 's' : ''}
        </p>
      </div>
    )
  }
  ...
  function renderGroupStep2_ValueAnchors() {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <p style={{ margin: 0, fontSize: 12, color: 'var(--io-text-secondary)', lineHeight: 1.5 }}>
          Value anchors have been auto-generated from the group's display elements.
          Drag the dots on the preview to adjust positions, rename labels, or delete unwanted anchors.
        </p>

        {groupValueAnchors.length === 0 && (
          <p style={{ margin: 0, fontSize: 12, color: 'var(--io-text-muted)' }}>
            No display elements found in this group — no value anchors will be created.
          </p>
        )}
        ...
        {groupValueAnchors.length > 0 && (
          <div style={{ display: 'flex', gap: 12 }}>
            <SvgPreviewWithOverlay
              ...
              width={PREVIEW_SIZE}
              height={PREVIEW_SIZE}
              onSvgRef={el => { overlaySvgRef.current = el }}
              ...
              overlayChildren={
                <>
                  {groupValueAnchors.map((anchor, i) => (
                    <g key={anchor.id}>
                      <circle
                        cx={anchor.x} cy={anchor.y} r={dotR}
                        fill="var(--io-success, #22c55e)" stroke="white" strokeWidth="0.5"
                        style={{ cursor: 'grab' }}
                        onMouseDown={e => onAnchorMouseDown(e, i)}
                      />
                      <text
                        x={anchor.x + dotR * 1.2} y={anchor.y - dotR * 0.5}
              ...
              {groupValueAnchors.map((anchor, i) => (
                <div key={anchor.id} style={{
                  ...
                  <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                    ...
                    />
                    <button
                      onClick={() => setGroupValueAnchors(prev => prev.filter((_, j) => j !== i))}
                      style={{ background: 'none', border: 'none', color: 'var(--io-text-muted)', cursor: 'pointer', fontSize: 14 }}
                    >x</button>
                  </div>
                  <select
            ...
            {state.tags.map((tag, i) => (
              <span key={i} style={{ background: 'var(--io-surface-elevated)', border: '1px solid var(--io-border)', borderRadius: 10, padding: '2px 8px', fontSize: 11, display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <button onClick={() => updateState({ tags: state.tags.filter((_, j) => j !== i) })}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--io-text-muted)', fontSize: 12, padding: 0, lineHeight: 1 }}>x</button>
              </span>
            ))}
          </div>
          <input
  ...
  function renderBoundaryAndSizing() {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <p style={{ margin: 0, fontSize: 12, color: 'var(--io-text-secondary)', lineHeight: 1.5 }}>
          The selected elements will be normalized to a <strong>48x48 viewBox</strong>,
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <svg
            ref={svgPreviewRef}
            width="144" height="144" viewBox={vb}
          ...
          >
            <rect x="0" y="0" width={previewW} height={previewH} fill="var(--io-accent)" fillOpacity="0.1" stroke="var(--io-accent)" strokeWidth="0.5" rx="1"/>
            {state.connectionPoints.map((cp, i) => (
              <g key={i}>
                <circle cx={cp.x} cy={cp.y} r={dotR} fill="var(--io-accent)" stroke="white" strokeWidth="0.5" />
                <text x={cp.x + dotR * 1.2} y={cp.y - dotR * 0.4} fontSize={labelFontSize} fill="var(--io-text-primary)">{cp.id}</text>
              </g>
            ))}
          </svg>
          <div style={{ flex: 1, overflow: 'auto', maxHeight: 160 }}>
            {state.connectionPoints.length === 0 && (
              <p style={{ fontSize: 11, color: 'var(--io-text-muted)', margin: 0 }}>Click the shape to add connection points.</p>
            )}
            {state.connectionPoints.map((cp, i) => (
              <div key={i} style={{ display: 'flex', gap: 4, marginBottom: 6, alignItems: 'center', fontSize: 11 }}>
                ...
                <button onClick={() => updateState({ connectionPoints: state.connectionPoints.filter((_, j) => j !== i) })}
                  style={{ background: 'none', border: 'none', color: 'var(--io-text-muted)', cursor: 'pointer', fontSize: 14, padding: '0 2px' }}>x</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          ...
          <p style={{ margin: '0 0 8px', fontSize: 11, color: 'var(--io-text-muted)' }}>
            Enter element IDs from your SVG. If left empty, the shape is purely structural.
          </p>
          {state.statefulElements.map((el, i) => (
            <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 4 }}>
              ...
              <button onClick={() => updateState({ statefulElements: state.statefulElements.filter((_, j) => j !== i) })}
                style={{ background: 'none', border: 'none', color: 'var(--io-text-muted)', cursor: 'pointer', fontSize: 14 }}>x</button>
            </div>
          ))}
          <button onClick={() => updateState({ statefulElements: [...state.statefulElements, ''] })} style={addBtnStyle}>
            + Add Element ID
          </button>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          >
            ...
            {state.textZones.map((tz, i) => (
              <g key={i}>
                <circle cx={tz.x ?? 24} cy={tz.y ?? 24} r={dotR} fill="var(--io-warning)" />
                <text x={(tz.x ?? 24) + dotR * 1.2} y={(tz.y ?? 24) - dotR * 0.4} fontSize={labelFontSize} fill="var(--io-text-primary)">{tz.id}</text>
              </g>
            ))}
          </svg>
  ...
  function renderValueDisplayAnchors() {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <p style={{ margin: 0, fontSize: 12, color: 'var(--io-text-secondary)', lineHeight: 1.5 }}>
          Click on the shape preview to place value display anchor positions (Quick Bind positions).
        </p>
        <div style={{ display: 'flex', gap: 12 }}>
          <svg
  ...
  function renderPreviewAndSave() {
    ...
    const effectiveAnchorCount = isGroupSource ? groupValueAnchors.length : state.valueAnchors.length
    const effectiveTextZoneCount = isGroupSource ? groupTextZonesState.length : state.textZones.length
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12 }}>
          ...
          <SummaryRow label="Orientations" value={state.orientations.map(o => o + ' deg').join(', ')} />
          <SummaryRow label="Connection Points" value={String(state.connectionPoints.length)} />
          <SummaryRow label="Text Zones" value={String(effectiveTextZoneCount)} />
          <SummaryRow label="Value Anchors" value={String(effectiveAnchorCount)} />
        ...
        <div style={{ marginTop: 8, padding: 12, background: 'var(--io-surface-elevated)', borderRadius: 4, border: '1px solid var(--io-border)', display: 'flex', justifyContent: 'center' }}>
          <img
            src={svgToDataUri(isGroupSource && groupSvg ? groupSvg : previewSvg)}
        ...
        {isGroupSource && groupNode && (
          <label style={{
            ...
            padding: '8px 10px', background: 'var(--io-surface-elevated)',
            borderRadius: 4, border: '1px solid var(--io-border)',
          }}>
            <input
              type="checkbox"
  ...
  const stepRenderers = isGroupSource ? groupStepRenderers : standardStepRenderers
  ...
  function canAdvance(): boolean {
    if (isGroupSource) {
      ...
      if (step === 3) return state.shapeIdPrefix.trim().length > 0 && state.displayName.trim().length > 0
      return true
    }
  ...
  async function handleFinish() {
    ...
    try {
      ...
      const svgViewBox = isGroupSource && groupAnalysis
        ? `0 0 ${groupAnalysis.bboxW.toFixed(2)} ${groupAnalysis.bboxH.toFixed(2)}`
        : VIEWBOX

      const svgWidth = isGroupSource && groupAnalysis ? groupAnalysis.bboxW : 48
      const svgHeight = isGroupSource && groupAnalysis ? groupAnalysis.bboxH : 48

      const svgData = (isGroupSource && groupSvg)
        ? groupSvg
      ...
      if (isGroupSource && replaceGroupWithShape && sourceNodeId && onReplaceGroup) {
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--io-border)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--io-text-primary)' }}>
              Promote to Shape
            </span>
            {isGroupSource && (
              <span style={{
                fontSize: 11, fontWeight: 400, color: 'var(--io-text-muted)',
...
function SummaryCard({
  ...
  return (
    <div style={{
      ...
      padding: '8px 10px',
      background: icon === 'warning' ? 'rgba(234,179,8,0.08)' : 'var(--io-surface-elevated)',
      border: `1px solid ${icon === 'warning' ? 'var(--io-warning)' : 'var(--io-border)'}`,
      borderRadius: 4, fontSize: 12,
    }}>
      <span style={{
        color: icon === 'warning' ? 'var(--io-warning)' : 'var(--io-success, #22c55e)',
        fontWeight: 600, flexShrink: 0, marginTop: 1,
      }}>
        {icon === 'checkmark' ? '\u2713' : '\u26A0'}
      </span>
      <div>
        <span style={{ color: 'var(--io-text-primary)', fontWeight: 500 }}>{count} {text}{plural}</span>
        <span style={{ color: 'var(--io-text-muted)', marginLeft: 6 }}>{detail}</span>
      </div>
    </div>
  )
}
