    ...
    >
      ...
      <button
        ...
        style={{
          ...
          cursor: "pointer",
          padding: "2px",
          color: layer.locked ? "var(--io-warning)" : "var(--io-text-muted)",
          fontSize: "11px",
        }}
      >
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
          {layer.locked ? (
            <>
              ...
              />
              <path
                d="M4 5V3.5C4 2.67157 4.67157 2 5.5 2H6.5C7.32843 2 8 2.67157 8 3.5V5"
                stroke="currentColor"
                strokeWidth="1.2"
              />
            </>
            ...
            <>
              ...
              />
              <path
                d="M4 5V3.5C4 2.67157 4.67157 2 5.5 2H6.5C7.32843 2 8 2.67157 8 3.5V4"
                stroke="currentColor"
                strokeWidth="1.2"
                opacity="0.4"
              />
        ...
        <>
          ...
          >
            ...
            <button
              ...
              style={{ ...ctxItemStyle, color: "var(--io-danger)" }}
            >
              Delete
            </button>
          </div>
...
function GlobalPanel({
  ...
  graphicName: string;
  onGraphicNameChange: (n: string) => void;
  canvasWidth: number;
  canvasHeight: number;
  onCanvasSizeChange: (w: number, h: number) => void;
  layers: DesignerLayer[];
  onLayersChange: (l: DesignerLayer[]) => void;
}) {
  ...
  const addLayer = () => {
    ...
    onLayersChange([
      ...layers,
      { id, name: `Layer ${num}`, visible: true, locked: false },
    ]);
  };
      ...
      >
        <div>
          ...
          <input
            ...
            value={canvasWidth}
            onChange={(e) =>
              onCanvasSizeChange(Number(e.target.value), canvasHeight)
            }
            style={inputStyle}
          />
        </div>
        <div>
          ...
          <input
            ...
            value={canvasHeight}
            onChange={(e) =>
              onCanvasSizeChange(canvasWidth, Number(e.target.value))
            }
            style={inputStyle}
          />
        </div>
...
function ShapePanel({
  ...
  onElementTransform?: (
    id: string,
    t: { x?: number; y?: number; w?: number; h?: number; rotation?: number },
  ) => void;
}) {
  ...
  const [bindingType, setBindingType] = useState<
    "color_map" | "rotation" | "visibility" | "display_value"
  >("display_value");
  ...
  const handlePointSelect = (tag: string, pointId: string) => {
    ...
    const updated: GraphicBindings = {
      ...
      [elementId]: {
        point_id: pointId,
        attribute:
          bindingType === "color_map"
            ? "fill"
            : bindingType === "display_value"
              ? "text"
          ...
          >
            ...
            >
              <svg width="12" height="12" viewBox="0 0 12 12">
                <circle cx="6" cy="6" r="4" fill="var(--io-success)" />
              </svg>
              <span
                style={{ color: "var(--io-text-primary)", fontWeight: 500 }}
              >
                Bound: {String(existing.point_id ?? "").slice(0, 8)}...
              </span>
            </div>
            <div style={{ fontSize: "11px", color: "var(--io-text-muted)" }}>
              {String(existing.attribute ?? "")} /{" "}
              {String(
                (existing.mapping as { type?: string } | undefined)?.type ?? "",
              )}
            </div>
            <button
              onClick={handleUnbind}
              style={{ ...btnDangerStyle, marginTop: "6px" }}
            >
              Unbind
            </button>
          </div>
...
function MultiSelectPanel({
  ...
  onDelete?: () => void;
  onAlign?: (
    a: "left" | "center" | "right" | "top" | "middle" | "bottom",
  ) => void;
  ...
  return (
    <div style={scrollAreaStyle}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
        <button onClick={onGroup} style={btnAccentStyle}>
          Group (Ctrl+G)
        </button>
        <button onClick={onDelete} style={btnDangerStyle}>
          Delete
        </button>
      </div>
    </div>
