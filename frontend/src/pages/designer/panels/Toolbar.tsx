...
function IconPhone() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <rect x="4" y="1.5" width="8" height="13" rx="1.5" stroke="currentColor" strokeWidth="1.3" fill="none"/>
      <line x1="6.5" y1="12.5" x2="9.5" y2="12.5" stroke="currentColor" strokeWidth="1" strokeLinecap="round"/>
    </svg>
  )
}
function IconFile() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M4 2H10L13 5V14H4V2Z" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinejoin="round"/>
...
function IconSave() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M3 2H11L14 5V13C14 13.5523 13.5523 14 13 14H3C2.44772 14 2 13.5523 2 13V3C2 2.44772 2.44772 2 3 2Z" stroke="currentColor" strokeWidth="1.3" fill="none"/>
      <rect x="5" y="9" width="6" height="5" rx="0.5" stroke="currentColor" strokeWidth="1" fill="none"/>
      <rect x="5" y="2" width="5" height="3" rx="0.5" stroke="currentColor" strokeWidth="1" fill="none"/>
    </svg>
  )
}
function IconPublish() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M8 2L8 10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
  ...
  return (
    <button
      ...
      style={{
        ...
        color: disabled ? 'var(--io-text-muted)' : active ? 'var(--io-accent)' : 'var(--io-text-secondary)',
...
function ToggleBtn({ icon, title, active, label, onClick }: {
  icon: React.ReactNode; title: string; active: boolean; label?: string; onClick: () => void
}) {
  return (
    <button
      title={title}
...
function LabelBtn({ icon, label, title, onClick, accent, disabled }: {
  icon?: React.ReactNode; label: string; title: string; onClick: () => void; accent?: boolean; disabled?: boolean
}) {
  return (
    <button
      title={title}
...
function ZoomPopover({ zoom, onZoomSet, onClose }: { zoom: number; onZoomSet: (z: number) => void; onClose: () => void }) {
  ...
  return (
    ...
    >
      {ZOOM_PRESETS.map((p) => (
        <button
          key={p}
          onClick={() => { onZoomSet(p / 100); onClose() }}
          style={{
            display: 'block',
            width: '100%',
            padding: '4px 8px',
            fontSize: '12px',
            border: 'none',
            background: Math.round(zoom * 100) === p ? 'var(--io-accent-subtle)' : 'transparent',
            color: Math.round(zoom * 100) === p ? 'var(--io-accent)' : 'var(--io-text-primary)',
            cursor: 'pointer',
            textAlign: 'left',
            borderRadius: '2px',
          }}
      ...
      <div style={{ borderTop: '1px solid var(--io-border-subtle)', margin: '4px 0' }} />
      <div style={{ display: 'flex', gap: '4px', padding: '0 4px' }}>
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              ...
              if (v >= 10 && v <= 800) { onZoomSet(v / 100); onClose() }
            }
          }}
          style={{
            flex: 1,
        ...
        />
        <span style={{ fontSize: '12px', color: 'var(--io-text-muted)', lineHeight: '26px' }}>%</span>
      </div>
    </div>
  )
}
...
// ---- File menu dropdown ----
function FileMenu({ onImport, onExportSvg, onVersionHistory, onProperties, onClose }: {
  onImport: () => void; onExportSvg: () => void; onVersionHistory: () => void; onProperties: () => void; onClose: () => void
}) {
  const ref = useRef<HTMLDivElement>(null)
  useEffect(() => {
    const handler = (e: MouseEvent) => {
  ...
  return (
    ...
    >
      {items.map((item, i) =>
        item.label === 'divider' ? (
          <div key={i} style={{ borderTop: '1px solid var(--io-border-subtle)', margin: '4px 0' }} />
        ) : (
          <button
            key={item.label}
...
export default function Toolbar(props: ToolbarProps) {
  const {
    ...
    onZoomIn, onZoomOut, onZoomFit, onZoomSet,
    zoom, mode,
    canUndo, canRedo, onUndo, onRedo,
    testMode, onTestModeToggle,
    onValidateBindings,
    onPhonePreview,
    onFileImport, onFileExportSvg, onVersionHistory, onProperties,
  ...
  return (
    <div style={toolbarStyle}>
      ...
      {/* Canvas controls */}
      <ToggleBtn icon={<IconSnap />} title="Snap to grid" active={snapEnabled} onClick={onSnapToggle} />
      <ToggleBtn icon={<IconGrid />} title="Show grid" active={gridEnabled} onClick={onGridToggle} />

      <div style={separatorStyle} />

      {/* Undo/Redo */}
      <ToolBtn icon={<IconUndo />} title="Undo (Ctrl+Z)" onClick={onUndo} disabled={!canUndo} />
      <ToolBtn icon={<IconRedo />} title="Redo (Ctrl+Shift+Z)" onClick={onRedo} disabled={!canRedo} />
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '2px' }}>
        ...
        {showZoom && (
          <ZoomPopover zoom={zoom} onZoomSet={onZoomSet} onClose={() => setShowZoom(false)} />
        )}
      </div>
      <ToolBtn icon={<IconFit />} title="Fit to screen (Ctrl+0)" onClick={onZoomFit} />
      <ToolBtn icon={<IconZoomIn />} title="Zoom in (Ctrl+=)" onClick={onZoomIn} />
      <ToolBtn icon={<IconZoomOut />} title="Zoom out (Ctrl+-)" onClick={onZoomOut} />
      ...
      <button
        ...
        title={testMode ? 'Stop test mode' : 'Start test mode — connect to live data'}
        style={{
          height: '30px',
          display: 'flex',
          alignItems: 'center',
          ...
          padding: '0 10px',
          cursor: 'pointer',
          border: testMode ? '1px solid var(--io-success)' : '1px solid var(--io-border)',
          borderRadius: 'var(--io-radius)',
      ...
      {mode !== 'report' && (
        <>
          <div style={separatorStyle} />
          <ToolBtn icon={<IconPhone />} title="Phone preview" onClick={onPhonePreview} />
        </>
      )}
