...
function ConnectedDot({ connected }: { connected: boolean }) {
  return (
    <svg width="8" height="8" viewBox="0 0 8 8">
      <circle cx="4" cy="4" r="3.5" fill={connected ? 'var(--io-success)' : 'var(--io-danger)'} />
    </svg>
  )
}

function formatAutoSave(status: AutoSaveStatus, age: number): { text: string; color: string } {
  switch (status) {
    case 'saving':
    ...
    case 'saved': {
      ...
      if (age < 60) return { text: `Auto-saved ${age}s ago`, color: 'var(--io-text-muted)' }
...
export default function StatusBar({
  ...
  return (
    <div style={barStyle}>
      ...
      <div style={segmentStyle}>
        {testMode ? (
          <span style={{ color: 'var(--io-success)', fontWeight: 600 }}>Test Mode</span>
        ) : (
          <span>Edit</span>
        )}
      </div>
      ...
      <div style={{ ...segmentStyle, color: autoSave.color }}>
        {autoSaveStatus === 'failed' && (
          <svg width="12" height="12" viewBox="0 0 12 12" style={{ flexShrink: 0 }}>
            <path d="M6 1L11 10H1L6 1Z" fill="none" stroke="currentColor" strokeWidth="1.2" />
            <line x1="6" y1="4" x2="6" y2="7" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
