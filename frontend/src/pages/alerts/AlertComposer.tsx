...
export default function AlertComposer() {
  ...
  const SEVERITIES = ALL_SEVERITIES.filter(
    (s) => s !== "emergency" || canSendEmergency,
  );
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: 600 }}>
        <div style={{ display: "grid", gap: 20 }}>
          ...
          {templates.filter(
            (t) =>
              t.severity === "critical" ||
              (t.severity === "emergency" && canSendEmergency),
          ).length > 0 && (
            <div>
              <div
                style={{
                  ...
                  .map((t) => (
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 700,
                        color:
                          t.severity === "emergency" ? "#ef4444" : "#f59e0b",
                        transition: "background 0.15s",
                      }}
                    >
                      {t.severity === "emergency" ? "🚨 " : "⚠️ "}
                      {t.name}
                    </button>
                  ))}
              </div>
            ...
            <div style={{ display: "grid", gap: 6 }}>
              ...
              {title && (
                ...
                >
                  ...
                  <span
                    style={{ fontSize: 13, color: "var(--io-text-primary)" }}
                  >
                    {title}
                  </span>
                </div>
          ...
          {(sendMutation.isError || sendEmergencyMutation.isError) && (
            ...
            >
              {(sendMutation.error ?? sendEmergencyMutation.error) instanceof
              Error
                ? (sendMutation.error ?? (sendEmergencyMutation.error as Error))
                    .message
            ...
            {(() => {
              ...
              return (
                <button
                  type="submit"
                  disabled={
                    activeMutation.isPending || !title.trim() || !body.trim()
                  }
                  style={{
                    padding: "8px 24px",
                    borderRadius: 6,
