...
import { notificationsApi, type GroupType, type CreateGroupPayload } from '../../api/notifications'
...
const GROUP_TYPES: GroupType[] = ['static', 'role_based', 'on_shift', 'on_site', 'all_users']
...
export default function AlertGroups() {
  ...
  function handleCreate(e: React.FormEvent) {
    ...
    setFormError('')
    createMutation.mutate({ name: name.trim(), description: description.trim() || undefined, group_type: groupType })
  }
  ...
  return (
    <div style={{ padding: 'var(--io-space-6)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h2 style={{ color: 'var(--io-text-primary)', margin: 0, fontSize: 20, fontWeight: 600 }}>Alert Groups</h2>
      ...
      {showCreate && (
        <div style={{
          ...
          <div style={{ color: 'var(--io-text-primary)', fontWeight: 600, fontSize: 15, marginBottom: 16 }}>Create Group</div>
          <form onSubmit={handleCreate}>
            <div style={{ display: 'grid', gap: 14 }}>
              ...
              {formError && (
                <div style={{ color: '#ef4444', fontSize: 13 }}>{formError}</div>
              )}
              <div>
                <button
                  type="submit"
      ...
      <div style={{ border: '1px solid var(--io-border)', borderRadius: 8, background: 'var(--io-surface)', overflow: 'hidden' }}>
        ...
        {isError && (
          <div style={{ padding: 32, textAlign: 'center', color: '#ef4444' }}>Failed to load groups.</div>
        )}
        {groups && groups.length === 0 && (
          <div style={{ padding: 32, textAlign: 'center', color: 'var(--io-text-muted)' }}>No groups found. Create one to get started.</div>
        )}
        {groups?.map(group => (
          <div
            key={group.id}
          ...
          >
            <div style={{ color: 'var(--io-text-primary)', fontWeight: 500, fontSize: 14 }}>{group.name}</div>
            <div style={{ color: 'var(--io-text-secondary)', fontSize: 13 }}>{group.description ?? '—'}</div>
            <div>
              <span style={{
