...
function MarkModal({
  ...
  const [selectedStatus, setSelectedStatus] = useState<MusterStatus>(
    mark.status,
  );
  ...
  return (
    ...
    >
      ...
      >
        <h3
          style={{ margin: "0 0 4px", fontSize: 16, color: "var(--io-text)" }}
        >
          Mark Status
        </h3>
        <p
...
function PersonCard({
  ...
  const c =
    STATUS_COLORS[mark.status as MusterStatus] ?? STATUS_COLORS.unaccounted;
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ fontSize: 14, fontWeight: 600, color: "var(--io-text)" }}
        >
          {mark.display_name ?? mark.user_id}
        </span>
...
export default function MusterDashboard() {
  ...
  const summary = musterData?.summary ?? {
    total: 0,
    accounted: 0,
    off_site: 0,
    unaccounted: 0,
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      ...
      {message && (
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: 12, color: "var(--io-text-muted)" }}>
              Sent {new Date(message.sent_at).toLocaleString()} by{" "}
              {message.sent_by_name ?? message.sent_by}
            </span>
          </div>
          <h2
            style={{ margin: "0 0 4px", fontSize: 17, color: "var(--io-text)" }}
          >
            {message.title}
          </h2>
          <p
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <button
          ...
          style={filterBtnStyle(filter === "all", "var(--io-accent, #4a9eff)")}
        >
          All ({allMarks.length})
        </button>
        <button
