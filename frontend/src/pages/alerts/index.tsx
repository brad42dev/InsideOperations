...
function ConfirmDialog({
  ...
  return (
    ...
    >
      ...
      >
        <h3 style={{ margin: "0 0 8px", color: "#ef4444", fontSize: 16 }}>
          {title}
        </h3>
        <p
          style={{
...
function SendAlertPanel() {
  ...
  const groups: NotificationGroup[] = (() => {
    ...
    return Array.isArray(paged?.data)
      ? (paged.data as NotificationGroup[])
      : [];
  })();
  // Enabled channels from config — fall back to websocket-only if API fails or is loading
  const enabledChannels: NotificationChannel[] =
    enabledChannelsResult?.success &&
    enabledChannelsResult.data &&
    enabledChannelsResult.data.length > 0
      ? enabledChannelsResult.data
      : FALLBACK_CHANNELS;
  ...
  const previewTitle = selectedTemplate
    ? substituteVars(selectedTemplate.title_template)
    : title;
  const previewBody = selectedTemplate
    ? substituteVars(selectedTemplate.body_template)
    : body;
  ...
  const sendEmergencyMutation = useMutation({
    mutationFn: (payload: SendNotificationPayload) =>
      notificationsApi.sendEmergency(payload),
    onSuccess: (result) => {
      if (result.success) {
        qc.invalidateQueries({ queryKey: ["notification-messages"] });
        qc.invalidateQueries({ queryKey: ["notification-active"] });
  ...
  const hasUnfilledRequired = selectedTemplate
    ? (selectedTemplate.variables ?? []).some(
        (v: TemplateVariable) => v.required && !variableValues[v.name]?.trim(),
      )
    : false;
  ...
  const doSend = () => {
    ...
    const payload: SendNotificationPayload = {
      ...
      channels,
      group_id:
        recipientMode === "group" ? selectedGroupId || undefined : undefined,
      variables:
  ...
  const handleTemplateSelect = (id: string) => {
    ...
    if (tpl) {
      ...
      const filteredChannels = (tpl.channels ?? []).filter((ch) =>
        enabledChannels.includes(ch),
      );
      setChannels(
        filteredChannels.length > 0 ? filteredChannels : ["websocket"],
      );
      const initVars: Record<string, string> = {};
      (tpl.variables ?? []).forEach((v) => {
        initVars[v.name] = v.default_value ?? "";
      });
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {(() => {
          const activeMutation =
            severity === "emergency" ? sendEmergencyMutation : sendMutation;
          return (
            <>
              <button
                onClick={handleSend}
              ...
              >
                {activeMutation.isPending
                  ? "Sending…"
                  : `Send ${severity.charAt(0).toUpperCase() + severity.slice(1)} Alert`}
              </button>

              {activeMutation.isSuccess && activeMutation.data?.success && (
                <p style={{ color: "#22c55e", fontSize: 13, margin: 0 }}>
                  Alert sent to {activeMutation.data.data.recipient_count}{" "}
                  recipient(s).
                </p>
              )}
              {activeMutation.isSuccess && !activeMutation.data?.success && (
                <p style={{ color: "#ef4444", fontSize: 13, margin: 0 }}>
                  {(activeMutation.data as { error?: { message?: string } })
      ...
      >
        ...
        <h4
          style={{ margin: "8px 0 4px", fontSize: 15, color: "var(--io-text)" }}
        >
          {previewTitle || (
            <span style={{ color: "var(--io-text-muted)" }}>
              Alert title will appear here
            </span>
          )}
        </h4>
        <p
...
function ActiveAlertsPanel() {
  ...
  const messages: NotificationMessage[] =
    activeResult?.success && activeResult.data ? activeResult.data : [];
  ...
  if (messages.length === 0) {
    return (
      ...
      >
        ...
        <p style={{ fontSize: 14 }}>
          No active emergency or critical alerts in the last 24 hours.
        </p>
      </div>
    );
  }
  ...
  const cancelTarget = cancelConfirmId
    ? messages.find((m) => m.id === cancelConfirmId)
    : null;
      ...
      {cancelConfirmId && cancelTarget && (
        ...
        >
          ...
          >
            ...
            >
              This will stop any escalation and remove the alert from the active
...
function HistoryPanel() {
  ...
  const { data: result, isLoading } = useQuery({
    queryKey: [
      ...
      { page, severity: severity || undefined },
    ],
    queryFn: () =>
  ...
  const pagedData =
    result?.success && result.data
      ? (result.data as unknown as HistoryData)
      : null;
  const messages: NotificationMessage[] = Array.isArray(pagedData?.data)
    ? pagedData.data
    : [];
    ...
    async (format: ExportFormat) => {
      ...
      try {
        ...
        if (exportResult.type === "download") {
          exportsApi.triggerDownload(
            exportResult.blob,
            historyExportFilename(format),
          );
        } else if (exportResult.type === "queued") {
          setExportNotice(
            `Export queued (job ${exportResult.job.job_id}). You will be notified when it is ready.`,
          );
        }
      } catch (err) {
    ...
    <div>
      ...
      >
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <label style={{ fontSize: 13, color: "var(--io-text-muted)" }}>
            Severity:
          </label>
          <select
            value={severity}
...
function TemplatesPanel() {
  ...
  const [testSendPendingId, setTestSendPendingId] = useState<string | null>(
    null,
  );
  ...
  const createMutation = useMutation({
    mutationFn: (payload: CreateTemplatePayload) =>
      notificationsApi.createTemplate(payload),
    onSuccess: (res) => {
      if (res.success) {
        ...
        >
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setCreateForm((f) => ({ ...f, title_template: e.target.value }))
              }
            ...
            <textarea
              ...
              onChange={(e) =>
                setCreateForm((f) => ({ ...f, body_template: e.target.value }))
              }
              rows={3}
              style={{
                ...inputStyle,
            ...
            {varDefs.map((vd, idx) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setVarDefs((prev) =>
                      prev.map((x, i) =>
                        i === idx
                          ? { ...x, default_value: e.target.value || undefined }
                          : x,
                      ),
                    )
                  }
                ...
                <button
                  type="button"
                  onClick={() =>
                    setVarDefs((prev) => prev.filter((_, i) => i !== idx))
                  }
                  style={{
                    padding: "4px 8px",
                    borderRadius: 4,
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => {
                if (
                  createForm.name &&
                  createForm.title_template &&
                  createForm.body_template
                ) {
                  createMutation.mutate({
                    ...(createForm as CreateTemplatePayload),
                    variables: varDefs.filter((v) => v.name.trim()),
                  });
                }
              }}
        ...
        >
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setEditForm((f) => ({ ...f, title_template: e.target.value }))
              }
              style={{ ...inputStyle, gridColumn: "1 / -1" }}
            />
            <textarea
              ...
              onChange={(e) =>
                setEditForm((f) => ({ ...f, body_template: e.target.value }))
              }
              rows={3}
              style={{
                ...inputStyle,
            ...
            {editVarDefs.map((vd, idx) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() =>
                    setEditVarDefs((prev) => prev.filter((_, i) => i !== idx))
                  }
                  style={{
                    padding: "4px 8px",
                    borderRadius: 4,
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => {
                if (
                  editForm.name &&
                  editForm.title_template &&
                  editForm.body_template
                ) {
                  updateMutation.mutate({
                    id: editTemplateId,
                    payload: {
                      ...(editForm as UpdateTemplatePayload),
                      variables: editVarDefs.filter((v) => v.name.trim()),
                    },
                  });
                }
              }}
...
function GroupsPanel() {
  ...
  const deleteGroupTarget = deleteGroupId
    ? groups.find((g) => g.id === deleteGroupId)
    : null;
  const editGroupTarget = editGroupId
    ? groups.find((g) => g.id === editGroupId)
    : null;
  const viewMembersTarget = viewMembersGroupId
    ? groups.find((g) => g.id === viewMembersGroupId)
    : null;

  const createMutation = useMutation({
    mutationFn: (payload: CreateGroupPayload) =>
      notificationsApi.createGroup(payload),
    onSuccess: (res) => {
      if (res.success) {
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  color: "#fff",
                  fontWeight: 700,
                  cursor: updateGroupMutation.isPending
                    ? "not-allowed"
                    : "pointer",
      ...
      {viewMembersTarget && (
        ...
        >
          ...
          >
            ...
            {viewMembersTarget.members &&
            viewMembersTarget.members.length > 0 ? (
    ...
    <div>
      ...
      >
        <button
          ...
          style={tabStyle(subTab === "templates")}
        >
          Templates
        </button>
        <button
          onClick={() => setSubTab("groups")}
          style={tabStyle(subTab === "groups")}
        >
          Groups
        </button>
      </div>
