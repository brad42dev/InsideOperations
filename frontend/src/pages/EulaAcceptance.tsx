...
function parseBlocks(markdown: string): Block[] {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('# ')) {
      blocks.push({ type: 'h1', text: line.slice(2) }); i++; continue
    }
    if (line.startsWith('## ')) {
      blocks.push({ type: 'h2', text: line.slice(3) }); i++; continue
    }
    if (/^-{3,}$/.test(line.trim())) {
    ...
    if (line.startsWith('>')) {
      ...
      while (i < lines.length && lines[i].startsWith('>')) {
        bqLines.push(lines[i].slice(1).trimStart()); i++
      }
      ...
      {body.map((block, idx) => {
        switch (block.type) {
          ...
          case 'h2':
            return (
              <h3 key={idx} style={{
                margin: '24px 0 8px', fontSize: '13px', fontWeight: 700,
                color: 'var(--io-text-primary)', textTransform: 'uppercase',
                letterSpacing: '0.06em', borderBottom: '1px solid var(--io-border-subtle)',
                paddingBottom: '6px',
              }}>
          ...
          case 'hr':
            return <hr key={idx} style={{ border: 'none', borderTop: '1px solid var(--io-border)', margin: '20px 0' }} />

          case 'blockquote': {
            const bqItems = block.lines.map(l => ({ isBullet: l.startsWith('- '), text: l.startsWith('- ') ? l.slice(2) : l }))
            return (
              <div key={idx} style={{
                margin: '16px 0', padding: '14px 18px',
                background: 'var(--io-accent-subtle)', borderLeft: '3px solid var(--io-accent)',
                borderRadius: '0 var(--io-radius) var(--io-radius) 0',
                color: 'var(--io-text-secondary)', fontSize: '13px',
              }}>
                {bqItems.map((item, j) => {
                  ...
                  return <div key={j} style={{ marginBottom: '2px' }}>{parseInline(item.text)}</div>
                })}
              </div>
            )
          }
          ...
          case 'ul':
            return (
              <ul key={idx} style={{ margin: '8px 0', paddingLeft: 0, listStyle: 'none' }}>
          ...
          case 'ol':
            return (
              <ol key={idx} style={{ margin: '8px 0', paddingLeft: 0, listStyle: 'none', counterReset: 'eula-ol' }}>
                {block.items.map((item, j) => (
                  <li key={j} style={{ display: 'flex', gap: '10px', marginBottom: '5px' }}>
                    <span style={{ color: 'var(--io-accent)', fontWeight: 600, flexShrink: 0, minWidth: '16px' }}>{j + 1}.</span>
                    <span>{parseInline(item)}</span>
                  </li>
                ))}
              </ol>
...
function EulaPanel({ item, stepLabel, onAccepted, onDecline, isPending, error }: EulaPanelProps) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px' }}>
          <h2 style={{ margin: 0, color: 'var(--io-text-primary)', fontSize: '20px', fontWeight: 700, letterSpacing: '-0.01em' }}>
            Inside/Operations
          </h2>
          <span style={{ color: 'var(--io-text-muted)', fontSize: '14px', fontWeight: 400 }}>
            {isInstaller ? 'Software License Agreement' : 'End User License Agreement'}
          </span>
          <span style={{
            marginLeft: 'auto', padding: '2px 8px',
            background: 'var(--io-surface-sunken)', border: '1px solid var(--io-border)',
            borderRadius: 'var(--io-radius-sm)', color: 'var(--io-text-muted)',
            fontSize: '11px', fontFamily: 'var(--io-font-mono)', flexShrink: 0,
          }}>
            v{item.version}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
          <span style={{
            ...
            background: isInstaller ? 'var(--io-accent-subtle)' : 'var(--io-surface-sunken)',
            color: isInstaller ? 'var(--io-accent)' : 'var(--io-text-muted)',
            border: `1px solid ${isInstaller ? 'var(--io-accent)' : 'var(--io-border)'}`,
          }}>
            {isInstaller ? 'Organizational License' : 'Individual Use Agreement'}
          </span>
          <span style={{ fontSize: '12px', color: 'var(--io-text-muted)' }}>{stepLabel}</span>
        </div>
      ...
      <div style={{
        ...
        {!scrolledToBottom && (
          <div style={{
            ...
            padding: '8px 12px', background: 'var(--io-surface-sunken)',
            border: '1px solid var(--io-border)', borderRadius: 'var(--io-radius)',
            fontSize: '12px', color: 'var(--io-text-muted)',
          }}>
        ...
        {isInstaller && scrolledToBottom && (
          <label style={{
            ...
            <input
              ...
              style={{ marginTop: '2px', flexShrink: 0, accentColor: 'var(--io-accent)' }}
            />
            <span style={{ fontSize: '13px', color: 'var(--io-text-secondary)', lineHeight: 1.5 }}>
              I am authorized to accept this agreement on behalf of my organization, and I agree
              to the Inside/Operations Software License Agreement on its behalf.
            </span>
          </label>
        )}
        ...
        {error && (
          <div style={{
            ...
            background: 'var(--io-status-error-bg)', border: '1px solid var(--io-status-error)',
            borderRadius: 'var(--io-radius)', color: 'var(--io-status-error)', fontSize: '13px',
          }}>
            {error}
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
          {!isInstaller ? (
            <p style={{ margin: 0, fontSize: '11px', color: 'var(--io-text-muted)', maxWidth: '420px' }}>
              The Installer EULA (software license) governing your organization's rights is available
              from{' '}<span style={{ color: 'var(--io-text-secondary)' }}>Settings → EULA</span>.
            </p>
          ) : (
            <p style={{ margin: 0, fontSize: '11px', color: 'var(--io-text-muted)', maxWidth: '420px' }}>
              An acceptance record with a cryptographic content hash will be stored for audit purposes.
            </p>
          )}
          ...
          <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
            <button
              ...
              disabled={isPending}
              style={{
                background: 'none', border: '1px solid var(--io-border)',
                borderRadius: 'var(--io-radius)', padding: '8px 18px',
                color: 'var(--io-text-secondary)', fontSize: '13px',
            ...
            <button
              ...
              style={{
                background: canAccept ? 'var(--io-accent)' : 'var(--io-surface-sunken)',
                border: '1px solid', borderColor: canAccept ? 'transparent' : 'var(--io-border)',
                borderRadius: 'var(--io-radius)', padding: '8px 22px',
                color: canAccept ? 'var(--io-accent-contrast)' : 'var(--io-text-muted)',
...
export default function EulaAcceptance() {
  ...
  const { user, pendingEulas, setPendingEulas, removePendingEula, logout } = useAuthStore()
  ...
  const acceptMutation = useMutation({
    mutationFn: async (item: EulaPendingItem) => {
      const context = item.eula_type === 'installer' ? 'installer_admin' : 'login'
      const result = await authApi.eulaAccept(item.version, item.eula_type, context)
  ...
  if (user?.eula_accepted === true && (effectivePending === null || effectivePending.length === 0)) {
  ...
  if (isLoading || effectivePending === null) {
    return (
      <div style={{
        ...
        minHeight: '100vh', background: 'var(--io-surface-primary)',
        color: 'var(--io-text-muted)', fontSize: '14px',
      }}>
        Loading agreements…
      </div>
    )
  ...
  return (
    <div style={{
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      minHeight: '100vh', background: 'var(--io-surface-primary)', padding: '24px',
    }}>
      <EulaPanel
