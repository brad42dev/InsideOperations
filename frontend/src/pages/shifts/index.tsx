...
function SummaryCard({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    ...
    >
      <div style={{ fontSize: 28, fontWeight: 700, color: color ?? 'var(--io-text)' }}>
        {value}
      </div>
      <div style={{ fontSize: 12, color: 'var(--io-text-muted)', marginTop: 2 }}>{label}</div>
    </div>
  )
}
      ...
      {tabs.map((t) => (
        <button
          ...
          style={{
            ...
            color: active === t.id ? 'var(--io-accent)' : 'var(--io-text-secondary)',
            borderBottom: active === t.id ? '2px solid var(--io-accent)' : '2px solid transparent',
            marginBottom: -1,
    ...
    return (
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {p.last_area && (
            <div style={{ fontSize: 12, color: 'var(--io-text-muted)' }}>{p.last_area}</div>
          )}
        </div>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {p.on_site && (
            <span style={badge('#22c55e', 'rgba(34,197,94,0.12)')}>On Site</span>
          )}
          {p.on_shift && (
            <span style={badge('#4a9eff', 'rgba(74,158,255,0.12)')}>On Shift</span>
          )}
        </div>
      </div>
    )
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
        ...
        <SummaryCard label="On Site" value={presence.filter((p) => p.on_site).length} color="#22c55e" />
      ...
      {presence.length === 0 && (
        ...
        >
          No active presence data. Personnel will appear here when on-site or on-shift.
        </div>
      )}
    </div>
  )
...
function ScheduleTab() {
  ...
  const [newCrew, setNewCrew] = useState<Partial<CreateCrewPayload>>({ color: '#6366f1' })
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600 }}>Upcoming Shifts</h3>
        <button
      ...
      {showNewShift && (
        <div style={{ ...surface, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            <div>
              <label style={{ fontSize: 12, color: 'var(--io-text-muted)', display: 'block', marginBottom: 4 }}>
                Shift Name *
              </label>
              <input
                value={newShift.name ?? ''}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewShift((s) => ({ ...s, crew_id: e.target.value || undefined }))}
                style={inputStyle}
              >
      ...
      <div style={{ ...surface, marginBottom: 32 }}>
        ...
        {shifts.map((s) => (
          <ShiftRow key={s.id} shift={s} onDelete={(id) => deleteShiftMut.mutate(id)} />
        ))}
      </div>
        ...
        <div style={{ ...surface, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            <div>
              ...
              <input
                value={newCrew.name ?? ''}
                onChange={(e) => setNewCrew((s) => ({ ...s, name: e.target.value }))}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewCrew((s) => ({ ...s, color: e.target.value }))}
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input
                ...
                onChange={(e) => setNewCrew((s) => ({ ...s, description: e.target.value }))}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              onClick={() => {
                ...
                createCrewMut.mutate({ name: newCrew.name, description: newCrew.description, color: newCrew.color })
              }}
            ...
            <button onClick={() => setShowNewCrew(false)} style={ghostBtn}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={surface}>
        ...
        {crews.map((c) => (
          <CrewRow key={c.id} crew={c} onDelete={(id) => deleteCrewMut.mutate(id)} />
        ))}
      </div>
    </div>
  )
...
function ShiftRow({ shift, onDelete }: { shift: Shift; onDelete: (id: string) => void }) {
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 500, fontSize: 14, color: 'var(--io-text)' }}>{shift.name}</div>
        <div style={{ fontSize: 12, color: 'var(--io-text-muted)', marginTop: 2 }}>
...
function CrewRow({ crew, onDelete }: { crew: ShiftCrew; onDelete: (id: string) => void }) {
  return (
    <div
      style={{
        display: 'flex',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                ...
                {p.email && <div style={{ fontSize: 11, color: 'var(--io-text-muted)' }}>{p.email}</div>}
              </div>
            </div>
            <div>
              <OnSiteDot on={p.on_site} />
              <span style={{ fontSize: 12, color: p.on_site ? '#22c55e' : 'var(--io-text-muted)' }}>
                {p.on_site ? 'Yes' : 'No'}
              </span>
            </div>
            <span style={{ color: 'var(--io-text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {p.last_area ?? '—'}
            </span>
            ...
            <div>
              ...
              <span style={{ fontSize: 12, color: p.on_shift ? '#4a9eff' : 'var(--io-text-muted)' }}>
                {p.on_shift ? 'Yes' : 'No'}
              </span>
            </div>
            {canManagePresence && (
...
function MusterTab() {
  ...
  const declareMut = useMutation({
    mutationFn: (payload: DeclareMusterPayload) => shiftsApi.declareMusterEvent(payload),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['muster-events'] })
  ...
  const accountMut = useMutation({
    mutationFn: ({ eventId, payload }: { eventId: string; payload: AccountPersonPayload }) =>
  ...
  return (
    <div>
      ...
      {/* No active muster */}
      {!activeEvent && (
        <div style={{ ...surface, padding: 32, textAlign: 'center', marginBottom: 24 }}>
          <div style={{ fontSize: 14, color: 'var(--io-text-secondary)', marginBottom: 16 }}>
            No active muster event. Use the button below to declare an emergency muster.
          </div>
          <button
      ...
      {activeEvent && eventDetailQ.data && (
        <div style={{ ...surface, marginBottom: 24 }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--io-text-muted)', marginTop: 2 }}>
                ...
                {activeEvent.declared_by_name && ` by ${activeEvent.declared_by_name}`}
                {activeEvent.total_on_site != null && ` · ${activeEvent.total_on_site} on-site at declaration`}
              </div>
            </div>
            <button
          ...
          {/* Progress */}
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--io-border)' }}>
            <div
              style={{
                display: 'flex',
                justifyContent: 'space-between',
            ...
            >
              ...
              <span style={{ color: 'var(--io-text-muted)' }}>{progressPct}%</span>
            </div>
            <div
              style={{
                height: 8,
          ...
          >
            {['all', 'unaccounted', 'accounted_manual', 'accounted_badge', 'off_site'].map((f) => (
              <button
                key={f}
                ...
                style={{
                  ...
                  borderRadius: 100,
                  background:
                    accountingFilter === f ? 'var(--io-accent)' : 'var(--io-surface-secondary)',
                  color: accountingFilter === f ? '#fff' : 'var(--io-text-secondary)',
                  cursor: 'pointer',
                  fontSize: 12,
                  fontWeight: 500,
                }}
              >
                ...
                {f === 'unaccounted' && accounting.filter((a) => a.status === 'unaccounted').length > 0 && (
                  <span
                    style={{
                      marginLeft: 4,
                      background: '#ef4444',
          ...
          <div>
            ...
            {filtered.length === 0 && (
              <div style={{ padding: 24, textAlign: 'center', color: 'var(--io-text-muted)', fontSize: 13 }}>
      ...
      {/* Historical events */}
      {(historyQ.data ?? []).filter((e) => e.status === 'resolved').length > 0 && (
        <div>
          <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 12px', color: 'var(--io-text-secondary)' }}>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div>
          <div style={{ fontWeight: 500 }}>{row.display_name ?? row.email ?? row.user_id}</div>
        </div>
      </div>
      ...
      <span style={{ color: 'var(--io-text-secondary)', fontSize: 12 }}>
        {row.accounted_at ? new Date(row.accounted_at).toLocaleTimeString() : '—'}
      </span>
      <div style={{ display: 'flex', gap: 6 }}>
        {row.status === 'unaccounted' && (
          <>
            ...
            <button
              onClick={() => onAccount({ user_id: row.user_id, status: 'off_site' })}
              style={{
                padding: '3px 10px',
                border: '1px solid var(--io-border)',
                borderRadius: 4,
...
function MusterHistoryRow({ event }: { event: MusterEvent }) {
  const pct =
    event.accounting_total && event.accounting_total > 0
      ? Math.round(((event.accounting_accounted ?? 0) / event.accounting_total) * 100)
      : null
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p style={{ margin: '0 0 20px', color: 'var(--io-text-secondary)', fontSize: 14, lineHeight: 1.5 }}>
          This will initiate an emergency headcount for all on-site personnel. Accountability rows will
          be created for all currently tracked personnel. This action is logged and cannot be undone.
        </p>
...
function BadgeSourcesSection() {
  ...
  const createMut = useMutation({
    mutationFn: (payload: CreateBadgeSourcePayload) => shiftsApi.createBadgeSource(payload),
    onSuccess: () => {
  ...
  return (
    <div style={{ marginTop: 32 }}>
      ...
      >
        <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600 }}>Badge Access Sources</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewSource((s) => ({ ...s, adapter_type: e.target.value }))}
                style={inputStyle}
              >
                <option value="">Select adapter...</option>
                {['lenel', 'ccure', 'genetec', 'honeywell', 'gallagher', 'generic_db'].map((a) => (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              onClick={() => {
                ...
                createMut.mutate({ name: newSource.name, adapter_type: newSource.adapter_type })
              }}
      ...
      <div style={surface}>
        {sources.length === 0 && (
          <div style={{ padding: 28, textAlign: 'center', color: 'var(--io-text-muted)', fontSize: 13 }}>
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--io-text-muted)', marginTop: 2 }}>
                ...
                {s.last_poll_at && ` · last poll ${new Date(s.last_poll_at).toLocaleString()}`}
            ...
            <button
              ...
              style={{
                ...
                border: '1px solid var(--io-border)',
                borderRadius: 4,
                background: s.enabled ? 'rgba(34,197,94,0.1)' : 'var(--io-surface-secondary)',
