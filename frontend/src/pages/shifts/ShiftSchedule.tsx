...
interface PatternTemplate {
  ...
  blocks: { offsetDays: number; startHour: number; durationHours: number; label: string }[]
}
...
const PATTERN_TEMPLATES: PatternTemplate[] = [
  {
    key: '8h-3',
    name: '8h × 3 Shifts',
    description: '3 rotating 8-hour shifts: Day, Swing, Night. Repeating weekly.',
    blocks: [
  ...
  },
  {
    key: '12h-2',
    name: '12h × 2 Shifts',
    description: '2 rotating 12-hour shifts: Day and Night. Standard continental pattern.',
    blocks: [
  ...
  },
  {
    key: 'dupont',
    name: 'DuPont 12h',
    description: '4-week rotating pattern: 4 nights on, 3 off, 3 days on, 1 off, 3 nights on, 3 off, 4 days on, 7 off.',
    blocks: [
  ...
  },
  {
    key: 'pitman',
    name: 'Pitman 12h',
    description: '2-week rotating pattern: 2 on, 2 off, 3 on, 2 off, 2 on, 3 off.',
    blocks: [
  ...
  },
  {
    key: 'custom',
    name: 'Custom',
    description: 'Single 12-hour day shift as a starting point. Customize before creating.',
    blocks: [
      { offsetDays: 0, startHour: 6, durationHours: 12, label: 'Day' },
    ],
  },
...
function PatternWizardDialog({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast({ title: 'Shifts created', description: `${selected?.blocks.length ?? 0} shifts added from pattern.`, variant: 'success' })
      onCreated()
      onClose()
    },
    onError: () => {
      showToast({ title: 'Failed', description: 'Could not create shifts.', variant: 'error' })
    },
  })
  ...
  const dialogStyle: React.CSSProperties = {
    ...
    borderRadius: 10, padding: 24, width: 560, maxWidth: '95vw', maxHeight: '90vh',
    overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16, zIndex: 1001,
  }
  const inputStyle: React.CSSProperties = {
    width: '100%', padding: '8px 10px', background: 'var(--io-surface-secondary)',
    border: '1px solid var(--io-border)', borderRadius: 6,
    color: 'var(--io-text-primary)', fontSize: 13, boxSizing: 'border-box',
  }

  return (
    <div style={overlayStyle} onClick={onClose}>
      <div style={dialogStyle} onClick={(e) => e.stopPropagation()}>
        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--io-text-primary)' }}>
          {step === 'pick' ? 'Choose a Shift Pattern' : `Configure — ${selected?.name}`}
        </h3>
        ...
        {step === 'pick' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {PATTERN_TEMPLATES.map((pt) => (
              <div
                key={pt.key}
                onClick={() => { setSelected(pt); setStep('config') }}
                style={{
                  padding: '12px 14px', border: '1px solid var(--io-border)', borderRadius: 8,
                  cursor: 'pointer', background: 'var(--io-surface)',
                  transition: 'border-color 0.15s',
                }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--io-accent)' }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--io-border)' }}
              >
                <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--io-text-primary)', marginBottom: 2 }}>{pt.name}</div>
                <div style={{ fontSize: 12, color: 'var(--io-text-muted)' }}>{pt.description}</div>
                <div style={{ fontSize: 11, color: 'var(--io-text-muted)', marginTop: 4 }}>{pt.blocks.length} shifts generated</div>
              </div>
            ))}
          </div>
        )}
        ...
        {step === 'config' && selected && (
          <>
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--io-text-secondary)', marginBottom: 4 }}>
                Pattern Start Date
              </label>
            ...
            <div style={{ background: 'var(--io-surface-secondary)', borderRadius: 6, padding: '10px 12px' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--io-text-secondary)', marginBottom: 6 }}>Preview ({selected.blocks.length} shifts)</div>
              {selected.blocks.map((b, i) => {
                ...
                start.setHours(b.startHour, 0, 0, 0)
                return (
                  <div key={i} style={{ fontSize: 12, color: 'var(--io-text-primary)', padding: '2px 0' }}>
                    {b.label} — {start.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} {b.startHour}:00 ({b.durationHours}h)
                  </div>
                )
              })}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                style={{ flex: 1, padding: '8px 0', background: 'none', border: '1px solid var(--io-border)', borderRadius: 6, cursor: 'pointer', fontSize: 13, color: 'var(--io-text-secondary)' }}
              >
                ← Back
              </button>
              <button
                ...
                style={{ flex: 2, padding: '8px 0', background: 'var(--io-accent)', border: 'none', borderRadius: 6, cursor: createMutation.isPending ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 600, color: '#fff', opacity: createMutation.isPending ? 0.6 : 1 }}
              >
                {createMutation.isPending ? 'Creating…' : `Create ${selected.blocks.length} Shifts`}
              </button>
            </div>
          </>
        )}
        ...
        <button
          onClick={onClose}
          style={{ alignSelf: 'center', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: 'var(--io-text-muted)', padding: '4px 8px' }}
        >
          Cancel
        </button>
      </div>
