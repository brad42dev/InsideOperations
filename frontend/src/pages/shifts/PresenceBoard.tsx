...
function PresenceRow({ p }: { p: PresenceStatus }) {
  return (
    <tr style={{ borderBottom: '1px solid var(--io-border)' }}>
      <td style={{ padding: '11px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ color: 'var(--io-text-primary)', fontWeight: 500, fontSize: 14 }}>
              {p.display_name ?? p.email ?? 'Unknown'}
            </div>
            {p.email && (
              <div style={{ color: 'var(--io-text-muted)', fontSize: 12 }}>{p.email}</div>
            )}
          </div>
        </div>
      </td>
      ...
      <td style={{ padding: '11px 16px', color: 'var(--io-text-secondary)', fontSize: 13 }}>
        {p.last_area ?? <span style={{ color: 'var(--io-text-muted)' }}>—</span>}
      </td>
      <td style={{ padding: '11px 16px', color: 'var(--io-text-secondary)', fontSize: 13 }}>
        {p.last_door ?? <span style={{ color: 'var(--io-text-muted)' }}>—</span>}
      </td>
      <td style={{ padding: '11px 16px', color: 'var(--io-text-muted)', fontSize: 13 }}>
...
export default function PresenceBoard() {
  ...
  const { data, isLoading, isError, dataUpdatedAt } = useQuery<PresenceStatus[]>({
    ...
    <div style={{ padding: 'var(--io-space-6)' }}>
      ...
      >
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: 'var(--io-text-muted)', fontSize: 13 }}>
            ...
            {lastRefreshed && (
              <span style={{ marginLeft: 8 }}>Last updated: {lastRefreshed}</span>
            )}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              ...
              >
                {['Personnel', 'Status', 'Area', 'Last Door', 'Last Seen', 'Shift'].map((h) => (
                  <th
                    key={h}
                    style={{
                      padding: '10px 16px',
      ...
      {!isLoading && !isError && onSiteFirst.length > 0 && (
        <p style={{ marginTop: 10, fontSize: 12, color: 'var(--io-text-muted)' }}>
          {onSiteFirst.length} person{onSiteFirst.length !== 1 ? 's' : ''} tracked
        </p>
      )}
    </div>
  )
