...
export default function ThresholdSearch() {
  ...
  const [selectedExceedance, setSelectedExceedance] = useState<ThresholdExceedance | null>(null)
  const [creatingFor, setCreatingFor] = useState<ThresholdExceedance | null>(null)
  ...
  const columns: ColumnDef<ThresholdExceedance>[] = [
    ...
    },
    {
      ...
      sortable: true,
      cell: (val) => (val as number).toLocaleString(undefined, { maximumFractionDigits: 4 }),
      width: 110,
    },
  ]
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', flex: '1 1 200px' }}>
          <label style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.05em', color: 'var(--io-text-muted)', textTransform: 'uppercase' }}>
            Point
          </label>
          <input
            type="text"
        ...
        {/* Threshold value */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', flex: '0 1 140px' }}>
          <label style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.05em', color: 'var(--io-text-muted)', textTransform: 'uppercase' }}>
            Threshold
          </label>
          <input
          ...
          >
            {(['list', 'trend'] as ViewMode[]).map((mode) => (
              <button
                ...
                style={{
                  padding: '6px 12px',
                  background: viewMode === mode ? 'var(--io-accent-subtle)' : 'var(--io-surface-elevated)',
                  border: 'none',
                  color: viewMode === mode ? 'var(--io-accent)' : 'var(--io-text-muted)',
                  cursor: 'pointer',
                  fontSize: '12px',
        ...
        <button
          ...
          style={{
            padding: '7px 18px',
            background: pointId.trim() && threshold ? 'var(--io-accent)' : 'var(--io-surface)',
            border: 'none',
            borderRadius: 'var(--io-radius)',
            color: pointId.trim() && threshold ? '#fff' : 'var(--io-text-muted)',
        ...
        {!searchMutation.isSuccess && !searchMutation.isPending && !searchMutation.isError && (
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: '14px', color: 'var(--io-text-secondary)' }}>
              Configure a point, operator, and threshold above, then click Search.
            </p>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {exceedances.length > 0 && (
              <div style={{ fontSize: '12px', color: 'var(--io-text-muted)' }}>
                Found {exceedances.length} exceedance{exceedances.length !== 1 ? 's' : ''} where{' '}
            ...
            <DataTable
              data={exceedances}
              columns={columns}
              height={Math.min(400, Math.max(200, exceedances.length * 36 + 40))}
