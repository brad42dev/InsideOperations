...
function StageCard({
  ...
  const [rangeStart, setRangeStart] = useState(
    stage.time_range_start.slice(0, 16),
  );
  ...
  const updateStageMutation = useMutation({
    mutationFn: async (
      ...
      const result = await forensicsApi.updateStage(
        investigationId,
        stage.id,
        patch,
      );
  ...
  const deleteEvidenceMutation = useMutation({
    mutationFn: async (evidenceId: string) => {
      const result = await forensicsApi.deleteEvidence(
        investigationId,
        stage.id,
        evidenceId,
      );
  ...
  const updateEvidenceMutation = useMutation({
    mutationFn: async ({
      ...
      const result = await forensicsApi.updateEvidence(
        investigationId,
        stage.id,
        evidenceId,
        config,
      ...
      >
        ...
        {showSnapshotDialog && (
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setShowSnapshotDialog(false);
            }}
          >
            <div
              style={{
              ...
              >
                ...
                <button
                  ...
                  style={{
                    padding: "6px 16px",
                    background:
                      snapshotGraphicId && snapshotTimestamp
                        ? "var(--io-accent)"
...
function LinksPanel({
  ...
  const removeMutation = useMutation({
    mutationFn: (linkId: string) =>
      forensicsApi.removeLink(investigationId, linkId),
    onSuccess: () => {
      void queryClient.invalidateQueries({
        queryKey: ["investigation-links", investigationId],
      });
          ...
          {showForm && (
            ...
            >
              <select
                value={linkType}
                onChange={(e) =>
                  setLinkType(e.target.value as InvestigationLinkType)
                }
                style={{
                  fontSize: "12px",
                  padding: "4px 6px",
              ...
              >
                {(Object.keys(LINK_TYPE_LABELS) as InvestigationLinkType[]).map(
                  (t) => (
              ...
              <div style={{ display: "flex", gap: "6px" }}>
                <button
                  onClick={() => {
                    if (targetId.trim()) addMutation.mutate();
                  }}
...
function PointsPanel({
  ...
  const removePointMutation = useMutation({
    mutationFn: async ({
      ...
      const result = await forensicsApi.removePoint(
        investigationId,
        pointId,
        reason,
      );
  ...
  const included = points.filter(
    (p: InvestigationPoint) => p.status === "included",
  );
  const suggested = points.filter(
    (p: InvestigationPoint) => p.status === "suggested",
  );
  const removed = points.filter(
    (p: InvestigationPoint) => p.status === "removed",
  );
  ...
  const PointRow = ({
    ...
    >
      ...
      {showRemove && !readOnly && (
        <button
          onClick={() =>
            removePointMutation.mutate({ pointId: point.point_id })
          }
          title="Remove point"
          style={{
            background: "none",
...
function CorrelationHeatmap({
  ...
  const textMuted =
    getComputedStyle(document.documentElement)
      .getPropertyValue("--io-text-muted")
      .trim() || "#a1a1aa";
  const borderColor =
    getComputedStyle(document.documentElement)
      .getPropertyValue("--io-border")
      .trim() || "#3f3f46";
  ...
  if (pointIds.length < 2) {
    return (
      <div
        style={{ padding: "16px", fontSize: 13, color: "var(--io-text-muted)" }}
      >
  ...
  return (
    <EChart
      option={option}
      height={Math.max(240, pointIds.length * 40 + 100)}
    />
  );
}
...
function ResultsPanel({
  ...
  const runAnalysis = async () => {
    ...
    setError(null);
    try {
      const res = await forensicsApi.runCorrelation({
        point_ids: included,
        start,
        end,
      });
  ...
  const corrColumns: ColumnDef<CorrelationResult>[] = [
    {
      id: "point_id_a",
      header: "Point A",
      accessorKey: "point_id_a",
      sortable: true,
    },
    {
      id: "point_id_b",
      header: "Point B",
      accessorKey: "point_id_b",
      sortable: true,
    },
    {
      id: "pearson",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        {tabs.map((t) => (
          <button
            ...
            style={{
              padding: "3px 10px",
              background:
                activeTab === t.key ? "var(--io-surface-elevated)" : "none",
              border:
                activeTab === t.key
        ...
        {error && (
          <span
            style={{ fontSize: "12px", color: "var(--io-danger, #ef4444)" }}
          >
            {error}
          </span>
        )}
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "12px 16px" }}>
        {!results && !running && (
          ...
          >
            No analysis results yet. Click "Run Analysis" to correlate included
            points.
          </div>
        )}
...
export default function InvestigationWorkspace() {
  ...
  const serverStages = (investigation.stages ?? [])
    .slice()
    .sort((a, b) => a.sort_order - b.sort_order);
  ...
  const stages =
    localStageOrder.length === serverStages.length && localStageOrder.length > 0
      ? localStageOrder
          .map((sid) => serverStages.find((s) => s.id === sid)!)
          .filter(Boolean)
      : serverStages;
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    reordered.forEach((stage, idx) => {
      ...
      if (stage.sort_order !== newSortOrder) {
        void forensicsApi
          .updateStage(id, stage.id, { sort_order: newSortOrder })
          .then(() => {
            void queryClient.invalidateQueries({
              queryKey: ["investigation", id],
            });
          ...
          <>
            ...
            <button
              onClick={() => {
                if (
                  confirm(
                    "Cancel this investigation? This action cannot be undone.",
                  )
                ) {
          ...
          >
            {stages.length === 0 && (
              ...
              >
                ...
                >
                  No stages yet. Add a stage to begin organizing your
                  investigation.
                </p>
              </div>
            )}
            ...
            <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
              ...
              >
                {stages.map((stage) => (
                  <ErrorBoundary
                    key={stage.id}
                    module={`Forensics — Stage: ${stage.name}`}
                  >
                    <SortableStageCard
                      stage={stage}
                      investigationId={investigation.id}
              ...
              {(["csv", "xlsx", "json", "pdf", "html"] as const).map((fmt) => (
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const result = await forensicsApi.exportInvestigation(
                        id,
                        fmt,
                      );
                      if (!result.success)
          ...
          >
            ...
            >
              ...
              <button
                disabled={
                  shareStatus === "loading" ||
                  (shareUserIds.trim() === "" && shareRoleIds.trim() === "")
                }
                onClick={async () => {
                  if (!id) return;
      ...
      {showCloseConfirm && (
        ...
        >
          ...
          >
            ...
            >
              Closing this investigation will lock it for editing. You can still
              view it.
            </p>
            <div
              style={{
