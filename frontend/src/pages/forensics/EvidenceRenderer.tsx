...
function TrendEvidence({
  ...
  const timestamps =
    (query.data ?? [])[0]?.data.map((d) =>
      Math.floor(new Date(d.ts).getTime() / 1000),
    ) ?? [];
...
function AnnotationEvidence({
  ...
  const [text, setText] = useState(
    (item.config.text as string | undefined) ?? "",
  );
  ...
  return (
    ...
    >
      ...
      >
        {text || (
          <span style={{ color: "var(--io-text-muted)", fontStyle: "italic" }}>
            No annotation text. Click edit to add a note.
          </span>
        )}
      </span>
      {!readOnly && (
                  ...
                  >
                    {pv
                      ? typeof pv.value === "number"
                        ? pv.value.toLocaleString(undefined, {
...
export default function EvidenceRenderer({
  item,
  stageStart,
  stageEnd,
  onDelete,
  ...
  const meta = typeLabels[item.evidence_type] ?? {
