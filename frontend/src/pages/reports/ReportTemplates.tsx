...
export default function ReportTemplates() {
  ...
  const [subscribeTarget, setSubscribeTarget] = useState<ReportTemplate | null>(
    null,
  );
  ...
  const categories = Array.from(
    new Set(
      templates
        .map((t: (typeof templates)[0]) => t.category)
    ...
    >
      ...
      <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
        ...
        >
          {isLoading
            ? "Loading…"
            : `${result?.pagination.total ?? 0} templates`}
        </span>
      </div>
      ...
      {subscribeTarget && (
        <SubscribeDialog
          ...
          onOpenChange={(open) => {
            if (!open) setSubscribeTarget(null);
          }}
        />
      )}
      ...
      {result && result.pagination.pages > 1 && (
        ...
        >
          ...
          <button
            onClick={() =>
              setPage((p) => Math.min(result.pagination.pages, p + 1))
            }
            ...
            style={{
              ...
              borderRadius: "6px",
              cursor:
                page === result.pagination.pages ? "not-allowed" : "pointer",
              color: "var(--io-text-secondary)",
              opacity: page === result.pagination.pages ? 0.4 : 1,
              fontSize: "13px",
            }}
          >
            Next →
