...
function WorkspaceNotFound({ workspaceId }: { workspaceId: string | undefined }) {
  return (
    <div
      style={{
        display: 'flex',
    ...
    >
      ...
      <p style={{ margin: 0, fontSize: 14, fontWeight: 500, color: 'var(--io-text-primary)' }}>
        Workspace not found
      </p>
      {workspaceId && (
...
export default function WorkspaceView({ detached = false }: Props) {
  ...
  const { workspaceId: workspaceIdDetached, workspace_id: workspaceIdMain } = useParams<{
    workspaceId?: string
    workspace_id?: string
  }>()
  ...
  const { data: workspace, isLoading, isError } = useQuery<WorkspaceLayout | null>({
    ...
    return (
      ...
      >
        ...
        <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
          ...
          {notFound && !isLoading && <WorkspaceNotFound workspaceId={workspaceId} />}
