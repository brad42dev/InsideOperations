...
function recurrenceLabel(type: RecurrenceType, config: Record<string, unknown>): string {
  switch (type) {
    case 'per_shift': return 'Every shift'
    case 'daily':     return `Daily at ${config.time ?? '00:00'}`
    case 'interval':  return `Every ${config.interval_hours ?? '?'}h`
    case 'weekly': {
...
export default function RoundSchedules() {
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      const config: Record<string, unknown> =
        newRecurrence === 'daily' ? { time: newTime } :
        newRecurrence === 'interval' ? { interval_hours: parseFloat(newInterval) } :
        newRecurrence === 'per_shift' ? {} : {}
      return roundsApi.createSchedule({ template_id: newTemplateId, recurrence_type: newRecurrence, recurrence_config: config })
    },
    onSuccess: (result) => {
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['rounds', 'schedules'] }),
  })
  ...
  return (
    <div style={{ padding: 'var(--io-space-6)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: 'var(--io-text-primary)' }}>Round Schedules</h2>
      ...
      {/* New schedule form */}
      {showNew && (
        <div style={{ padding: '20px', background: 'var(--io-surface)', border: '1px solid var(--io-accent)', borderRadius: '8px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 700, color: 'var(--io-text-primary)' }}>New Schedule</h3>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--io-text-secondary)', marginBottom: '4px' }}>Template</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--io-text-secondary)', marginBottom: '4px' }}>Recurrence</label>
              <select value={newRecurrence} onChange={(e) => setNewRecurrence(e.target.value as RecurrenceType)} style={inputStyle}>
          ...
          {newRecurrence === 'daily' && (
            <div style={{ maxWidth: '160px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--io-text-secondary)', marginBottom: '4px' }}>Time</label>
          ...
          {newRecurrence === 'interval' && (
            <div style={{ maxWidth: '160px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--io-text-secondary)', marginBottom: '4px' }}>Interval (hours)</label>
              <input type="number" min={1} max={24} value={newInterval} onChange={(e) => setNewInterval(e.target.value)} style={inputStyle} />
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
            <button style={btnStyle()} onClick={() => { setShowNew(false); setError(null) }}>Cancel</button>
            <button style={btnStyle(true)} onClick={() => createMutation.mutate()} disabled={!newTemplateId || createMutation.isPending}>
      ...
      {isLoading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {schedules.map((s) => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', background: 'var(--io-surface)', border: '1px solid var(--io-border)', borderRadius: '8px' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--io-text-muted)', marginTop: '2px' }}>
                  {recurrenceLabel(s.recurrence_type as RecurrenceType, s.recurrence_config)}
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <button
                  ...
                  style={{ padding: '5px 12px', background: 'none', border: '1px solid var(--io-border)', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', color: 'var(--io-text-secondary)' }}
                >
