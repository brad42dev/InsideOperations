...
function checkpointToApi(cp: EditableCheckpoint): Checkpoint {
  ...
  if (cp.data_type === 'numeric' && cp.validation_mode !== 'none') {
    const val: CheckpointValidation = { mode: cp.validation_mode as 'limit' | 'alarm' }
...
function CheckpointEditor({
  ...
  function set<K extends keyof EditableCheckpoint>(key: K, value: EditableCheckpoint[K]) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ color: 'var(--io-text-muted)', fontSize: '12px', minWidth: '20px' }}>
          {index + 1}.
        </span>
        <span style={{ flex: 1, fontWeight: 600, fontSize: '13px', color: 'var(--io-text-primary)' }}>
          {cp.title || <em style={{ fontWeight: 400, color: 'var(--io-text-muted)' }}>Untitled checkpoint</em>}
        </span>
        <span style={{ fontSize: '11px', color: 'var(--io-text-muted)', background: 'var(--io-surface)', padding: '2px 6px', borderRadius: '4px' }}>
          {cp.data_type}
        </span>
        <div style={{ display: 'flex', gap: '4px' }} onClick={(e) => e.stopPropagation()}>
          <button
            ...
            disabled={index === 0}
            title="Move up"
            style={{ background: 'none', border: 'none', cursor: index === 0 ? 'not-allowed' : 'pointer', color: 'var(--io-text-muted)', opacity: index === 0 ? 0.3 : 1, padding: '2px 6px' }}
          >
            ↑
          </button>
          <button
            ...
            title="Move down"
            style={{ background: 'none', border: 'none', cursor: index === total - 1 ? 'not-allowed' : 'pointer', color: 'var(--io-text-muted)', opacity: index === total - 1 ? 0.3 : 1, padding: '2px 6px' }}
          >
            ↓
          </button>
          <button
        ...
        <span style={{ color: 'var(--io-text-muted)', fontSize: '12px' }}>{expanded ? '▲' : '▼'}</span>
      </div>
        ...
        <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <Field label="Data Type">
              <select value={cp.data_type} onChange={(e) => set('data_type', e.target.value as DataType)} style={inputStyle}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            />
            <label htmlFor={`req-${cp.index}`} style={{ fontSize: '13px', color: 'var(--io-text-secondary)', cursor: 'pointer' }}>
              Required
            </label>
          </div>
          ...
          {cp.data_type === 'numeric' && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <Field label="Unit">
                  <input value={cp.unit} onChange={(e) => set('unit', e.target.value)} placeholder="PSI, °C, RPM…" style={inputStyle} />
                </Field>
                <Field label="Validation">
                  <select value={cp.validation_mode} onChange={(e) => set('validation_mode', e.target.value as ValidationMode)} style={inputStyle}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '8px' }}>
                  {(['hh', 'h', 'l', 'll'] as const).map((key) => (
                    <Field key={key} label={key.toUpperCase()}>
                      <input type="number" value={cp[key]} onChange={(e) => set(key, e.target.value)} style={inputStyle} />
                    </Field>
                  ))}
                </div>
              )}
              ...
              {cp.validation_mode === 'limit' && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <Field label="Min">
                    <input type="number" value={cp.min} onChange={(e) => set('min', e.target.value)} style={inputStyle} />
                  </Field>
                  <Field label="Max">
                    <input type="number" value={cp.max} onChange={(e) => set('max', e.target.value)} style={inputStyle} />
                  </Field>
                </div>
              )}
            </>
            ...
            <Field label="Options">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                <button
                  ...
                  style={{ alignSelf: 'flex-start', background: 'none', border: '1px dashed var(--io-border)', borderRadius: '6px', cursor: 'pointer', color: 'var(--io-text-secondary)', padding: '6px 12px', fontSize: '12px' }}
                >
                  + Add option
                </button>
              </div>
          ...
          {cp.data_type === 'multi_field' && (
            <Field label="Sub-fields">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                <button
                  onClick={() => set('fields', [...cp.fields, { name: '', type: 'text' }])}
                  style={{ alignSelf: 'flex-start', background: 'none', border: '1px dashed var(--io-border)', borderRadius: '6px', cursor: 'pointer', color: 'var(--io-text-secondary)', padding: '6px 12px', fontSize: '12px' }}
                >
                  + Add sub-field
                </button>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <Field label="Comments requirement">
              <select value={cp.comments} onChange={(e) => set('comments', e.target.value)} style={inputStyle}>
                <option value="none">None</option>
                <option value="optional">Optional</option>
                <option value="required">Required</option>
                <option value="required_on_alarm">Required on alarm</option>
...
export default function TemplateDesigner() {
  ...
  const [checkpoints, setCheckpoints] = useState<EditableCheckpoint[]>([emptyCheckpoint(0)])
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: { name: string; description?: string; checkpoints: Checkpoint[] }) =>
      isNew
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--io-text-secondary)', fontSize: '18px', padding: '4px' }}
        >
          ←
        </button>
        <h1 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: 'var(--io-text-primary)', flex: 1 }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        >
          <h2 style={{ margin: 0, fontSize: '14px', fontWeight: 700, color: 'var(--io-text-primary)' }}>
            Template Details
          </h2>
