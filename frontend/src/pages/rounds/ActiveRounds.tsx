...
export default function ActiveRounds() {
  ...
  return (
    <div style={{ padding: 'var(--io-space-6)', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: 'var(--io-text-primary)' }}>Active Rounds</h2>
      ...
      {/* In-Progress */}
      <section>
        <h3 style={{ margin: '0 0 12px', fontSize: '13px', fontWeight: 700, color: 'var(--io-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          In Progress ({activeInstances.length})
        </h3>
        {isLoading ? (
          <div style={{ color: 'var(--io-text-muted)', fontSize: '14px' }}>Loading…</div>
        ) : activeInstances.length === 0 ? (
          <div style={{ padding: '24px', textAlign: 'center', color: 'var(--io-text-muted)', fontSize: '14px', background: 'var(--io-surface)', borderRadius: '8px', border: '1px solid var(--io-border)' }}>
            ...
            {activeInstances.map((inst) => (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: '14px', color: 'var(--io-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {inst.template_name ?? 'Unnamed template'}
                  </div>
                  <div style={{ fontSize: '12px', color: 'var(--io-text-muted)', marginTop: '2px' }}>
                    Started {inst.started_at ? new Date(inst.started_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}
            ...
            {pendingInstances.map((inst) => (
              <div key={inst.id} style={{ ...cardStyle, cursor: 'default' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--io-text-muted)', marginTop: '2px' }}>
                    Created {new Date(inst.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <DueBadge due={inst.due_by} />
