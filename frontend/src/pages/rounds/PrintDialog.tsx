...
import { roundsApi, type RoundTemplate, type Checkpoint, type RoundResponse } from '../../api/rounds'
...
function getResponseValue(
  ...
  if (resp.response_value === null || resp.response_value === undefined) return ''
...
function getPassFail(
  ...
  if (resp.response_value !== null && resp.response_value !== undefined) return 'Pass'
  return ''
}
...
function PrintArea({ template, mode, pageSize: _pageSize, responses }: PrintAreaProps) {
    ...
    if (cp.data_type === 'multi_field' && cp.fields && cp.fields.length > 0) {
      cp.fields.forEach((field, fi) => {
        ...
        rows.push({
          ...
          reading,
          passFail: fi === 0 && mode === 'current_results' ? getPassFail(responses, cp.index) : '',
          notes: '',
          groupStart: fi === 0,
      ...
      rows.push({
        ...
        reading,
        passFail: mode === 'current_results' ? getPassFail(responses, cp.index) : '',
        notes: '',
      })
    }
    rowNum++
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '12px', borderBottom: '2px solid #333', paddingBottom: '8px' }}>
        <div style={{ fontSize: '14pt', fontWeight: 700, color: '#000', marginBottom: '4px' }}>
          {template.name}
        </div>
        <div style={{ fontSize: '9pt', color: '#333', marginBottom: '2px' }}>
          Printed: {printDate}
          {mode === 'current_results' && (
            <span style={{ marginLeft: '16px', fontStyle: 'italic' }}>Mode: Current Results</span>
          )}
          {mode === 'blank' && (
            <span style={{ marginLeft: '16px', fontStyle: 'italic' }}>Mode: Blank Checklist</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: '32px', marginTop: '8px', fontSize: '10pt' }}>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i}>
              <td style={{
                ...cellStyle,
                borderTop: row.groupStart ? '2px solid #555' : cellStyle.border,
                borderBottom: row.groupEnd ? '2px solid #555' : cellStyle.border,
              }}>{row.num}</td>
              <td style={{
                ...cellStyle,
                borderTop: row.groupStart ? '2px solid #555' : cellStyle.border,
              ...
              <td style={{
                ...
                borderBottom: row.groupEnd ? '2px solid #555' : cellStyle.border,
                fontSize: '8pt',
              }}>{row.expectedRange}</td>
              <td style={{
                ...cellStyle,
                borderTop: row.groupStart ? '2px solid #555' : cellStyle.border,
                borderBottom: row.groupEnd ? '2px solid #555' : cellStyle.border,
                background: mode === 'current_results' && row.reading ? '#fff' : '#f9f9f9',
              }}>{row.reading}</td>
              <td style={{
                ...cellStyle,
                borderTop: row.groupStart ? '2px solid #555' : cellStyle.border,
      ...
      >
        <span style={{ fontWeight: 700, letterSpacing: '0.05em' }}>UNCONTROLLED COPY</span>
        <span>Page 1</span>
      </div>
    </div>
  )
...
function PrintDialogContent({ preselectedTemplateId, onClose }: PrintDialogContentProps) {
  ...
  const radioOptionStyle = (active: boolean): React.CSSProperties => ({
    ...
    borderRadius: '6px',
    background: active ? 'var(--io-accent-subtle, rgba(74,158,255,0.1))' : 'var(--io-surface)',
    cursor: 'pointer',
    fontSize: '13px',
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div>
          ...
          <div style={radioGroupStyle}>
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setMode('blank') }}
            >
            ...
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setMode('current_results') }}
            >
              <span>Current results</span>
            </div>
          </div>
          {mode === 'current_results' && (
            <div style={{ fontSize: '12px', color: 'var(--io-text-muted)', marginTop: '6px' }}>
        ...
        <div>
          ...
          <div style={radioGroupStyle}>
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setPageSize('letter') }}
            >
              Letter (8.5 × 11)
            </div>
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setPageSize('a4') }}
            >
              A4 (210 × 297 mm)
            </div>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '4px' }}>
          ...
          <button
            onClick={handlePrint}
            disabled={!selectedTemplateId || !template || isLoading || isPrinting}
            style={{
              padding: '8px 20px',
              ...
              border: 'none',
              borderRadius: '6px',
              cursor: !selectedTemplateId || !template || isLoading || isPrinting ? 'not-allowed' : 'pointer',
              fontSize: '13px',
              fontWeight: 600,
              opacity: !selectedTemplateId || !template || isLoading || isPrinting ? 0.65 : 1,
            }}
          >
            {isPrinting ? 'Opening print dialog…' : isLoading ? 'Loading…' : 'Print'}
          </button>
        </div>
      </div>
...
export function PrintDialog({ preselectedTemplateId, trigger }: PrintDialogProps) {
