...
function statusBadge(status: string) {
  ...
  return (
    <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '100px', background: c.bg, color: c.text, fontWeight: 700, letterSpacing: '0.04em' }}>
      {status.replace('_', ' ')}
    </span>
  )
}
...
export default function RoundHistory() {
  ...
  return (
    <div style={{ padding: 'var(--io-space-6)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: 'var(--io-text-primary)' }}>Round History</h2>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: 'var(--io-text-secondary)' }}>
          From
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          <thead>
            <tr style={{ background: 'var(--io-surface-secondary)' }}>
              {['Template', 'Status', 'Started', 'Completed', 'Responses', 'Out of Range'].map((h) => (
          ...
          <tbody>
            {entries.length === 0 ? (
              <tr>
                <td colSpan={6} style={{ padding: '32px', textAlign: 'center', color: 'var(--io-text-muted)', fontSize: '14px' }}>
                  {isLoading ? 'Loading…' : 'No rounds found for this period.'}
                </td>
              </tr>
            ) : (
              entries.map((entry, i) => (
                <tr key={entry.id} style={{ background: i % 2 === 0 ? 'transparent' : 'var(--io-surface-secondary)' }}>
                  <td style={{ padding: '10px 12px', color: 'var(--io-text-primary)', fontWeight: 500, maxWidth: '240px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {entry.template_name}
                  </td>
                  <td style={{ padding: '10px 12px' }}>
                    {statusBadge(entry.status)}
                  </td>
                  <td style={{ padding: '10px 12px', color: 'var(--io-text-secondary)', whiteSpace: 'nowrap' }}>
                    {formatDate(entry.started_at)}
                  </td>
                  <td style={{ padding: '10px 12px', color: 'var(--io-text-secondary)', whiteSpace: 'nowrap' }}>
                    {formatDate(entry.completed_at)}
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'center', color: 'var(--io-text-secondary)' }}>
