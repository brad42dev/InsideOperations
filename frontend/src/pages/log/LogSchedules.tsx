...
const RECURRENCE_BADGE_COLORS: Record<
  LogSchedule["recurrence_type"],
  React.CSSProperties
> = {
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                set(
                  "recurrence_type",
                  e.target.value as ScheduleFormData["recurrence_type"],
                )
              }
              style={fieldStyle()}
            >
              <option value="per_shift">
                Per Shift — one instance per shift
              </option>
...
export default function LogSchedules() {
  ...
  const schedules =
    schedData?.success && Array.isArray(schedData.data) ? schedData.data : [];
  const templates =
    tmplData?.success && Array.isArray(tmplData.data) ? tmplData.data : [];
  ...
  return (
    ...
    >
      ...
      {schedLoading ? (
        ...
        >
          No schedules yet. Create one to start generating log instances
          automatically.
        </div>
      ) : (
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: "4px",
                      fontWeight: 600,
                      ...RECURRENCE_BADGE_COLORS[s.recurrence_type],
                    }}
                  >
                    {RECURRENCE_LABELS[s.recurrence_type]}
                    {s.recurrence_type === "time_window" &&
                    s.interval_hours != null
                      ? ` / ${s.interval_hours}h`
                      : ""}
                  </span>
                  {/* Active badge */}
                  <span
                    style={{
                      ...
                      color: s.is_active ? "#22c55e" : "var(--io-text-muted)",
                      fontWeight: 600,
                    }}
                  >
        ...
        >
          ...
          >
            ...
            >
              ...
              <strong>
                {deleteTarget.template_name ?? deleteTarget.template_id}
              </strong>
