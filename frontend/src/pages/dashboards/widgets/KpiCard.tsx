...
export default function KpiCard({ config }: Props) {
  ...
  const isValidPointRef =
    !!pointId &&
    (UUID_RE.test(pointId) || pointId.includes("-") || pointId.includes("."));
  ...
  const liveValues = usePointValues(
    pointId && cfg.staticValue === undefined ? [pointId] : [],
  );
  const livePoint = pointId ? liveValues.get(pointId) : undefined;

  const value =
    cfg.staticValue !== undefined
      ? cfg.staticValue
  ...
  const color =
    convertedValue !== null
      ? getTrendColor(convertedValue, cfg.thresholds)
      : "var(--io-text-muted)";
