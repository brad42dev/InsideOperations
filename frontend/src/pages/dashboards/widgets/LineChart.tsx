...
function parseTimeRange(range: string): { start: string; end: string } {
  ...
  const hours = parseInt(
    range.replace("h", "").replace("m", "").replace("d", ""),
    10,
  );
...
export default function LineChart({ config }: Props) {
    ...
    queryFn: async () => {
      ...
      const results = await Promise.all(
        points.map(async (pointId) => {
          ...
          if (!res.success)
            return { point_id: pointId, data: [] as HistoryPoint[] };
          return res.data;
        }),
      );
      return results;
  ...
  const timestamps =
    hasLive && baseTimestamps.length > 0
  ...
  const seriesData = (query.data ?? []).map((result, idx) => {
    ...
    const data =
      livePoint != null && historicalValues.length > 0
  ...
  return (
    ...
    >
      ...
      >
        <TimeSeriesChart timestamps={timestamps} series={seriesData} />
      </div>
    </PointContextMenu>
  );
}
