...
export default function QualityDistributionWidget({ config: _config }: Props) {
  const query = useQuery({
    ...
    queryFn: async () => {
      const result = await api.get<PointCurrentRow[]>('/api/opc/points/current-quality')
    ...
    series: [
      {
        ...
        data: [
          ...
          { value: counts.Uncertain, name: 'Uncertain', itemStyle: { color: '#f59e0b' } },
          { value: counts.Unknown, name: 'Unknown', itemStyle: { color: '#6b7280' } },
        ].filter((d) => d.value > 0),
        label: { show: false },
...
function QualityDistributionFallback() {
  const query = useQuery({
    ...
    queryFn: async () => {
      const result = await api.get<{ source_id: string; name: string; connected: boolean; subscribed_points?: number }[]>(
  ...
  const totalPts = sources.reduce((sum, s) => sum + (s.subscribed_points ?? 0), 0)
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '11px', color: 'var(--io-text-muted)', textAlign: 'center' }}>
        Good Quality
      </div>
