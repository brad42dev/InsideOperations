...
export default function AreaStatusTableWidget({ config, variables }: Props) {
  ...
  for (const alarm of alarms) {
    ...
    if (!areaMap.has(area)) {
      areaMap.set(area, { area, total: 0, critical: 0, high: 0, medium: 0, unacked: 0 })
    }
  ...
  let rows = Array.from(areaMap.values()).sort((a, b) => b.critical - a.critical || b.high - a.high || b.total - a.total)
  if (areaFilter && areaFilter !== '__all__') {
    rows = rows.filter((r) => r.area.toLowerCase().includes(areaFilter.toLowerCase()))
  }
