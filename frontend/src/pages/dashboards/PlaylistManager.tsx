...
import { dashboardsApi, type Playlist, type PlaylistItem, type Dashboard } from '../../api/dashboards'
  ...
  return (
    ...
    >
      <span
        {...attributes}
        {...listeners}
        style={{ color: 'var(--io-text-muted)', cursor: 'grab', fontSize: '14px', flexShrink: 0, userSelect: 'none' }}
      >
        ⠿
      </span>
      ...
      <label style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
        <input
          type="number"
          min={5}
          max={3600}
          value={item.dwell_seconds}
          onChange={(e) => onUpdate({ dwell_seconds: parseInt(e.target.value) || 30 })}
          style={{
            width: '56px',
            padding: '5px 6px',
            background: 'var(--io-surface-secondary)',
        ...
        />
        <span style={{ fontSize: '11px', color: 'var(--io-text-muted)' }}>s</span>
      </label>
...
function NewPlaylistForm({
  ...
  function handleSave() {
    if (!name.trim()) return
    const playlistItems: Omit<PlaylistItem, 'id'>[] = items.map((item, idx) => ({
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {items.map((item) => (
              <SortablePlaylistRow
                ...
                onUpdate={(updates) => updateItem(item.id, updates as Partial<NewPlaylistItem>)}
...
export default function PlaylistManager({ onClose }: Props) {
  ...
  const createMutation = useMutation({
    mutationFn: async ({ name, items }: { name: string; items: Omit<PlaylistItem, 'id'>[] }) => {
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {playlistsQuery.isLoading && (
            <div style={{ color: 'var(--io-text-muted)', fontSize: '13px', textAlign: 'center', padding: '20px' }}>
              Loading playlists...
            </div>
          )}

          {!playlistsQuery.isLoading && playlists.map((playlist) => (
            <div
              key={playlist.id}
              style={{
                background: 'var(--io-surface-secondary)',
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--io-text-primary)', marginBottom: '2px' }}>
                  {playlist.name}
                </div>
                <div style={{ fontSize: '12px', color: 'var(--io-text-muted)' }}>
                  {playlist.items.length} dashboard{playlist.items.length !== 1 ? 's' : ''}
                </div>
              </div>
              <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                <button
                  onClick={() => {
                    onClose()
                    navigate(`/dashboards/playlist/${playlist.id}?kiosk=true`)
                  }}
                  style={{
                    padding: '5px 10px',
                    background: 'var(--io-accent)',
          ...
          {!playlistsQuery.isLoading && playlists.length === 0 && !showNewForm && (
            <div style={{ textAlign: 'center', padding: '24px', color: 'var(--io-text-muted)', fontSize: '13px' }}>
              No playlists yet. Create one to auto-advance dashboards in kiosk mode.
            </div>
          )}
