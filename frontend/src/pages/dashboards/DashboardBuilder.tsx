      ...
      >
        ...
        {(widget.type === "kpi-card" || widget.type === "line-chart") && (
          ...
          >
            ...
            <select
              value={String(
                cfg.aggregationType ?? aggOptions[0]?.value ?? "min",
              )}
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          >
            ...
            <select
              value={widget.w}
              onChange={(e) =>
                onUpdate({ ...widget, w: parseInt(e.target.value) })
              }
              style={{
                padding: "6px 8px",
                background: "var(--io-surface-elevated)",
          ...
          >
            ...
            <select
              value={widget.h}
              onChange={(e) =>
                onUpdate({ ...widget, h: parseInt(e.target.value) })
              }
              style={{
                padding: "6px 8px",
                background: "var(--io-surface-elevated)",
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateVar(idx, {
                        type: e.target.value as DashboardVariable["type"],
                      })
                    }
                    style={{
                      padding: "5px 8px",
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateVar(idx, { default: e.target.value || undefined })
                  }
                  style={{
                    padding: "5px 8px",
                    background: "var(--io-surface-elevated)",
...
function DashboardExportDialog({
  ...
  async function handleExport() {
    ...
    setError(null);
    try {
      const blob = await dashboardsApi.exportIographic(
        dashboardId,
        description,
      );
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>
            Packages this dashboard definition as a portable .iographic file.
          </div>
        </div>
...
export default function DashboardBuilder() {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      ...
      showToast({
        title: "Save failed",
        description:
          err.message || "Could not save the dashboard. Please try again.",
        variant: "error",
        duration: 0,
      });
    },
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    setWidgets((prev) =>
      prev.map((w) => {
        ...
        const newX = Math.max(
          0,
          Math.min(11, w.x + Math.round(delta.x / (colWidth * 60))),
        );
  ...
  const filteredLibraryItems = WIDGET_LIBRARY.filter(
    (item) => item.category === widgetCategory,
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            onClick={() =>
              navigate(isNew ? "/dashboards" : `/dashboards/${id}`)
            }
            style={{
              padding: "5px 12px",
              background: "none",
