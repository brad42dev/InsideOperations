  ...
  return (
    ...
    >
      {variables.map((variable) => {
        const currentValues = values[variable.name] ?? (variable.default ? [variable.default] : [])
        const options = variable.options ?? []

        if (variable.type === 'text' || variable.type === 'constant') {
          return (
            <div key={variable.name} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <label
                style={{ fontSize: '12px', color: 'var(--io-text-muted)', fontWeight: 600 }}
              >
                {variable.label}
              </label>
              <input
        ...
        if (variable.type === 'interval') {
          const intervalOptions = options.length > 0 ? options : ['1m', '5m', '15m', '30m', '1h', '6h', '24h']
          return (
            <div key={variable.name} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        const allOption = variable.include_all ? ['__all__', ...options] : options
