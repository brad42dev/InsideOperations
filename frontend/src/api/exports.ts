...
export const exportsApi = {
  ...
  create: async (req: CreateExportRequest): Promise<ExportResult> => {
    ...
    if (res.status === 202) {
      ...
      try {
        const envelope = (await res.json()) as { data?: ExportJobRef } & Record<
          string,
          unknown
        >;
    ...
    const filename =
      filenameMatch?.[1]?.replace(/['"]/g, "") ?? derivedFilename(req);
  ...
  ): Promise<{ data: ExportJob[]; total: number }> => {
    const qs = new URLSearchParams();
    if (params?.limit !== undefined) qs.set("limit", String(params.limit));
    if (params?.page !== undefined) qs.set("page", String(params.page));
    const url =
      API_BASE + "/api/exports" + (qs.toString() ? "?" + qs.toString() : "");
  ...
  deleteExport: async (jobId: string): Promise<void> => {
    const res = await authFetch(API_BASE + "/api/exports/" + jobId, {
      method: "DELETE",
    });
    if (!res.ok) {
  ...
  cancelExport: async (jobId: string): Promise<void> => {
    const res = await authFetch(
      API_BASE + "/api/exports/" + jobId + "/cancel",
      { method: "POST" },
    );
    if (!res.ok) {
  ...
  retryExport: async (jobId: string): Promise<void> => {
    const res = await authFetch(API_BASE + "/api/exports/" + jobId + "/retry", {
      method: "POST",
    });
    if (!res.ok) {
