...
import type { WorkspaceLayout, PaneConfig, LayoutPreset, GridItem } from '../pages/console/types'
...
function normalizeWorkspace(raw: WorkspaceSummary): WorkspaceLayout {
  ...
  return {
    ...
    overflowPanes: Array.isArray(meta.overflowPanes) ? meta.overflowPanes : undefined,
...
export const consoleApi = {
  listWorkspaces: async (): Promise<ApiResult<WorkspaceLayout[]>> => {
    const result = await api.get<PaginatedResult<WorkspaceSummary>>('/api/console/workspaces')
  ...
  getWorkspace: async (id: string): Promise<ApiResult<WorkspaceLayout>> => {
    const result = await api.get<WorkspaceSummary>(`/api/console/workspaces/${id}`)
    if (!result.success) return result
  ...
  publishWorkspace: async (id: string, published: boolean): Promise<ApiResult<WorkspaceLayout>> => {
    const result = await api.patch<WorkspaceSummary>(`/api/console/workspaces/${id}/publish`, { published })
    if (!result.success) return result
    return { success: true, data: normalizeWorkspace(result.data) }
  },
