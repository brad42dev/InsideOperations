...
export const videoExportsApi = {
  create: (req: CreateVideoExportRequest) =>
    fetch('/api/video-exports', {
      ...
      let body: unknown;
      try { body = await r.json(); } catch { body = { status: r.status, message: `HTTP ${r.status}` }; }
      return Promise.reject(body);
    }),

  list: (): Promise<VideoExportJob[]> =>
    fetch('/api/video-exports', { headers: authHeaders() })
      .then(async r => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        const body = await r.json() as { data?: VideoExportJob[] } | VideoExportJob[];
        return Array.isArray(body) ? body : (body as { data?: VideoExportJob[] }).data ?? [];
      }),
  ...
  cancel: (id: string): Promise<void> =>
    fetch(`/api/video-exports/${id}`, { method: 'DELETE', headers: authHeaders() }).then(() => undefined),
