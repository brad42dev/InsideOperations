...
export const snapshotsApi = {
  ...
  create: (req: CreateSnapshotRequest) =>
    api.post<{ id: string }>("/api/snapshots", req),

  delete: (id: string) => api.delete(`/api/snapshots/${id}`),

  restorePreview: (id: string) =>
    api.get<RestorePreview>(`/api/snapshots/${id}/restore-preview`),

  restore: (id: string, req?: RestoreRequest) =>
    api.post<RestoreResult>(
      `/api/snapshots/${id}/restore`,
      req ?? { mode: "all", create_safety_snapshot: true },
    ),
};
...
> {
  ...
  const envelope = json as { success?: boolean; data?: T } & Record<
    string,
    unknown
  >;
  if ("data" in envelope) {
...
export const bulkUpdateApi = {
  ...
  downloadTemplate: async (targetType: TargetType): Promise<void> => {
    ...
    const res = await fetch(
      `${API_BASE}/api/bulk-update/template/${targetType}`,
      {
        headers,
        credentials: "include",
      },
  ...
  downloadErrorReport: async (
    ...
    if (failedRows && failedRows.length > 0) {
      ...
      const lines = failedRows.map((r) => {
        const reasonEscaped = r.reason.includes(",")
          ? `"${r.reason.replace(/"/g, '""')}"`
          : r.reason;
    ...
    const res = await fetch(
      `${API_BASE}/api/bulk-update/${snapshotId}/error-report`,
      {
        headers,
        credentials: "include",
      },
  ...
  downloadFullResults: (
    ...
    const header =
      "snapshot_id,target_type,modified,unchanged,added,removed,validation_failed\n";
