...
export const emailApi = {
  ...
  createProvider: (data: CreateProviderRequest): Promise<ApiResult<EmailProvider>> =>
    api.post<EmailProvider>('/api/email/providers', data),

  updateProvider: (id: string, data: UpdateProviderRequest): Promise<ApiResult<EmailProvider>> =>
  ...
  testProvider: (id: string): Promise<ApiResult<{ tested: boolean; ok: boolean }>> =>
    api.post<{ tested: boolean; ok: boolean }>(`/api/email/providers/${id}/test`, {}),

  // Templates
  listTemplates: (): Promise<ApiResult<EmailTemplate[]>> =>
    api.get<EmailTemplate[]>('/api/email/templates'),

  createTemplate: (data: CreateTemplateRequest): Promise<ApiResult<EmailTemplate>> =>
    api.post<EmailTemplate>('/api/email/templates', data),

  updateTemplate: (id: string, data: UpdateTemplateRequest): Promise<ApiResult<EmailTemplate>> =>
  ...
  renderTemplate: (
    ...
    api.post<TemplateRenderResult>(`/api/email/templates/${id}/render`, { variables }),

  // Queue
  enqueue: (data: EnqueueRequest): Promise<ApiResult<EmailQueueItem>> =>
    api.post<EmailQueueItem>('/api/email/queue', data),

  listQueue: (params?: { status?: string; limit?: number }): Promise<ApiResult<EmailQueueItem[]>> =>
  ...
  listDeliveryLog: (params?: {
    ...
    api.get<DeliveryLogEntry[]>(`/api/email/delivery-log${queryString(params)}`),
}
