...
export const recognitionApi = {
  ...
  async runInference(
  ...
  ): Promise<ApiResult<RecognitionResult>> {
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          error: (json as { error?: { code: string; message: string } })
            .error ?? {
            code: "SERVER_ERROR",
  ...
  generateGraphic(
    body: GenerateGraphicBody,
  ): Promise<ApiResult<GenerateGraphicResult>> {
  ...
  exportFeedback(): Promise<ApiResult<{ exported: boolean }>> {
    return api.post<{ exported: boolean }>(
      "/api/recognition/feedback/export",
      {},
    );
  },
  ...
  submitCorrections(
    corrections: unknown,
  ): Promise<ApiResult<{ correction_id: string }>> {
    return api.post<{ correction_id: string }>(
      "/api/recognition/feedback/corrections",
      corrections,
    );
  },
  ...
  deleteGapReport(id: string): Promise<ApiResult<{ deleted: boolean }>> {
    return api.delete<{ deleted: boolean }>(
      `/api/recognition/gap-reports/${id}`,
    );
  },

  /** Upload a .iogap gap report via multipart form */
  async uploadGapReport(
    file: File,
  ): Promise<ApiResult<{ imported: boolean; filename: string }>> {
    const token = localStorage.getItem(TOKEN_KEY);
    const form = new FormData();
    form.append("file", file, file.name);
    ...
    try {
      ...
      const envelope = json as {
        data?: { imported: boolean; filename: string };
      };
      return {
        success: true,
        data: envelope.data as { imported: boolean; filename: string },
      };
    } catch {
      return {
        success: false,
