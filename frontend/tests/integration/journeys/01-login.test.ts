  ...
  steps: [
    {
      ...
      verify: async (page) => {
        if (page.url().includes("/login"))
          throw new Error("Still on login page after auth");
    ...
    },
    {
      ...
      run: async (page) => {
        await takeNamedScreenshot(page, "01-app-shell");
      },
      verify: async (page) => {
        ...
        if (links.length < 3)
          throw new Error(
            `Expected at least 3 nav links, found ${links.length}`,
          );
        // User info visible in header
        const userBtn = await page.$('button:has-text("admin")');
        if (!userBtn)
          throw new Error('User button with "admin" not visible in header');
      },
    },
    {
      ...
      verify: async (page) => {
        if (page.url().includes("/login"))
          throw new Error("Navigation redirected to login — session lost");
        const heading = await page.$("h1");
        const text = heading ? await heading.textContent() : null;
        if (text?.trim() !== "Reports")
          throw new Error(
            `Expected h1 "Reports" after navigation, got "${text}"`,
          );
      },
    },
  ],
