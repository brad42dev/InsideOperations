  ...
  steps: [
    {
      ...
      verify: async (page) => {
        ...
        if (text?.trim() !== "Reports")
          throw new Error(`Expected h1 "Reports", got "${text}"`);
      },
    },
    {
      name: "report list renders",
      run: async (page) => {
        await takeNamedScreenshot(page, "05-reports");
      },
      verify: async (_page) => {
        ...
        throw new Error(
          "SKIP: Reports module fails to load — fill in when reports module is implemented",
        );
      },
    },
    {
      ...
      run: async (_page) => {
        /* select first available report and trigger generation */
      },
      verify: async (_page) => {
        // SKIP: requires working reports module and async generation pipeline
        throw new Error(
          "SKIP: fill in when async report generation pipeline is implemented",
        );
      },
    },
  ],
