  ...
  steps: [
    {
      ...
      verify: async (page) => {
        ...
        if (text?.trim() !== "Console")
          throw new Error(`Expected h1 "Console", got "${text}"`);
      },
    },
    {
      name: "console module has content",
      run: async (page) => {
        await takeNamedScreenshot(page, "02-console");
      },
      verify: async (_page) => {
        ...
        throw new Error(
          "SKIP: Console module fails to load — fill in when console module is implemented",
        );
      },
    },
    {
      ...
      run: async (_page) => {
        /* navigate to first workspace if list is non-empty */
      },
      verify: async (_page) => {
        // SKIP: requires OPC data and a working console module
        throw new Error(
          "SKIP: requires OPC data — fill in when OPC pipeline is connected",
        );
      },
    },
  ],
