...
export default {
  ...
  steps: [
    ...
    },
    {
      ...
      run: async (page) => {
        await takeNamedScreenshot(page, "04-data-binding");
      },
      verify: async (_page) => {
        // SKIP: requires OPC service running and working console/designer module
        throw new Error(
          "SKIP: requires OPC service — fill in when OPC pipeline is connected",
        );
      },
    },
    {
      ...
      run: async (_page) => {
        /* wait for value updates from live OPC data */
      },
      verify: async (_page) => {
        // SKIP: requires live OPC data from SimBLAH simulator
        throw new Error(
          "SKIP: requires live OPC data — fill in when SimBLAH is connected",
        );
      },
    },
  ],
