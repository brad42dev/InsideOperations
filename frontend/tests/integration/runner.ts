...
async function runAll() {
  const results: Array<{journey: string, steps: Array<{name: string, passed: boolean, skipped?: boolean, error?: string}>}> = [];

  for (const file of journeyFiles) {
    const mod = await import(`./journeys/${file.replace('.ts', '')}`);
    const journey: Journey = mod.default;
    console.log(`\n▶ ${journey.name}`);
    const stepResults: Array<{name: string, passed: boolean, skipped?: boolean, error?: string}> = [];
        ...
        for (const step of journey.steps) {
          try {
            ...
            if (e.message && e.message.startsWith('SKIP:')) {
              stepResults.push({ name: step.name, passed: true, skipped: true, error: e.message });
  ...
  console.log(`  Steps:    ${totalSteps} total, ${passedSteps} passed, ${skippedSteps} skipped, ${failedSteps} failed`);
