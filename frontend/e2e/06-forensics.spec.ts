...
test.describe('Forensics', () => {
  ...
  test('forensics module is accessible', async ({ page }) => {
    ...
    expect(violations, `axe violations on /forensics: ${JSON.stringify(violations.map(v => v.id))}`).toHaveLength(0)
  })

  test(
    'select 3+ points → set time range → run correlation → verify chart renders',
    async ({ page }) => {
      await page.goto('/forensics')
      await expect(page).toHaveURL(/\/forensics/)

      // --- Step 1: Open the point picker ---
      const addPointBtn = page.getByRole('button', { name: /add point|select point|add tag/i })
      ...
      for (const pointName of pointNames) {
        ...
        const firstResult = page.getByRole('option', { name: new RegExp(pointName.replace('.', '\\.'), 'i') }).first()
        const altResult = page.getByRole('row').filter({ hasText: pointName }).first()

        if (await firstResult.isVisible({ timeout: 3_000 }).catch(() => false)) {
          await firstResult.click()
        } else if (await altResult.isVisible({ timeout: 3_000 }).catch(() => false)) {
          await altResult.click()
        } else {
          // Point picker may have already added it or uses checkboxes
          const checkbox = page.getByRole('checkbox', { name: new RegExp(pointName.replace('.', '\\.'), 'i') })
        ...
        const confirmBtn = page.getByRole('button', { name: /confirm|done|add|ok/i })
      ...
      const selectedPoints = page.locator('[data-testid*="selected-point"], [class*="selected-point"], [class*="point-chip"]')
      ...
      const timeRangePicker = page.getByRole('button', { name: /time range|last 24|24h|1 day/i })
      if (await timeRangePicker.isVisible({ timeout: 3_000 }).catch(() => false)) {
        await timeRangePicker.click()
        // Select the 24h preset
        const preset24h = page.getByRole('option', { name: /24 hour|last 24|1 day/i })
      ...
      const runBtn = page.getByRole('button', { name: /run|analyze|correlate|calculate/i })
      ...
      await chartLocator.waitFor({ timeout: 15_000 }).catch(() => {
        test.skip(true, 'Correlation chart did not render — backend data not available')
      })
      ...
      const correlationValues = page.locator('[data-testid*="correlation-value"], [class*="correlation-value"], td[class*="matrix"]')
      ...
      expect(violationsResults, `axe violations on forensics results: ${JSON.stringify(violationsResults.map(v => v.id))}`).toHaveLength(0)
    }
  )
})
