...
test.describe('Login → Console → Live Data', () => {
  test('login page is accessible', async ({ page }) => {
    ...
    expect(violations, `axe violations on /login: ${JSON.stringify(violations.map(v => v.id))}`).toHaveLength(0)
  })
  ...
  test(
    'console is accessible and live data updates via WebSocket',
    async ({ page }) => {
      // Step 1: login
      await loginAs(page)
      ...
      const workspaceGrid = page.locator('[data-testid="workspace-grid"], [class*="workspace"], [class*="console"]').first()
      ...
      page.on('websocket', ws => {
        ws.on('framereceived', event => {
          ...
          if (data) {
            ...
            if (/\"value\"\s*:\s*[\d.-]+/.test(data) || /\"v\"\s*:\s*[\d.-]+/.test(data)) {
              wsDataReceived = true
            }
          }
        })
      ...
      if (wsDataReceived) {
        ...
        const dynamicValues = page.locator('[data-testid*="point-value"], [class*="point-value"], [class*="dynamic-value"]')
        ...
        test.skip(true, 'No live backend: WebSocket data not received within 8s — skipping live-data assertion')
      }
    }
  )
})
