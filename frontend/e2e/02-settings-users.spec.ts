...
test.describe("Settings — User management", () => {
  ...
  test("settings users page is accessible", async ({ page }) => {
    ...
    expect(
      violations,
      `axe violations on /settings/users: ${JSON.stringify(violations.map((v) => v.id))}`,
    ).toHaveLength(0);
  });
  ...
  test("can open create user dialog", async ({ page }) => {
    await page
      .getByRole("button", { name: /add user|new user|create/i })
      .click();
    ...
    expect(
      violations,
      `axe violations on create user dialog: ${JSON.stringify(violations.map((v) => v.id))}`,
    ).toHaveLength(0);
  });
});
