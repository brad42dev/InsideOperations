...
test.describe('Workspace CRUD @nightly', () => {
  ...
  test('full workspace CRUD lifecycle @nightly', async ({ page }) => {
    ...
    expect(violations, `axe violations on /console: ${JSON.stringify(violations.map(v => v.id))}`).toHaveLength(0)

    // Create a new workspace
    const newWorkspaceBtn = page.getByRole('button', { name: /new workspace|create workspace|add workspace/i })
    ...
    // Rename it
    const renameBtn = page.locator(`[data-testid*="workspace"]:has-text("${workspaceName}")`).getByRole('button', { name: /rename|edit/i })
    ...
    // Delete it
    const deleteBtn = page.locator(`[data-testid*="workspace"]`).last().getByRole('button', { name: /delete|remove/i })
    if (await deleteBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await deleteBtn.click()
      // Confirm deletion dialog
      const confirmDeleteBtn = page.getByRole('button', { name: /confirm|yes|delete/i }).last()
...
test.describe('Designer → Console pipeline @nightly', () => {
  ...
  test('import SVG → bind points → save → open in Console @nightly', async ({ page }) => {
    await page.goto('/designer')
    await expect(page).toHaveURL(/\/designer/)

    const violations = await checkA11y(page)
    expect(violations, `axe violations on /designer: ${JSON.stringify(violations.map(v => v.id))}`).toHaveLength(0)
    ...
    await importBtn.waitFor({ timeout: 8_000 }).catch(() => {
      test.skip(true, 'Import button not found in Designer — backend not available')
    })
    ...
    const textElement = page.locator('[data-tag="TEST.POINT.001"], text').first()
    if (await textElement.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await textElement.click()
      // Open point binding panel
      const bindBtn = page.getByRole('button', { name: /bind|assign point|link/i })
      if (await bindBtn.isVisible({ timeout: 2_000 }).catch(() => false)) {
        ...
        await page.getByRole('option', { name: /TEST\.POINT\.001/i }).first().click().catch(() => {})
        await page.getByRole('button', { name: /confirm|apply|bind/i }).first().click().catch(() => {})
      }
    }
    ...
    await expect(page.getByText(/saved|published/i)).toBeVisible({ timeout: 8_000 })
  })
})
...
test.describe('Full report generation @nightly', () => {
  ...
  test('generate a canned report as PDF @nightly', async ({ page }) => {
    ...
    const violations = await checkA11y(page)
    expect(violations, `axe violations on /reports: ${JSON.stringify(violations.map(v => v.id))}`).toHaveLength(0)

    // Select the first available canned report
    const reportItem = page.getByRole('row').filter({ hasText: /report|summary/i }).first()
    ...
    const downloadLink = page.getByRole('link', { name: /download|pdf|view report/i })
