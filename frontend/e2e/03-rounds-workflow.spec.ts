...
test.describe("Rounds workflow", () => {
  ...
  test("rounds page is accessible", async ({ page }) => {
    ...
    expect(
      violations,
      `axe violations on /rounds: ${JSON.stringify(violations.map((v) => v.id))}`,
    ).toHaveLength(0);
  });
  ...
  test("rounds page shows tabs", async ({ page }) => {
    ...
    await expect(
      page
        .getByRole("tab", { name: /active|schedule|history|templates/i })
        .first(),
    ).toBeVisible();
  });

  test("create template → generate instance → fill readings → verify 60px touch targets → submit", async ({
    page,
  }) => {
    await page.goto("/rounds");
    await expect(page).toHaveURL(/\/rounds/);
    ...
    expect(
      violationsLanding,
      `axe violations on /rounds landing: ${JSON.stringify(violationsLanding.map((v) => v.id))}`,
    ).toHaveLength(0);
    ...
    const createTemplateBtn = page.getByRole("button", {
      name: /new template|create template|add template/i,
    });
    await createTemplateBtn.waitFor({ timeout: 5_000 }).catch(() => {
      test.skip(
        true,
        "Create template button not found — skipping workflow test",
      );
    });
    ...
    const addItemBtn = page.getByRole("button", {
      name: /add item|add check|add step/i,
    });
    ...
    const generateBtn = page
      .getByRole("button", { name: /generate|start round|create instance/i })
      .first();
    ...
    await roundRow.waitFor({ timeout: 8_000 }).catch(() => {
      test.skip(
        true,
        "Round instance not found in Active tab — backend may be unavailable",
      );
    });
        ...
        document.querySelectorAll<HTMLElement>(selector).forEach((el) => {
          ...
          if (rect.width > 0 && rect.height > 0) {
            if (
              ...
              violations.push(
                `${selector} at (${Math.round(rect.x)},${Math.round(rect.y)}) is ${Math.round(rect.width)}x${Math.round(rect.height)}px — below 60px`,
              );
            }
          }
        });
    ...
    expect(
      violationsForm,
      `axe violations on round form: ${JSON.stringify(violationsForm.map((v) => v.id))}`,
    ).toHaveLength(0);
