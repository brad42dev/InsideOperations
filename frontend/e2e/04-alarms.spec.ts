...
test.describe("Alarms", () => {
  ...
  test("alarm definitions page is accessible", async ({ page }) => {
    ...
    expect(
      violations,
      `axe violations on /settings/alarm-definitions: ${JSON.stringify(violations.map((v) => v.id))}`,
    ).toHaveLength(0);
  });

  test("configure alarm threshold → inject value above threshold → verify alarm fires in UI", async ({
    page,
  }) => {
    ...
    const createBtn = page.getByRole("button", {
      name: /new alarm|create alarm|add alarm/i,
    });
    ...
    const conditionSelect = page
      .getByLabel(/condition|type|comparison/i)
      .first();
    if (
      await conditionSelect.isVisible({ timeout: 2_000 }).catch(() => false)
    ) {
      // Try to select "Greater Than" or equivalent
      await conditionSelect
        .selectOption({ label: "Greater Than" })
        .catch(async () => {
          // If it's a custom select, click and pick
          await conditionSelect.click();
          await page
            .getByRole("option", { name: /greater|high|>/i })
            .first()
            .click()
            .catch(() => {});
        });
    ...
    expect(
      violationsAfterCreate,
      `axe violations after alarm create: ${JSON.stringify(violationsAfterCreate.map((v) => v.id))}`,
    ).toHaveLength(0);
    ...
    const injected = await page.request.post(
      `${apiBase}/api/v1/test/inject-point-value`,
      {
    ...
    if (!injected.ok()) {
      ...
      test.skip(
        true,
        "Test injection endpoint not available — skipping live alarm fire assertion",
      );
    }
    ...
    const alarmRow = page
      .locator(`[data-alarm-name="${alarmName}"], tr:has-text("${alarmName}")`)
      .first();
    if (await alarmRow.isVisible({ timeout: 2_000 }).catch(() => false)) {
      const statusCell = alarmRow
        .locator('[data-testid="alarm-state"], [class*="alarm-state"], td')
        .first();
    ...
    expect(
      violationsAlarms,
      `axe violations on alarms view: ${JSON.stringify(violationsAlarms.map((v) => v.id))}`,
    ).toHaveLength(0);
  });
});
