...
test.describe("Emergency Alert", () => {
  test("alerts page is accessible", async ({ page }) => {
    ...
    expect(
      violations,
      `axe violations on /alerts: ${JSON.stringify(violations.map((v) => v.id))}`,
    ).toHaveLength(0);
  });

  test("send emergency alert → verify overlay in Console → acknowledge", async ({
    browser,
  }: {
    browser: Browser;
  }) => {
    ...
    try {
      ...
      expect(
        violationsConsole,
        `axe violations on /console (receiver): ${JSON.stringify(violationsConsole.map((v) => v.id))}`,
      ).toHaveLength(0);
      ...
      const sendAlertBtn = senderPage.getByRole("button", {
        name: /send alert|new alert|emergency|broadcast/i,
      });
      ...
      const messageField = senderPage
        .getByLabel(/message|description|details/i)
        .first();
      ...
      const recipientField = senderPage
        .getByLabel(/recipient|send to|audience/i)
        .first();
      if (
        await recipientField.isVisible({ timeout: 2_000 }).catch(() => false)
      ) {
        // Try selecting "All Users" option
        await recipientField
          .selectOption({ label: "All Users" })
          .catch(async () => {
            await recipientField.click();
            await senderPage
              .getByRole("option", { name: /all users|everyone|broadcast/i })
              .first()
              .click()
              .catch(() => {});
          });
      ...
      const severityField = senderPage
        .getByLabel(/severity|priority|level/i)
        .first();
      if (
        await severityField.isVisible({ timeout: 2_000 }).catch(() => false)
      ) {
        await severityField
          .selectOption({ label: "Emergency" })
          .catch(() => {});
      }
      ...
      expect(
        violationsForm,
        `axe violations on alert compose form: ${JSON.stringify(violationsForm.map((v) => v.id))}`,
      ).toHaveLength(0);

      // --- Step 4: Send the alert ---
      const confirmSendBtn = senderPage
        .getByRole("button", { name: /send|confirm|submit|broadcast/i })
        .last();
      await confirmSendBtn.click();

      // Confirm the alert was sent (success toast or state change)
      const successIndicator = senderPage.getByText(
        /sent|alert sent|success|broadcasted/i,
      );
      ...
      const alertOverlay = receiverPage
        .locator(
      ...
      const alertText = receiverPage.getByText(
        new RegExp(alertMessage.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"),
      );
      const overlayVisible = await alertOverlay
        .isVisible({ timeout: 2_000 })
        .catch(() => false);
      const textVisible = await alertText
        .isVisible({ timeout: 2_000 })
        .catch(() => false);
      ...
      if (!overlayVisible && !textVisible) {
        test.skip(
          true,
          "Alert overlay not visible on receiver page — real-time push may require live WebSocket backend",
        );
      }

      // --- Step 6: Acknowledge the alert on the receiver page ---
      const acknowledgeBtn = receiverPage.getByRole("button", {
        name: /acknowledge|ack|dismiss|ok/i,
      });
      ...
      await senderPage.goto("/alerts");
      const ackedIndicator = senderPage
        .getByText(new RegExp(`${alertMessage}`, "i"))
        .locator("..")
        .getByText(/acknowledged|ack'd|resolved/i);
      ...
      expect(
        violationsSender,
        `axe violations on sender /alerts (post-send): ${JSON.stringify(violationsSender.map((v) => v.id))}`,
      ).toHaveLength(0);
    } finally {
